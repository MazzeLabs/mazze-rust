...
use mazzestore::{
    ...
    mazzekey::{Address, Password, Secret},
    import_accounts, MazzeStore, PresaleWallet, SecretStore, SecretVaultRef,
...
Usage:
    ...
    mazzestore import [<password>] [--src DIR] [--dir DIR]
...
enum Error {
    Ethstore(mazzestore::Error),
    Docopt(docopt::Error),
}

impl From<mazzestore::Error> for Error {
    fn from(err: mazzestore::Error) -> Self { Error::Ethstore(err) }
}
...
fn open_args_vault_account(
    store: &MazzeStore, address: Address, args: &Args,
...
fn load_password(path: &str) -> Result<Password, Error> {
    let mut file = fs::File::open(path).map_err(|e| {
        mazzestore::Error::Custom(format!(
    ...
    S: AsRef<str>,
{
    ...
    let store = MazzeStore::open(key_dir(&args.flag_dir, None)?)?;

    return if args.cmd_insert {
        let secret = args
            .arg_secret
            .parse()
            .map_err(|_| mazzestore::Error::InvalidSecret)?;
        ...
        let address = args
            .arg_address
            .parse()
            .map_err(|_| mazzestore::Error::InvalidAccount)?;
        ...
        let message = args
            .arg_message
            .parse()
            .map_err(|_| mazzestore::Error::InvalidMessage)?;
        let password = load_password(&args.arg_password)?;
        let account_ref = open_args_vault_account(&store, address, &args)?;
