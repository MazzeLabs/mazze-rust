...
use mazze_types::address_util::{self, AddressUtil};
...
//
// In Mazze we have so far only one type of account key format. So we try to
...
// In Mazze we only have 160 bits hash size, however we keep these unused
...
// Prefixes
const MAINNET_PREFIX: &str = "mazze";
