...
impl ExtendedPublic {
    ...
    pub fn new(public: Public, chain_code: H256) -> Self {
        ExtendedPublic { public, chain_code }
    }
...
impl ExtendedKeyPair {
    ...
    pub fn with_seed(seed: &[u8]) -> Result<ExtendedKeyPair, DerivationError> {
        ...
        Ok(ExtendedKeyPair::with_secret(
            Secret::from_unsafe_slice(master_key.as_bytes())
...
mod derivation {
    ...
    fn hmac_pair(
        data: &[u8], private_key: H256, chain_code: H256,
    ) -> (H256, H256) {
        ...
        let child_key =
            BigEndianHash::from_uint(&private_add(hmac_key, private));
...
mod tests {
    ...
    fn h256_soft_match() {
        ...
        let derived_secret0 =
            extended_secret.derive(Derivation::Soft(derivation_secret));
