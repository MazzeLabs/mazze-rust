...
use mazzestore::{
    ...
    mazzekey::{verify_address, Generator, KeyPair, Random, Secret},
    MazzeStore, SecretVaultRef, SimpleSecretStore, StoreAccountRef,
};
use std::str::FromStr;
...
fn secret_store_create() {
    ...
    let _ = MazzeStore::open(Box::new(dir)).unwrap();
}
...
fn secret_store_create_account() {
    let dir = TransientDir::create().unwrap();
    let store = MazzeStore::open(Box::new(dir)).unwrap();
...
fn test_path() -> &'static str {
    match ::std::fs::metadata("mazzestore") {
        Ok(_) => "mazzestore/tests/res/geth_keystore",
...
fn ciphertext_path() -> &'static str {
    match ::std::fs::metadata("mazzestore") {
        Ok(_) => "mazzestore/tests/res/ciphertext",
