...
impl MazzeStore {
    ...
    pub fn open_with_iterations(
        ...
        Ok(MazzeStore {
            store: MazzeMultiStore::open_with_iterations(directory, iterations)?,
        })
    }
    ...
    /// instance of `MazzeStore` are taken into account.
...
impl SimpleSecretStore for MazzeStore {
    fn insert_account(
...
impl SecretStore for MazzeStore {
    fn raw_secret(
        &self, account: &StoreAccountRef, password: &Password,
...
/// Similar to `MazzeStore` but may store many accounts (with different passwords)
/// for the same `Address`
pub struct MazzeMultiStore {
            ...
            Derivation::Hierarchical(path) => {
                for path_item in path {
                    extended = extended.derive(
                        if path_item.soft {
                            mazzekey::Derivation::Soft(path_item.index)
                        } else {
                            mazzekey::Derivation::Hard(path_item.index)
                        },
                    )?;
                }
            }
            Derivation::SoftHash(h256) => {
                extended = extended.derive(mazzekey::Derivation::Soft(h256))?;
            }
            Derivation::HardHash(h256) => {
                extended = extended.derive(mazzekey::Derivation::Hard(h256))?;
            }
        }
        Ok(extended)
    }
...
impl SimpleSecretStore for MazzeMultiStore {
    fn insert_account(
        &self, vault: SecretVaultRef, secret: Secret, password: &Password,
    ) -> Result<StoreAccountRef, Error> {
        let keypair =
    ...
    fn generate_derived(
        ...
        if let Some(account) = accounts.first() {
            let extended =
                self.generate(account.crypto.secret(password)?, derivation)?;
            Ok(mazzekey::public_to_address(extended.public().public(), true))
        } else {
            Err(Error::InvalidPassword)
        }
    }
    ...
    fn sign_derived(
        ...
        if let Some(account) = accounts.first() {
            ...
            Ok(mazzekey::sign(&secret, message)?)
        } else {
            Err(Error::InvalidPassword)
        }
    }
...
mod tests {
    ...
    fn store() -> MazzeStore {
        MazzeStore::open(Box::new(MemoryDirectory::default()))
            .expect("MemoryDirectory always load successfuly; qed")
    }

    fn multi_store() -> MazzeMultiStore {
        MazzeMultiStore::open(Box::new(MemoryDirectory::default()))
            .expect("MemoryDirectory always load successfuly; qed")
    }
    ...
    fn should_create_and_open_vaults() {
        ...
        let store = MazzeStore::open(dir.key_dir.take().unwrap()).unwrap();
        let name1 = "vault1";
