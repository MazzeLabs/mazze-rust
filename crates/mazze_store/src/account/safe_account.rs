...
impl SafeAccount {
				...
				match json_address {
					Some(json_address) => {
						...
						if derived_address != json_address {
                            warn!("Detected address mismatch when opening an account. Derived: {:?}, in json got: {:?}. Are you trying to import an Ethkey for Mazze? Note that the address scheme between Ethereum and Mazze are different.", derived_address, json_address);
    ...
    pub fn from_vault_file(
        ...
        if !Address::from_slice(&meta_plain.address).is_user_account_address() {
            warn!("Trying to import a non-user type account address. Are you trying to import an Ethkey for Mazze? Note that the address scheme between Ethereum and Mazze are different.");
    ...
    pub fn decrypt(
        ...
        let secret = self.crypto.secret(password)?;
        mazzekey::crypto::ecies::decrypt(&secret, shared_mac, message)
            .map_err(From::from)
    }
...
mod tests {
    ...
    use mazzekey::{verify_public, Generator, Message, Random};
