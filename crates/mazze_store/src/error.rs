...
pub enum Error {
    ...
    /// `EthKey` error
    EthKey(EthKeyError),
    /// `mazzekey::crypto::Error`
    EthKeyCrypto(mazzekey::crypto::Error),
    /// `EthCrypto` error
...
impl From<mazzekey::crypto::Error> for Error {
    fn from(err: mazzekey::crypto::Error) -> Self { Error::EthKeyCrypto(err) }
}
