...
Mazze key management. It is built on top of the Parity Ethereum counterpart.

### Usage

```
Mazze key management tool.

Usage:
    mazzestore insert <secret> <password> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore change-pwd <address> <old-pwd> <new-pwd> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore list [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore import [--src DIR] [--dir DIR]
    mazzestore import-wallet <path> <password> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore find-wallet-pass <path> <password>
    mazzestore remove <address> <password> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore sign <address> <password> <message> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore public <address> <password> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore list-vaults [--dir DIR]
    mazzestore create-vault <vault> <password> [--dir DIR]
    mazzestore change-vault-pwd <vault> <old-pwd> <new-pwd> [--dir DIR]
    mazzestore move-to-vault <address> <vault> <password> [--dir DIR] [--vault VAULT] [--vault-pwd VAULTPWD]
    mazzestore move-from-vault <address> <vault> <password> [--dir DIR]
    mazzestore [-h | --help]
...
```
mazzestore insert 7d29fab185a33e2cd955812397354c472d2b84615b645aa135ff539f6b0d70d5 password.txt
```
...
```
mazzestore insert `mazzekey generate random -s` "this is sparta"
```
...
```
mazzestore change-pwd a8fa5dd30a87bb9e3288d604eb74949c515ab66e old_pwd.txt new_pwd.txt
```
...
```
mazzestore import-wallet ethwallet.json password.txt
```
...
```
mazzestore find-wallet-pass ethwallet.json passwords.txt
```
...
```
mazzestore remove a8fa5dd30a87bb9e3288d604eb74949c515ab66e password.txt
```
...
```
mazzestore sign 24edfff680d536a5f6fe862d36df6f8f6f40f115 password.txt 7d29fab185a33e2cd955812397354c472d2b84615b645aa135ff539f6b0d70d5
```
...
```
mazzestore public 00e63fdb87ceb815ec96ae185b8f7381a0b4a5ea account_password.txt --vault vault_name --vault-pwd vault_password.txt
```
...
```
mazzestore create-vault vault3 vault3_password.txt
```
...
```
mazzestore change-vault-pwd vault3 vault3_password.txt new_vault3_password.txt
```
...
```
mazzestore move-to-vault 00e63fdb87ceb815ec96ae185b8f7381a0b4a5ea vault3 vault3_password.txt
mazzestore move-to-vault 00e63fdb87ceb815ec96ae185b8f7381a0b4a5ea vault1 vault1_password.txt --vault vault3 --vault-pwd vault3_password.txt
```
...
```
mazzestore move-from-vault 00e63fdb87ceb815ec96ae185b8f7381a0b4a5ea vault1 vault1_password.txt
```
