...
use mazze_vm_types::{contract_address, CreateContractAddress};
use primitives::{
    receipt::{
        ...
        let (address, action, space) = match transaction.unsigned {
            ...
            Transaction::Ethereum(ref unsigned) => {
                if include_eth_receipt {
                    ...
                    bail!(format!("Does not support EIP-155 transaction in Mazze space RPC. get_receipt for tx: {:?}",transaction));
                }
            }
        };
