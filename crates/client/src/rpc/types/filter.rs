...
pub enum MazzeFilterLog {
    Log(Log),
    ChainReorg(RevertTo),
}

impl Serialize for MazzeFilterLog {
    fn serialize<S>(&self, s: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        match *self {
            MazzeFilterLog::Log(ref log) => log.serialize(s),
            MazzeFilterLog::ChainReorg(ref revert_to) => revert_to.serialize(s),
        }
    }
}
...
pub enum MazzeFilterChanges {
    /// New logs.
    Logs(Vec<MazzeFilterLog>),
...
impl Serialize for MazzeFilterChanges {
    fn serialize<S>(&self, s: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        match *self {
            MazzeFilterChanges::Logs(ref logs) => logs.serialize(s),
            MazzeFilterChanges::Hashes(ref hashes) => hashes.serialize(s),
            MazzeFilterChanges::Empty => (&[] as &[Value]).serialize(s),
        }
    }
}
...
#[cfg(test)]
mod tests {
    use crate::rpc::types::{filter::RevertTo, MazzeFilterLog, Log};

    use super::{
        super::RpcAddress, MazzeRpcLogFilter, EpochNumber, VariadicValue,
    };
    use mazze_addr::Network;
    use mazze_types::{Space, H160, H256, U256, U64};
    use primitives::{
    ...
    fn test_serialize_filter() {
        ...
        assert_eq!(
            ...
            "{\
             ...
             \"address\":[\"MAZZE:TYPE.NULL:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA0SFBNJM2\",\"MAZZE:TYPE.BUILTIN:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEJC4EYEY6\"],\
             \"topics\":[\
    ...
    fn test_deserialize_filter() {
        ...
        let result_filter = MazzeRpcLogFilter {
            from_epoch: None,
            to_epoch: None,
            from_block: None,
            to_block: None,
        ...
        let deserialized_filter: MazzeRpcLogFilter =
    ...
    fn test_convert_filter() {
        let epoch_filter = MazzeRpcLogFilter {
        ...
        let block_number_filter = MazzeRpcLogFilter {
            from_epoch: None,
            to_epoch: None,
        ...
        let block_hash_filter = MazzeRpcLogFilter {
            from_epoch: None,
            to_epoch: None,
            from_block: None,
            to_block: None,
    ...
    #[test]
    fn test_serialize_mazze_filter_log() {
        let mut logs = vec![];
        let log = Log {
