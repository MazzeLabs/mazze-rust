...
pub enum RpcTransactionPayload {
    ...
    ///
    MainDecision(MainDecisionPayload),
    ///
...
impl From<TransactionPayload> for RpcTransactionPayload {
    fn from(payload: TransactionPayload) -> Self {
        match payload {
            ...
            TransactionPayload::MainDecision(p) => {
                RpcTransactionPayload::MainDecision(MainDecisionPayload {
...
#[derive(Debug, Serialize, Clone)]
#[serde(rename_all = "camelCase")]
pub struct MainDecisionPayload {
    pub height: U64,
    pub block_hash: H256,
}
    ...
    where S: Serializer {
        ...
        if self.payload.is_some() {
            match &self.payload.as_ref().unwrap() {
                ...
                RpcTransactionPayload::MainDecision(p) => {
...
pub fn tx_type(payload: TransactionPayload) -> RpcTransactionType {
    match payload {
        ...
        TransactionPayload::MainDecision(_) => {
            RpcTransactionType::MainDecision
        }
