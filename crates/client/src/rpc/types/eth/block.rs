...
use mazze_types::{hexstr_to_h256, Bloom as H2048, Space, H160, H256, H64, U256};
use mazzecore::consensus::PhantomBlock;
...
impl Block {
    pub fn from_phantom(pb: &PhantomBlock, full: bool) -> Self {
                ...
                pb.transactions
                    ...
                    .map(|(idx, t)| {
                        ...
                        Transaction::from_signed(
                            &**t,
                            (
                                Some(pb.main_header.hash()),          // block_hash
                                Some(pb.main_header.height().into()), // block_number
        ...
        Block {
            hash: pb.main_header.hash(),
            parent_hash: pb.main_header.parent_hash().clone(),
            uncles_hash: hexstr_to_h256(SHA3_HASH_OF_EMPTY_UNCLE),
            author: pb.main_header.author().clone(),
            miner: pb.main_header.author().clone(),
            state_root: pb.main_header.deferred_state_root().clone(),
            transactions_root: pb.main_header.transactions_root().clone(),
            receipts_root: pb.main_header.deferred_receipts_root().clone(),
            // We use height to replace block number for ETH interface.
            // Note: this will correspond to the epoch number.
            number: pb.main_header.height().into(),
            gas_used: pb
                .receipts
                .last()
                .map(|r| r.accumulated_gas_used)
                .unwrap_or_default(),
            gas_limit: pb.main_header.gas_limit().into(),
            extra_data: Default::default(),
            logs_bloom: pb.bloom,
            timestamp: pb.main_header.timestamp().into(),
            difficulty: pb.main_header.difficulty().into(),
            ...
            nonce: pb.main_header.nonce().low_u64().to_be_bytes().into(),
...
mod tests {
    ...
    use mazze_types::{Bloom as H2048, H160, H256, H64, U256};
