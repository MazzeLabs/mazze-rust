    ...
    where S: Serializer {
        match self {
            ...
            BlockHashOrEpochNumber::BlockHashWithOption {
                ...
                if let Some(require_main) = require_main {
...
// the `require_main` field is designed to be Option<bool>
// if input is "hash:0x..." then `require_main` will be None
// else if input is a object { blockHash: 0x... }
// the `require_main` will be Some and default to true
impl<'a> Visitor<'a> for BlockHashOrEpochNumberVisitor {
    ...
    where V: MapAccess<'a> {
        // require_main defaults to true if input is a map
        let (mut require_main, mut epoch_number, mut block_hash) =
        ...
        loop {
            ...
            match key_str {
                Some(key) => match key.as_str() {
                    ...
                    "requireMain" => {
                        require_main = visitor.next_value()?;
                    }
                    key => {
        ...
        if let Some(hash) = block_hash {
            return Ok(BlockHashOrEpochNumber::BlockHashWithOption {
                hash,
                require_main: Some(require_main),
            });
        }
...
mod tests {
    ...
    fn block_hash_or_epoch_number_deserialization() {
        let s = r#"[
			...
			{"blockHash": "0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347", "requireMain": false}
		]"#;
