...
use crate::rpc::{
    ...
    traits::mazze::MazzeFilter,
    types::{MazzeFilterChanges, MazzeFilterLog, MazzeRpcLogFilter, Log, RevertTo},
};
use mazze_addr::Network;
use mazze_types::{Space, H128, H256};
use mazzecore::{
    channel::Channel, rpc_errors::Error as MazzeRpcError, BlockDataManager,
...
/// Mazze filter rpc implementation for a full node.
pub struct MazzeFilterClient {
...
impl MazzeFilterClient {
    /// Creates new Mazze filter client.
    pub fn new(
        ...
        let filter_client = MazzeFilterClient {
            consensus,
            tx_pool,
...
impl Filterable for MazzeFilterClient {
    /// Current best epoch number.
    fn best_executed_epoch_number(&self) -> u64 {
    ...
    fn epochs_since_last_request(
        ...
        for i in 0..recent_reported_epochs.len() {
            ...
            if num < end_epoch_number {
                ...
                debug!("reorg for {}, main hash {:?}", num, main_hash);
                reorg_epochs.push((num, main_hash));
            }
            reorg_len += 1;
        }
...
impl<T: Filterable + Send + Sync + 'static> MazzeFilter for T {
    /// Returns id of new filter.
    fn new_filter(&self, filter: MazzeRpcLogFilter) -> JsonRpcResult<H128> {
    ...
    fn filter_changes(&self, index: H128) -> JsonRpcResult<MazzeFilterChanges> {
        ...
        filter.modify(|filter| match *filter {
            PollFilter::Block {
                ...
                Ok(MazzeFilterChanges::Hashes(hashes))
            }
            PollFilter::PendingTransaction(ref mut previous_hashes) => {
                ...
                *previous_hashes = current_hashes;
                ...
                // return new hashes
                Ok(MazzeFilterChanges::Hashes(new_hashes))
            }
            PollFilter::Logs {
                ref mut last_epoch_number,
                ref mut recent_reported_epochs,
                ...
                for (num, blocks) in epochs.into_iter() {
                    ...
                    log.iter()
                        // .map(|l| MazzeFilterLog::Log(l))
                        .for_each(|l| logs.push(MazzeFilterLog::Log(l.clone())));
                ...
                Ok(MazzeFilterChanges::Logs(limit_logs(
                    logs,
...
fn retrieve_epoch_logs(
    ...
    let fut = hashes
        .iter()
        .map(|h| retrieve_block_receipts(&data_man, h, &main));
...
fn retrieve_block_receipts(
    data_man: &Arc<BlockDataManager>, block: &H256, main: &H256,
