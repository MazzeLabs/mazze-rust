...
use mazze_execute_helper::exec_tracer::TraceFilter as PrimitiveTraceFilter;
                            ...
                            .map(|traces| {
                                traces
                                    ...
                                    .map(|trace| RpcLocalizedTrace {
                                        ...
                                        valid: trace.valid,
                                        epoch_hash: Some(main_hash),
                                        epoch_number: Some(
                                            self.data_man
                                                .block_height_by_hash(
                                                    &main_hash,
                                                )
                                                .expect("main block missing")
                                                .into(),
                                        ),
...
impl EthTrace for EthTraceHandler {
    fn block_traces(
        ...
        let block_number = phantom_block.main_header.height();
        let block_hash = phantom_block.main_header.hash();
    ...
    fn transaction_traces(
        ...
        for (action, result, subtraces) in trace_pairs {
            let mut eth_trace = EthLocalizedTrace {
                ...
                block_hash: phantom_block.main_header.hash(),
                // action and its result should have the same `valid`.
                valid: action.valid,
            };
