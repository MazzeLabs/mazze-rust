...
use self::{
    impls::{
        mazze::{MazzeHandler, LocalRpcImpl, RpcImpl, TestRpcImpl},
        mazze_filter::MazzeFilterClient,
        common::RpcImpl as CommonImpl,
        eth_pubsub::PubSubClient as EthPubSubClient,
        light::{
            MazzeHandler as LightMazzeHandler, DebugRpcImpl as LightDebugRpcImpl,
...
fn setup_rpc_apis(
    ...
    for api in &apis {
        match api {
            Api::Mazze => {
                let mazze =
                    MazzeHandler::new(common.clone(), rpc.clone()).to_delegate();
                ...
                if let Some(poll_lifetime) = rpc.config.poll_lifetime_in_seconds
                {
                    if let Some(h) = pubsub.handler().upgrade() {
                        let filter_client = MazzeFilterClient::new(
                            rpc.consensus.clone(),
                            rpc.tx_pool.clone(),
...
fn setup_rpc_apis_light(
    ...
    for api in apis {
        match api {
            Api::Mazze => {
                let mazze = LightMazzeHandler::new(common.clone(), rpc.clone())
                    .to_delegate();
                let interceptor = ThrottleInterceptor::new(
                    throttling_conf,
                    throttling_section,
                );
                handler.extend_with(RpcProxy::new(mazze, interceptor));
            }
            Api::Eth => {
