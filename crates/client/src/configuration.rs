...
build_config! {
    {
        ...
        (metrics_influxdb_db, (String), "mazze".into())
        ...
        (outlier_penalty_ratio, (u64), OUTLIER_PENALTY_RATIO)
        ...
        (storage_delta_mpts_node_map_vec_size, (u32), mazze_storage::defaults::MAX_CACHED_TRIE_NODES_R_LFU_COUNTER)
        (storage_delta_mpts_slab_idle_size, (u32), mazze_storage::defaults::DEFAULT_DELTA_MPTS_SLAB_IDLE_SIZE)
        (storage_single_mpt_cache_size, (u32), mazze_storage::defaults::DEFAULT_DELTA_MPTS_CACHE_SIZE * 2)
        (storage_single_mpt_cache_start_size, (u32), mazze_storage::defaults::DEFAULT_DELTA_MPTS_CACHE_START_SIZE * 2)
        (storage_single_mpt_slab_idle_size, (u32), mazze_storage::defaults::DEFAULT_DELTA_MPTS_SLAB_IDLE_SIZE * 2)
        (storage_max_open_snapshots, (u16), mazze_storage::defaults::DEFAULT_MAX_OPEN_SNAPSHOTS)
        (storage_max_open_mpt_count, (u32), mazze_storage::defaults::DEFAULT_MAX_OPEN_MPT)
...
impl Configuration {
    ...
    pub fn storage_config(&self, node_type: &NodeType) -> StorageConfiguration {
        let mazze_data_path = Path::new(&self.raw_conf.mazze_data_dir);
        StorageConfiguration {
            ...
            path_delta_mpts_dir: mazze_data_path
                .join(&*storage_dir::DELTA_MPTS_DIR),
            path_snapshot_dir: mazze_data_path
    ...
    pub fn txpool_config(&self) -> TxPoolConfig {
        let (min_native_tx_price_default, min_eth_tx_price_default) =
            if self.is_test_or_dev_mode() {
                ...
                (ONE_GMAZZY_IN_MAZZY, 20 * ONE_GMAZZY_IN_MAZZY)
            };
        TxPoolConfig {
    ...
    pub fn common_params(&self) -> CommonParams {
        ...
        params.outlier_penalty_ratio = self.raw_conf.outlier_penalty_ratio;
...
pub fn parse_config_address_string(
    ...
    let base32_err = match mazze_addr_decode(addr) {
        Ok(address) => {
...
mod tests {
    ...
    fn test_config_address_string() {
        ...
        assert_eq!(
            addr,
            parse_config_address_string(
                "mazze:type.user:aarc9abycue0hhzgyrr53m6cxedgccrmmyybjgh4xg",
                &Network::Main,
            )
            .unwrap()
        );
