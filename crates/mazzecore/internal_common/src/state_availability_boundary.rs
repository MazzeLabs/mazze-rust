...
pub struct StateAvailabilityBoundary {
    /// This is the hash of blocks in main chain based on current graph.
    #[derivative(Debug = "ignore")]
    pub main_chain: Vec<H256>,
...
impl StateAvailabilityBoundary {
    ...
    pub fn check_availability(&self, height: u64, block_hash: &H256) -> bool {
        (height == 0 || height != self.synced_state_height)
            ...
            && {
                let r = self.main_chain[(height - self.lower_bound) as usize]
                    == *block_hash;
                if !r {
                    debug!(
                        "main_chain={:?} should be {:?} asked is {:?}",
                        self.main_chain,
                        self.main_chain[(height - self.lower_bound) as usize],
                        block_hash
                    );
                }
                r
    ...
    pub fn adjust_upper_bound(&mut self, executed_block: &BlockHeader) {
        ...
        if next_index < self.main_chain.len()
            && executed_block.height() == self.upper_bound + 1
            && executed_block.hash() == self.main_chain[next_index]
        {
            self.upper_bound += 1;
        }
    }
