...
use mazze_vm_interpreter::{FinalizationResult, GasPriceTier};
use mazze_vm_types::{
...
fn test_sender_balance() {
    ...
    let storage_limit_in_mazzy = U256::MAX;
    ...
    assert_eq!(
        ...
        *COLLATERAL_MAZZIES_PER_STORAGE_KEY + U256::from(0x100)
    );
    let env = Env::default();
    ...
    assert_eq!(
        ...
        *COLLATERAL_MAZZIES_PER_STORAGE_KEY + U256::from(0xf9)
    );
...
fn test_call_to_create() {
    ...
    let storage_limit_in_mazzy = *MAZZIES_PER_STORAGE_COLLATERAL_UNIT
    ...
    state
        .add_balance(
            &sender_with_space,
            &(U256::from(100) + storage_limit_in_mazzy),
            CleanupMode::NoEmpty,
        )
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(state.total_storage_tokens(), storage_limit_in_mazzy);
...
fn test_storage_commission_privilege() {
    ...
    let tx = Transaction::from(NativeTransaction {
        ...
        gas_price: U256::from(1),
        gas: U256::from(100_000),
        value: *COLLATERAL_MAZZIES_PER_STORAGE_KEY,
        action: Action::Call(address.address),
    ...
    state
        .add_balance(
            &caller1.address().with_native_space(),
            &(*COLLATERAL_MAZZIES_PER_STORAGE_KEY + U256::from(1000_000)),
            CleanupMode::NoEmpty,
        )
        .unwrap();
    state
    ...
    assert_eq!(
        state.collateral_for_storage(&sender.address()).unwrap(),
        *COLLATERAL_MAZZIES_PER_STORAGE_KEY * U256::from(1),
    );
