...
impl<'a> ContextTrait for Context<'a> {
    ...
    ) -> mazze_statedb::Result<::std::result::Result<MessageCallResult, TrapKind>>
    {
    ...
    where Self: Sized {
        ...
        if self.space == Space::Native {
            ...
            let collateral_in_mazzies = U256::from(collateral_units_for_code)
                * *MAZZIES_PER_STORAGE_COLLATERAL_UNIT;
            debug!("ret()  collateral_for_code={:?}", collateral_in_mazzies);
...
mod tests {
    ...
    use mazze_parameters::consensus::TRANSACTION_DEFAULT_EPOCH_BOUND;
    use mazze_storage::{
    ...
    use mazze_vm_types::{Context as ContextTrait, Env, Spec};
