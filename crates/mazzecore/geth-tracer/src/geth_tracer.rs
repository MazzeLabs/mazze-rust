...
impl OpcodeTracer for GethTracer {
    ...
    fn initialize_interp(&mut self, gas_limit: mazze_types::U256) {
        self.inner
            .gas_inspector
    ...
    fn log(
        &mut self, _address: &mazze_types::Address,
        topics: &Vec<mazze_types::H256>, data: &[u8],
    ) {
    ...
    fn selfdestruct(
        &mut self, _contract: &mazze_types::Address, target: &mazze_types::Address,
        _value: mazze_types::U256,
    ) {
        if self.is_fourbyte_tracer() {
            return;
        }
