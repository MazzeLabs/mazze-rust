...
pub enum CreateContractAddress {
    ...
    /// `create` scheme.
    FromSenderNonce,
    /// Address is calculated from sender, nonce, and code hash. Mazze
    /// `create` scheme.
    FromSenderNonceAndCodeHash,
    /// Address is calculated from block_hash, sender, nonce and code_hash.
    /// Potential new Mazze `create` scheme when kill_dust is enabled.
    FromBlockNumberSenderNonceAndCodeHash,
    /// Address is calculated from sender, salt and code hash. Mazze and
...
pub fn contract_address(
    ...
    let (address, code_hash) = match address_scheme {
        ...
        CreateContractAddress::FromBlockNumberSenderNonceAndCodeHash => {
            ...
            // // In Mazze, we take block_number and CodeHash into address
            ...
            // // In Mazze, we use the first four bits to indicate the type of
        ...
        CreateContractAddress::FromSenderNonceAndCodeHash => {
            ...
            // In Mazze, we append CodeHash to determine the address as well.
        ...
        CreateContractAddress::FromSenderSaltAndCodeHash(salt) => {
            ...
            buffer[(1 + 20 + 32)..].copy_from_slice(&code_hash[..]);
            // In Mazze, we use the first bit to indicate the type of the
