...
use mazze_internal_common::state_root_with_aux_info::{
    StateRootAuxInfo, StateRootWithAuxInfo,
};
use mazze_statedb::{StateDb, StateDbExt};
use mazze_storage::{
...
use mazze_types::{Address, AddressSpaceUtil, AddressWithSpace, H256};
use mazzecore::sync::Error;
...
// cargo run --release -p mazzecore --example snapshot_merge_test
// cargo run --release -p mazzecore --example snapshot_merge_test -- --help
fn main() -> Result<(), Error> {
    ...
    let info = SnapshotInfo {
        ...
        main_chain_parts: vec![snapshot1_epoch],
    ...
    let info = SnapshotInfo {
        ...
        main_chain_parts: vec![snapshot2_epoch],
        serve_one_step_sync: false,
        snapshot_info_kept_to_provide_sync: Default::default(),
    };
    ...
    let info = SnapshotInfo {
        ...
        merkle_root: Default::default(),
        parent_snapshot_epoch_id: NULL_EPOCH,
        main_chain_parts: vec![snapshot3_epoch],
        serve_one_step_sync: false,
        snapshot_info_kept_to_provide_sync: Default::default(),
    };
