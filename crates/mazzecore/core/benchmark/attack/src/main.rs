...
/// Assume `main[i]` be the `i`-th block in main chain, we generate a
/// `block[i]` whose parent is `main[i - 3]` and which has only one referee
...
fn out_of_memory_attack2() {
    ...
    for i in 1..100000 {
        // insert a main block
        let (new_hash, mut new_block) =
            create_simple_block(sync.clone(), main[i - 1], Vec::new(), 1);
        main.push(new_hash);
        sync.insert_block_header(
            &mut new_block.block_header,
            false, // need_to_verify
        ...
        if i >= 3 {
            ...
            let (new_hash, mut new_block) =
                create_simple_block(sync.clone(), main[i - 3], ref_hashes, 1);
            block.push(new_hash);
            sync.insert_block_header(
                &mut new_block.block_header,
                false, // need_to_verify
...
/// `on_new_block` call. Assume `main[i]` be the `i`-th block in main chain,
/// we generate a `block[i]` whose parent is `main[i - 3]` and which has only
/// one referee `main[i + era_epoch_count]`. We can see that `block[i]` can be
