...
impl ChunkRequestInfo {
    pub fn new(
        version: u64, peers: Vec<PeerNetworkId>, multicast_level: NetworkId,
    ) -> Self {
...
enum PeerScoreUpdateType {
    Success,
    EmptyChunk,
    // A received chunk cannot be directly applied (old / wrong version). Note
    // that it could happen that a peer would first timeout and would then
    // be punished with ChunkVersionCannotBeApplied.
...
pub struct RequestManager {
    ...
    // duration with the same version before the next attempt to get the next
    // chunk
    request_timeout: Duration,
    // duration with the same version before multicasting, i.e. sending the
    // next chunk request to more networks
    multicast_timeout: Duration,
    // The maximum network level that chunk requests will be sent to. The
    // greater the network level, the more networks this node will send
    // multicast requests to. Network ordering is defined by NetworkId.
...
impl RequestManager {
    pub fn new(
        request_timeout: Duration, multicast_timeout: Duration,
        network_senders: HashMap<NodeNetworkId, StateSyncSender>,
    ) -> Self
    {
    ...
    pub fn enable_peer(
        &mut self, peer: PeerNetworkId, metadata: ConnectionMetadata,
    ) -> Result<(), Error> {
        ...
        diem_info!(LogSchema::new(LogEntry::NewPeer)
            .peer(&peer)
            .is_valid_peer(true));
        ...
        match self.peer_scores.entry(peer) {
            Occupied(occupied_entry) => {
                diem_warn!(LogSchema::new(LogEntry::NewPeerAlreadyExists)
                    .peer(occupied_entry.key()));
            }
            Vacant(vacant_entry) => {
    ...
    pub fn disable_peer(&mut self, peer: &PeerNetworkId) -> Result<(), Error> {
        diem_info!(LogSchema::new(LogEntry::LostPeer).peer(&peer));
        ...
        if self.peer_scores.contains_key(peer) {
            ...
            diem_warn!(LogSchema::new(LogEntry::LostPeerNotKnown).peer(&peer));
        }

        Ok(())
    }

    pub fn no_available_peers(&self) -> bool { self.peer_scores.is_empty() }

    fn update_score(
        &mut self, peer: &PeerNetworkId, update_type: PeerScoreUpdateType,
    ) {
    ...
    // Calculates a weighted index for each peer per network. This is used to
    // probabilistically select a peer (per network) to send a chunk request
    // to.
    ...
    ) -> BTreeMap<NetworkId, (Vec<PeerNetworkId>, Option<WeightedIndex<f64>>)>
    {
        ...
        let peers_by_network_level = self
            ...
            .iter()
            .map(|(peer, peer_score)| {
                (peer.raw_network_id(), (peer, peer_score))
            })
            .into_group_map();
    ...
    /// Picks a set of peers to send chunk requests to. Here, we attempt to pick
    /// one peer per network, in order of network level preference. The set
    /// of networks selected is determined by the multicast network level.
    /// All networks with preference level <= multicast level are sampled.
    /// If there are no live peers in these networks, the multicast level is
    /// updated to the preference level of the first chosen network.
    fn pick_peers(&mut self) -> Vec<PeerNetworkId> {
        // Calculate a weighted peer selection map per network level
        let weighted_peers_per_network =
            self.calculate_weighted_peers_per_network();

        let mut chosen_peers = vec![];
        let mut new_multicast_network_level = None;

        for (network_level, (peers, weighted_index)) in
            &weighted_peers_per_network
        {
            if let Some(peer) = pick_peer(peers, weighted_index) {
                chosen_peers.push(peer)
            }
            // At minimum, go through networks with preference level <=
            // multicast level. If no peers are found for the
            // current multicast level, continue doing best effort
            // search of the networks to failover to.
            if !chosen_peers.is_empty()
                && *network_level >= self.multicast_network_level
            {
    ...
    pub fn send_chunk_request(
        &mut self, req: GetChunkRequest,
    ) -> Result<(), Error> {
        let log = LogSchema::new(LogEntry::SendChunkRequest)
            .chunk_request(req.clone());

        let peers = self.pick_peers();
        if peers.is_empty() {
            diem_warn!(log.event(LogEvent::MissingPeers));
        ...
        for peer in peers {
            ...
            let result_label = if let Err(e) = send_result {
                ...
                diem_error!(curr_log
                    .event(LogEvent::NetworkSendError)
                    .error(&e));
                counters::SEND_FAIL_LABEL
            } else {
                diem_debug!(curr_log.event(LogEvent::Success));
    ...
    pub fn send_chunk_response(
        &mut self, peer: &PeerNetworkId, message: StateSyncMessage,
    ) -> Result<(), Error> {
    ...
    pub fn add_request(
        &mut self, version: u64, peers: Vec<PeerNetworkId>,
    ) -> ChunkRequestInfo {
        if let Some(prev_request) = self.requests.get_mut(&version) {
            let now = SystemTime::now();
            if self.multicast_network_level != prev_request.multicast_level {
                // restart multicast timer for this request if multicast level
                // changed
                prev_request.multicast_level =
    ...
    pub fn process_success_response(&mut self, peer: &PeerNetworkId) {
        ...
        if peer_network_level < self.multicast_network_level {
            // Reduce the multicast network level as we received a chunk
            // response from a peer in a lower (that is, higher
            // priority) network.
    ...
    // Penalize the peer for giving a chunk with a starting version that doesn't
    // match the local synced version.
    pub fn process_chunk_version_mismatch(
        &mut self, peer: &PeerNetworkId, chunk_version: u64,
        synced_version: u64,
    ) -> Result<(), Error>
    {
        if self.is_multicast_response(chunk_version, peer) {
            // If the chunk is a stale multicast response (for a request that
            // another peer sent a response to earlier) don't
            // penalize the peer (no mismatch occurred -- it's just slow).
    ...
    fn is_multicast_response(
        &self, version: u64, peer: &PeerNetworkId,
    ) -> bool {
        self.requests.get(&version).map_or(false, |req| {
            req.last_request_peers.contains(peer)
                && req.last_request_peers.len() > 1
        })
    }
    ...
    /// Removes requests whose known_version < `version` if they are older than
    /// now - `timeout` We keep the requests that have not timed out so we
    /// don't penalize peers who send chunks after the first peer who sends
    /// the first successful chunk response for a multicasted request
    pub fn remove_requests(&mut self, version: u64) {
        // only remove requests that have timed out or sent to one peer, so we
        // don't penalize for multicasted responses that still came back
        // on time, based on per-peer timeout
    ...
    /// Checks whether the request sent with known_version = `version` has timed
    /// out Returns true if such a request timed out (or does not exist),
    /// else false.
    pub fn has_request_timed_out(
        &mut self, version: u64,
    ) -> Result<bool, Error> {
        let last_request_time =
            self.get_last_request_time(version).unwrap_or(UNIX_EPOCH);
        ...
        // Increase the multicast network level if this request has also hit a
        // multicast timeout
        let multicast_start_time =
            self.get_multicast_start_time(version).unwrap_or(UNIX_EPOCH);
        if is_timeout(multicast_start_time, self.multicast_timeout) {
            ...
            self.update_multicast_network_level(
                new_multicast_network_level,
                Some(version),
            );
        }
        Ok(timeout)
    ...
    /// The validator network and vfn network are always considered valid when
    /// state syncing, as multicasting will prioritize sending chunk
    /// requests to the most important network first. If the peer is on the
    /// public network, we only consider it a valid peer to send chunk
    /// requests to if we connected to it.
    fn is_valid_state_sync_peer(
        &self, peer: &PeerNetworkId, origin: ConnectionOrigin,
    ) -> bool {
    ...
    fn update_multicast_network_level(
        &mut self, new_level: NetworkId, request_version: Option<u64>,
    ) {
        ...
        let mut log_event =
            LogSchema::event_log(LogEntry::Multicast, log_event)
...
// Returns whether the timeout for the given params has occurred, compared to
// SystemTime at function call returns true if the timeout (=`timeout_start +
// timeout_duration`) has happened, else false
...
fn pick_peer(
    peers: &[PeerNetworkId], weighted_index: &Option<WeightedIndex<f64>>,
...
// sync (and other places in the code/other components, too). For example, this
// mapping between NetworkId and integer for the purpose of maintaining visible
// counters should be done by NetworkId and not here. Look into updating the
// NetworkId interface to expose this conversion, as well as provide clearer
// interfaces around max, min network values as well as moving between network
// levels.
fn update_multicast_network_counter(multicast_network_level: NetworkId) {
    let network_counter_value =
        if multicast_network_level.is_validator_network() {
            0
...
mod tests {
    ...
    fn test_disable_peer() {
        let (mut request_manager, validators) =
            generate_request_manager_and_validators(0, 1);
        ...
        add_validator_to_request_manager(
            &mut request_manager,
            &validator_0,
            PeerRole::Validator,
        );
    ...
    fn test_score_chunk_success() {
        ...
        for _ in 0..NUM_CHUNKS_TO_PROCESS {
            for validator_index in 0..num_validators {
                request_manager
                    .process_empty_chunk(&validators[validator_index as usize]);
            }
        }
        ...
        verify_validator_picked_most_often(
            &mut request_manager,
            &validators,
            0,
        );
    ...
    fn test_score_chunk_timeout() {
        let (mut request_manager, validators) =
            generate_request_manager_and_validators(0, 4);
        ...
        verify_validator_picked_least_often(
            &mut request_manager,
            &validators,
            0,
        );
    ...
    fn test_score_chunk_version_mismatch_multicast() {
        ...
        for _ in 0..NUM_CHUNKS_TO_PROCESS {
            request_manager.add_request(
                100,
                vec![validator_0.clone(), validator_1.clone()],
            );
            request_manager
                .process_chunk_version_mismatch(&validator_0, 100, 0)
                .unwrap_err();
    ...
    fn test_score_empty_chunk() {
        let (mut request_manager, validators) =
            generate_request_manager_and_validators(10, 4);
    ...
    fn test_score_preferred() {
        ...
        // Create the validators and add them to the request manager. Note that
        // validator at index 0 is the only preferred validator.
    ...
    fn test_remove_requests() {
        let (mut request_manager, validators) =
            generate_request_manager_and_validators(0, 2);
    ...
    /// Verify that the specified validator is chosen most often (due to having
    /// a higher peer score internally).
    fn verify_validator_picked_most_often(
        request_manager: &mut RequestManager, validators: &[PeerNetworkId],
        validator_index: usize,
    )
    {
        verify_validator_pick_frequency(
            request_manager,
            validators,
            validator_index,
            true,
    ...
    /// Verify that the specified validator is chosen least often (due to having
    /// a lower peer score internally).
    ...
    /// Verifies the picking frequency of the specified validator: if
    /// `check_highest_frequency` is true we verify the validator is chosen
    /// most often, otherwise, we verify it is chosen least often.
    fn verify_validator_pick_frequency(
        request_manager: &mut RequestManager, validators: &[PeerNetworkId],
        validator_index: usize, check_highest_frequency: bool,
    )
    {
        // Calculate selection counts for validators
        let pick_counts = calculate_pick_counts_for_validators(
            request_manager,
            NUM_PICKS_TO_MAKE,
        );
        ...
        let validator_count =
            pick_counts.get(&validators[validator_index]).unwrap_or(&0);
        for (index, validator) in validators.iter().enumerate() {
            if validator_index != index {
                if check_highest_frequency {
                    assert!(
                        validator_count > pick_counts.get(&validator).unwrap()
                    );
                } else {
                    assert!(
                        validator_count < pick_counts.get(&validator).unwrap()
    ...
    /// Picks a peer to send a chunk request to (multiple times) and constructs
    /// a pick count for each of the chosen peers.
    fn calculate_pick_counts_for_validators(
        request_manager: &mut RequestManager, number_of_picks_to_execute: u64,
    ...
    /// Generates a new request manager with a specified number of validator
    /// peers enabled.
    fn generate_request_manager_and_validators(
        request_timeout: u64, num_validators: u64,
    ...
    /// Adds the given validator to the specified request manager using the peer
    /// role.
    fn add_validator_to_request_manager(
        request_manager: &mut RequestManager, validator: &PeerNetworkId,
        peer_role: PeerRole,
    )
    {
