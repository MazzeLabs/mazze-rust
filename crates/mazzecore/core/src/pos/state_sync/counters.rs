...
pub enum VersionType {
    ...
    Highest,
    /// Version of most recent txn that was synced (even if it is not backed by
    /// an LI)
    Synced,
    /// Current version a node is trying to catch up to usually within the
    /// current epoch
    Target,
}
...
/// It is usually 0 if the node's primary network is healthy, but can be >0 if
/// the node's primary network is unhealthy/all peers in that network are dead
...
/// How long it takes to make progress, from requesting a chunk to processing
/// the response and committing the block
...
/// Number of failures that occur during the commit flow across consensus, state
/// sync, and mempool
