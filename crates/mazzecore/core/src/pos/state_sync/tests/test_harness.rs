...
use diem_crypto::{
    hash::ACCUMULATOR_PLACEHOLDER_HASH, test_utils::TEST_SEED, x25519,
    HashValue, Uniform,
};
...
use network::{
    peer_manager::{
        builder::AuthenticationMode, conn_notifs_channel,
...
pub static VALIDATOR_NETWORK: Lazy<NetworkId> =
    Lazy::new(|| NetworkId::Validator);
pub static VFN_NETWORK: Lazy<NetworkId> = Lazy::new(NetworkId::vfn_network);
pub static VFN_NETWORK_2: Lazy<NetworkId> =
    Lazy::new(|| NetworkId::Private("Second VFN".into()));
...
pub struct StateSyncPeer {
    ...
    multi_peer_ids: HashMap<NetworkId, PeerId>, /* Holds the peer's PeerIds
                                                 * (to support nodes with
...
impl StateSyncPeer {
    pub fn commit(&self, version: u64) {
        let num_txns =
            version - self.storage_proxy.as_ref().unwrap().write().version();
        assert!(num_txns > 0);
        ...
        // Run StateSyncClient::commit on a tokio runtime to support
        // tokio::timeout in commit().
        assert!(Runtime::new()
            .unwrap()
            .block_on(
                self.client.as_ref().unwrap().commit(committed_txns, vec![])
            )
            .is_ok());
    ...
    // Moves to the next epoch. Note that other peers are not going to be able
    // to discover their new signers: they're going to learn about the new
    // epoch public key through state sync. Private keys are discovered
    // separately.
    pub fn move_to_next_epoch(
        &self, validator_infos: Vec<ValidatorInfo>, validator_index: usize,
    ) {
        let (validator_set, validator_signers) =
            create_new_validator_set(validator_infos);
        self.storage_proxy
            .as_ref()
            .unwrap()
            .write()
            .move_to_next_epoch(
                validator_signers[validator_index].clone(),
                validator_set,
            );
    }
    ...
    pub fn wait_for_version(
        &self, target_version: u64, highest_li_version: Option<u64>,
    ) -> bool {
        let max_retries = 30;
        for _ in 0..max_retries {
            let state =
                block_on(self.client.as_ref().unwrap().get_state()).unwrap();
    ...
    pub fn wait_until_initialized(&self) {
        block_on(self.client.as_ref().unwrap().wait_until_initialized())
            .unwrap();
    }
}
...
impl StateSyncEnvironment {
    ...
    pub fn start_validator_peer(
        &mut self, peer_index: usize, mock_network: bool,
    ) {
    ...
    pub fn start_state_sync_peer(
        &mut self, index: usize, handler: MockRpcHandler, role: RoleType,
        waypoint: Waypoint, mock_network: bool,
    )
    {
    ...
    pub fn setup_state_sync_peer(
        &mut self, index: usize, handler: MockRpcHandler, role: RoleType,
        waypoint: Waypoint, timeout_ms: u64, multicast_timeout_ms: u64,
        mock_network: bool,
    )
    {
        let (config, network_id) =
            setup_state_sync_config(role, timeout_ms, multicast_timeout_ms);
        let network_handles =
            self.setup_network_handles(index, &role, mock_network, network_id);
        ...
        let (mempool_channel, mempool_requests) =
        ...
        peer.client = Some(
            bootstrapper
                .create_client(config.state_sync.client_commit_timeout_ms),
        );
    ...
    fn setup_network_handles(
        &mut self, index: usize, role: &RoleType, mock_network: bool,
    ...
    ) -> Vec<(NodeNetworkId, StateSyncSender, StateSyncEvents)>
    {
        ...
        if mock_network {
            ...
            for (idx, network) in networks.into_iter().enumerate() {
                ...
                // mock the StateSyncEvents and StateSyncSender to allow
                // manually controlling msg delivery in test
                ...
                let (network_notifs_tx, network_notifs_rx) =
                    diem_channel::new(QueueStyle::LIFO, 1, None);
                let (conn_status_tx, conn_status_rx) =
                ...
                let network_events =
                    StateSyncEvents::new(network_notifs_rx, conn_status_rx);
            ...
            let auth_mode =
                AuthenticationMode::Mutual(peer.network_key.clone());
            ...
            let (sender, events) = network_builder.add_protocol_handler(
                state_sync::network::network_endpoint_config(),
            );
    ...
    /// Delivers next message from peer with index `sender` in this
    /// StateSyncEnvironment Returns the recipient of the msg
    pub fn deliver_msg(&mut self, sender_peer_id: PeerId) -> (PeerId, Message) {
        let network_reqs_rx =
            self.network_reqs_rxs.get_mut(&sender_peer_id).unwrap();
        let network_req = block_on(network_reqs_rx.next()).unwrap();

        // await next message from node
        if let PeerManagerRequest::SendDirectSend(receiver_id, msg) =
            network_req
        {
            let receiver_network_notif_tx =
                self.network_notifs_txs.get_mut(&receiver_id).unwrap();
    ...
    // checks that the `env_idx`th peer in this env sends no message to its
    // `network_idx`th network
    ...
    pub fn send_peer_event(
        &mut self, sender_peer_id: PeerId, receiver_peer_id: PeerId,
        new_peer: bool, direction: ConnectionOrigin,
    )
    {
...
fn initial_setup(
    ...
    for (idx, signer) in signers.iter().enumerate() {
        ...
        let addr = addr.append_prod_protos(
        ...
        // The voting power of peer 0 is enough to generate an LI that passes
        // validation.
        ...
        let validator_info =
            ValidatorInfo::new(peer_id, voting_power, validator_config);
...
fn setup_state_sync_config(
    role: RoleType, timeout_ms: u64, multicast_timeout_ms: u64,
...
impl MockStorage {
    pub fn new(
        genesis_li: LedgerInfoWithSignatures, signer: ValidatorSigner,
    ) -> Self {
        let epoch_state =
            genesis_li.ledger_info().next_epoch_state().unwrap().clone();
    ...
    pub fn version(&self) -> u64 { self.transactions.len() as u64 }

    pub fn synced_trees(&self) -> &ExecutedTrees { &self.synced_trees }

    pub fn epoch_num(&self) -> u64 { self.epoch_num }
    ...
    pub fn get_epoch_changes(
        &self, known_epoch: u64,
    ...
    pub fn get_chunk(
        &self, start_version: u64, limit: u64, target_version: u64,
    ) -> Vec<Transaction> {
        let mut res = vec![];
        ...
        while version - 1 < self.transactions.len() as u64
            && version - start_version < limit
        {
    ...
    pub fn add_txns_with_li(
        &mut self, mut transactions: Vec<Transaction>,
    ...
    )
    {
        ...
        if verified_target_li.ledger_info().version() == self.version() {
            ...
            if let Some(next_epoch_state) =
                verified_target_li.ledger_info().next_epoch_state()
            {
    ...
    // with the version corresponding to the new transactions, signed by this
    // storage signer.
    pub fn commit_new_txns(
        &mut self, num_txns: u64,
    ) -> (Vec<Transaction>, Vec<SignedTransaction>) {
    ...
    // This function is applying the LedgerInfo with the next epoch info to the
    // existing version (yes, it's different from reality, we're not adding
    // any real reconfiguration txn, just adding a new LedgerInfo).
    // The validator set is different only in the consensus public / private
    // keys, network data remains the same.
    pub fn move_to_next_epoch(
        &mut self, signer: ValidatorSigner, validator_set: ValidatorSet,
    ) {
...
impl MockExecutorProxy {
    pub fn new(
        handler: MockRpcHandler, storage: Arc<RwLock<MockStorage>>,
    ) -> Self {
...
impl ExecutorProxyTrait for MockExecutorProxy {
    ...
    fn get_chunk(
        ...
        let start_version = known_version.checked_add(1).ok_or_else(|| {
            Error::IntegerOverflow("Known version has overflown!".into())
        })?;
        let txns =
            self.storage
                .read()
    ...
    fn publish_on_chain_config_updates(
        &mut self, _events: Vec<ContractEvent>,
    ) -> Result<(), Error> {
        Ok(())
    }
}
