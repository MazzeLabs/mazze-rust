...
use diem_types::{
    transaction::TransactionListWithProof, waypoint::Waypoint, PeerId,
};
...
fn test_flaky_peer_sync() {
    ...
    let handler =
        Box::new(move |resp| -> Result<TransactionListWithProof, Error> {
...
fn test_request_timeout() {
    ...
    let handler =
        Box::new(move |_| -> Result<TransactionListWithProof, Error> {
            Err(Error::UnexpectedError("Failed to fetch chunk!".into()))
        });
    env.start_state_sync_peer(
        0,
...
fn test_full_node() {
    ...
    validator.commit(20);
    // second sync will be done via long polling cause first node should send
    // new request after receiving first chunk immediately
...
fn catch_up_through_epochs_full_node() {
    ...
    for epoch in 1..10 {
        validator_0.commit(epoch * 100);
        validator_0.move_to_next_epoch(
            vec![validator_0.get_validator_info().clone()],
            0,
        );
    }
...
fn catch_up_with_waypoints() {
    ...
    // Create a waypoint based on LedgerInfo of peer 0 at version 3500 (epoch
    // 14)
    let waypoint_li = validator_0.get_epoch_ending_ledger_info(3500);
    let waypoint =
        Waypoint::new_epoch_boundary(waypoint_li.ledger_info()).unwrap();
    drop(validator_0);
    ...
    // Once caught up with the waypoint fullnode 1 continues with the regular
    // state sync
...
fn test_lagging_upstream_long_poll() {
    ...
    let validator_0_peer_id =
        validator_0.get_peer_id(VALIDATOR_NETWORK.clone());
    ...
    send_connection_notifications(
        &mut env,
        validator_0_peer_id,
        fullnode_1_peer_id_vfn,
        true,
    );

    // Trigger another commit so that the fullnodes's commit will trigger
...
fn test_fullnode_catch_up_moving_target_epochs() {
    ...
    let fullnode_peer_id =
        env.get_state_sync_peer(1).get_peer_id(VFN_NETWORK.clone());
    ...
    // Expected fullnode sync states (i.e., synced version and committed
    // version)
    ...
    // Update the versions at the validator and check the full node syncs
    // correctly.
    for (index, (synced_version, committed_version)) in
        expected_states.iter().enumerate()
    {
...
fn test_fullnode_catch_up_moving_target() {
    ...
    // Update the versions at the validator and check the full node syncs
    // correctly. Every iteration the validator commits another 500
    // transactions (i.e., the validator is committing at twice the rate the
    // full node can sync).
    for (index, (synced_version, committed_version)) in
        expected_states.iter().enumerate()
    {
...
// Unfortunately, this test contains race conditions that are inherent to the
// way the integration tests are built. While flakes don't seem to occur locally
// (or often), we've hit them at least once in CI, so let's ignore this for now
// and return to fix it once this is higher priority.
...
fn test_fn_failover() {
    ...
    // Set up the PFN network: fullnode 0 sends a new peer event to all upstream
    // PFNs
    let upstream_peer_ids =
        [fn_1_pfn_peer_id, fn_2_pfn_peer_id, fn_3_pfn_peer_id];
    for peer in upstream_peer_ids.iter() {
        send_connection_notifications(&mut env, *peer, fn_0_pfn_peer_id, true);
    }

    // Commit transactions on the validator and verify that fullnode 0 sends
    // chunk requests to the validator only.
    ...
    // Bring down the validator connection to fullnode 0 and deliver the last
    // chunk response
    send_connection_notifications(
        &mut env,
        validator_peer_id,
    ...
    // Verify fullnode 0 only broadcasts to the single live fallback peer
    // (fullnode 3)
    let responding_peer_id = execute_commit_and_verify_chunk_requests(
        &mut env,
        3,
    ...
    // Verify that we only broadcast to the single live fallback peer (fullnode
    // 2)
    let responding_peer_id = execute_commit_and_verify_chunk_requests(
        &mut env,
        4,
    ...
    // Check that fullnode 0 sends chunk requests to the validator and fullnode
    // 2
    let responding_peer_id = execute_commit_and_verify_chunk_requests(
        &mut env,
        5,
...
// This test helper is used to execute a commit (at commit_version) on a
// validator node. It verifies that requesting_peers send chunk requests to one
// of the specified target_peers, and that the inactive_peers don't send any
// chunk requests. Note: if skip_all_responses is true, this helper doesn't
// deliver a single chunk response to the requesting_peer. Otherwise, all
// responses are sent, except the chunk from the last responding node. In all
// cases, we return the peer_id of the last node that should have responded to
// the requesting_peer so that it can be used to deliver the last chunk
// manually.
fn execute_commit_and_verify_chunk_requests(
    env: &mut StateSyncEnvironment, commit_version: u64,
    requesting_peers: &[PeerId], target_peers: &[PeerId],
    inactive_peers: &[PeerId], skip_all_responses: bool,
) -> PeerId
{
    ...
    panic!(
        "Unexpected code path in test helper! No chunk responses were sent?"
    );
}
...
fn test_multicast_failover() {
    ...
    // Wait for fullnode 0's chunk request to time out before delivering the
    // response
    std::thread::sleep(std::time::Duration::from_millis(multicast_timeout_ms));
    env.deliver_msg(validator_peer_id);

    // Verify that fullnode 0 broadcasts to the validator peer and both fallback
    // peers
    let _ = execute_commit_and_verify_chunk_requests(
        &mut env,
        2,
    ...
    // Wait for fullnode 0's chunk request to time out before delivering
    // responses
    std::thread::sleep(std::time::Duration::from_millis(multicast_timeout_ms));
    env.deliver_msg(validator_peer_id);
    ...
    // Verify that fullnode 0 still broadcasts to the validator peer and both
    // fallback peers
    let _ = execute_commit_and_verify_chunk_requests(
        &mut env,
        4,
    ...
    // Verify that fullnode 0 still broadcasts to the validator peer and both
    // fallback peers as optimistic fetch hasn't processed the response from
    // the validator
    let _ = execute_commit_and_verify_chunk_requests(
        &mut env,
        6,
...
fn check_chunk_request(
    message: Message, known_version: u64, target_version: Option<u64>,
) {
    let chunk_request: StateSyncMessage =
        bcs::from_bytes(&message.mdata).unwrap();
    match chunk_request {
...
fn check_chunk_response(
    message: Message, response_li_version: u64, chunk_start_version: u64,
    chunk_length: usize,
)
{
    let chunk_response: StateSyncMessage =
        bcs::from_bytes(&message.mdata).unwrap();
    match chunk_response {
        ...
        StateSyncMessage::GetChunkResponse(chunk_response) => {
            assert_eq!(
                chunk_response.response_li.version(),
                response_li_version
            );
            assert_eq!(
                chunk_response
...
// Sends a connection notification to the given peers (i.e.,
// connecting/disconnecting peer_id_0 and peer_id_1). If `new_peer_notification`
// is true, the connection notification is a "new peer" (connect) notification,
// otherwise, a "lost peer" (disconnect) notification is sent.
fn send_connection_notifications(
    env: &mut StateSyncEnvironment, peer_id_0: PeerId, peer_id_1: PeerId,
