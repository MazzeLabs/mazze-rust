...
struct Inner {
    ...
    /// [`Instant`] type is opaque. So long as nothing leaks, we should
...
impl MockTimeService {
    ...
    pub fn num_waiters(&self) -> usize { self.lock().pending.len() }
...
impl TimeServiceTrait for MockTimeService {
    ...
    fn now_unix_time(&self) -> Duration { self.lock().now }
...
impl Inner {
    ...
    fn get_mut_sleep(
        &mut self, deadline: Duration, index: SleepIndex,
    ) -> Option<&mut Option<Waker>> {
        self.pending.get_mut(&(deadline, index))
    }

    fn is_sleep_registered(
        &self, deadline: Duration, index: SleepIndex,
    ) -> bool {
        self.pending.contains_key(&(deadline, index))
    }

    // Register a new waiter that will sleep for `duration`. Return an index
    // that can be used to query, trigger, or unregister this waiter later
    // on.
    fn register_sleep(
        &mut self, duration: Duration, maybe_waker: Option<Waker>,
    ) -> (Duration, SleepIndex) {
    ...
    fn unregister_min_sleep(
        &mut self,
    ) -> Option<((Duration, SleepIndex), Option<Waker>)> {
...
impl MockSleep {
    fn new(time_service: MockTimeService, duration: Duration) -> Self {
        let (deadline, index) =
            time_service.lock().register_sleep(duration, None);
...
impl SleepTrait for MockSleep {
    ...
    fn reset(self: Pin<&mut Self>, duration: Duration) {
        ...
        let maybe_waker =
            inner.unregister_sleep(this.deadline, this.index).flatten();
    ...
    fn reset_until(self: Pin<&mut Self>, deadline: Instant) {
        let this = self.get_mut();
        let duration =
            deadline.saturating_duration_since(this.time_service.now());
...
impl Future for MockSleep {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match maybe_entry {
            Some(maybe_waker) => {
                // We're still waiting. Update our `Waker` so we can get
                // notified.
...
impl Drop for MockSleep {
    fn drop(&mut self) {
        // Be sure to unregister us from the pending queue if we get
        // dropped/canceled.
        self.time_service
            .lock()
...
mod test {
    ...
    use tokio_test::{
        assert_pending, assert_ready, assert_ready_eq, assert_ready_err,
        assert_ready_ok, task,
    };
    ...
    async fn test_auto_advance() {
        ...
        // While we're under the limit, sleeps should resolve immediately and
        // time should advance.
    ...
    fn test_auto_advance_blocking() {
        ...
        // Sleep for a long time so we'll timeout unless auto advance is
        // working.
    ...
    fn ms(duration: u64) -> Duration { Duration::from_millis(duration) }

    fn secs(duration: u64) -> Duration { Duration::from_secs(duration) }
}
