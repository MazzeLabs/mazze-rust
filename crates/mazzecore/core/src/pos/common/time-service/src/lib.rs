...
impl TimeService {
    ...
    ///
    /// See [`RealTimeService`].
    pub fn real() -> Self { RealTimeService::new().into() }
    ...
    /// See [`MockTimeService`].
    #[cfg(any(test, feature = "testing"))]
    pub fn mock() -> Self { MockTimeService::new().into() }
    ...
    pub fn into_mock(self) -> MockTimeService {
        match self {
            ...
            ts => panic!(
                "Unexpected TimeService, expected MockTimeService: {:?}",
                ts
            ),
        }
    }
...
pub trait TimeServiceTrait: Send + Sync + Clone + Debug {
    ...
    fn now_secs(&self) -> u64 { self.now_unix_time().as_secs() }
    ...
    fn timeout_at<F: Future>(
        &self, deadline: Instant, future: F,
    ) -> Timeout<F> {
...
impl From<RealSleep> for Sleep {
    fn from(sleep: RealSleep) -> Self { Sleep::RealSleep(sleep) }
}

#[cfg(any(test, feature = "fuzzing", feature = "testing"))]
impl From<MockSleep> for Sleep {
    fn from(sleep: MockSleep) -> Self { Sleep::MockSleep(sleep) }
}
...
impl SleepTrait for Sleep {
    ...
    fn reset(self: Pin<&mut Self>, duration: Duration) {
        match self.project() {
            SleepProject::RealSleep(inner) => {
                SleepTrait::reset(inner, duration)
            }
            #[cfg(any(test, feature = "fuzzing", feature = "testing"))]
            SleepProject::MockSleep(inner) => {
                SleepTrait::reset(Pin::new(inner), duration)
            }
        }
    }
    ...
    fn reset_until(self: Pin<&mut Self>, deadline: Instant) {
        match self.project() {
            SleepProject::RealSleep(inner) => {
                SleepTrait::reset_until(inner, deadline)
            }
            #[cfg(any(test, feature = "fuzzing", feature = "testing"))]
            SleepProject::MockSleep(inner) => {
                SleepTrait::reset_until(Pin::new(inner), deadline)
            }
        }
    }
}
