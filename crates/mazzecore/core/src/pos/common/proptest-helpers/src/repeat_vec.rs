...
impl<T> RepeatVec<T> {
    ...
    pub fn remove_all(
        &mut self, logical_indexes: impl IntoIterator<Item = usize>,
    ) {
    ...
    fn remove_all_impl(&mut self, logical_indexes: Vec<usize>) {
        // # Notes
        //
        // * This looks pretty long and complicated, mostly because the logic is
        //   complex enough to require manual loops and iteration.
        // * This is unavoidably linear time in the number of physical elements.
        //   One way to make the constant factors smaller would be to implement
        //   a ChunkedRepeatVec<T>, which can be done by wrapping a
        //   RepeatVec<RepeatVec<T>> plus some glue logic. The idea would be
        //   similar to skip-lists. 2 levels should be enough, but 3 or more
        //   would work as well.
        ...
        let first_idx = match self.binary_search(first) {
            ...
            Err(start_idx) => {
                // This is the physical index after the logical item. Start
                // reasoning from the previous index to match
                // the Ok branch above.
                start_idx - 1
            }
        };

        // This serves two purposes -- it represents the number of elements to
        // decrease by and the current position in indexes.
        let mut decrease = 0;
        ...
        let new_items = {
            ...
            while let Some((current_logical_idx_old, current_elem)) =
                items.next()
            {
                let current_logical_idx_new =
                    current_logical_idx_old - decrease;

                let next_logical_idx_old =
                    items.peek().map_or(self.len, |&(idx, _)| idx);

                // Remove all indexes until the next logical index or
                // sorted_indexes runs out.
    ...
    pub fn get(&self, at: usize) -> Option<(&T, usize)> {
        ...
        match self.binary_search(at) {
            ...
            Err(start_idx) => {
                // start_idx can never be 0 because usize starts from 0 and
                // items[0].0 is always 0. So start_idx is
                // always at least 1.
    ...
    /// randomness.
    pub fn pick_uniform_indexes(
        &self, indexes: &[impl AsRef<Index>],
    ) -> Vec<usize> {
