...
impl Encryptor {
    ...
    /// This generates an Encryptor for use in default / testing scenarios where
    /// (proper) encryption is not necessary.
    pub fn for_testing() -> Self {
        let storage = Storage::InMemoryStorage(
            diem_secure_storage::InMemoryStorage::new(),
        );
    ...
    pub fn add_key(
        &mut self, version: KeyVersion, key: Key,
    ) -> Result<(), Error> {
    ...
    pub fn decrypt(
        &self, encrypted_network_addresses: &[u8], account: AccountAddress,
    ) -> Result<Vec<NetworkAddress>, Error> {
        let keys = self.read()?;
        let enc_addrs: Vec<EncNetworkAddress> =
            bcs::from_bytes(&encrypted_network_addresses).map_err(|e| {
                Error::AddressDeserialization(account, e.to_string())
            })?;
        let mut addrs = Vec::new();
        for (idx, enc_addr) in enc_addrs.iter().enumerate() {
            let key =
                keys.keys.get(&enc_addr.key_version()).ok_or_else(|| {
                    Error::VersionNotFound(enc_addr.key_version())
                })?;
            let addr = enc_addr
                .clone()
                .decrypt(&key.0, &account, idx as u32)
                .map_err(|e| {
                Error::DecryptionError(account, e.to_string())
            })?;
            addrs.push(addr);
        }
        Ok(addrs)
...
where D: serde::Deserializer<'de> {
    ...
    base64::decode(s)
        ...
        .and_then(|v| {
            std::convert::TryInto::try_into(v.as_slice())
                .map_err(serde::de::Error::custom)
        })
}
...
mod tests {
    ...
    fn e2e() {
        ...
        let addr =
            std::str::FromStr::from_str("/ip4/10.0.0.16/tcp/80").unwrap();
        let addrs = vec![addr];
    ...
    fn initializer() {
        let storage =
            Storage::VaultStorage(diem_secure_storage::VaultStorage::new(
