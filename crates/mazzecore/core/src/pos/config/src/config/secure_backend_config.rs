...
use diem_secure_storage::{
    GitHubStorage, InMemoryStorage, NamespacedStorage, OnDiskStorage, Storage,
    VaultStorage,
};
...
pub struct VaultConfig {
    ...
    /// have your lease renewed up to that many seconds more. If this is
    /// not specified, the lease will not automatically be renewed.
...
vault:
    ...
    token:
        ...
        // Just assert that it can be serialized, not about to do string
        // comparison
...
vault:
    ...
    response_timeout_ms: 5000
        ...
        let de_from_config: Config =
            serde_yaml::from_str(text_from_config).unwrap();
        assert_eq!(de_from_config, from_config);
        // Just assert that it can be serialized, no need to do string
        // comparison
        serde_yaml::to_string(&from_config).unwrap();
    }
