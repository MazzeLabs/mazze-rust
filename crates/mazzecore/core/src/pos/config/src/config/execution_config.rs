    ...
    pub fn load(&mut self, root_dir: &RootPath) -> Result<(), Error> {
        if !self.genesis_file_location.as_os_str().is_empty() {
            ...
            file.read_to_end(&mut buffer)
                .map_err(|e| Error::IO("genesis".into(), e))?;
            let data = bcs::from_bytes(&buffer)
...
pub enum ExecutionCorrectnessService {
    ...
    Serializer,
    /// This creates a separate thread to run execution correctness, it is
    /// similar to a fork / exec style
    Thread,
}
