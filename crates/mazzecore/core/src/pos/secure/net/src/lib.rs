...
mod test {
    ...
    fn test_server_shutdown() {
        ...
        let data = vec![4, 5, 6, 7];
        // We aren't notified immediately that a server has shutdown, but it
        // happens eventually
    ...
    fn test_server_timeout() {
        ...
        // New client, success, note the previous client connection is still
        // active, the server is actively letting it go due to lack of
        // activity.
    ...
    fn test_client_timeout() {
        ...
        // Clean up old Server listener but keep the stream online. Start a new
        // server, which will be the one the client now connects to.
