...
static DIEM_VAULT: Lazy<Option<VaultRunner>> =
    Lazy::new(|| match VaultRunner::run() {
        Err(err) => {
            assert!(
...
pub fn test_host() -> String { test_host_safe().unwrap() }
...
impl VaultRunner {
    ...
    fn _port() -> Result<u16, String> {
        let listener =
            TcpListener::bind(("localhost", 0)).map_err(|e| e.to_string())?;
    ...
    pub fn root_token(&self) -> &str { ROOT_TOKEN }
