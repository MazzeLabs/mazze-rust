...
use crate::{
    Capability, CryptoStorage, Error, GetResponse, Identity, KVStorage, Policy,
    PublicKeyResponse,
};
use chrono::DateTime;
...
/// vault service leveraged herein is called \[KV (Key Value) Secrets Engine - Version 2\](`<https://www.vaultproject.io/api/secret/kv/kv-v2.html>`). So while Diem Secure Storage
...
impl VaultStorage {
    pub fn new(
        host: String, token: String, namespace: Option<String>,
    ...
    pub fn set_policy(
        ...
        let mut vault_policy =
            self.client().read_policy(&policy_name).unwrap_or_default();
        ...
        for capability in capabilities {
            match capability {
                ...
                Capability::Read => {
                    core_capabilities.push(vault::Capability::Read)
                }
                Capability::Rotate => {
                ...
                Capability::Write => {
                    core_capabilities.push(vault::Capability::Update)
                }
            }
        }

        let path =
            format!("{}/{}", engine.to_policy_path(), self.name(key, engine));
    ...
    pub fn set_policies(
        &self, name: &str, engine: &VaultEngine, policy: &Policy,
    ) -> Result<(), Error> {
        for perm in &policy.permissions {
            match &perm.id {
                Identity::User(id) => {
                    self.set_policy(id, engine, name, &perm.capabilities)?
                }
                Identity::Anyone => self.set_policy(
                    DIEM_DEFAULT,
                    engine,
                    name,
                    &perm.capabilities,
...
impl KVStorage for VaultStorage {
    ...
    fn get<T: DeserializeOwned>(
        ...
        let last_update = DateTime::parse_from_rfc3339(&resp.creation_time)?
            .timestamp() as u64;
    ...
    fn set<T: Serialize>(&mut self, key: &str, value: T) -> Result<(), Error> {
        ...
        let new_version = self.client().write_secret(
            &secret,
            key,
...
impl CryptoStorage for VaultStorage {
    ...
    fn get_public_key(&self, name: &str) -> Result<PublicKeyResponse, Error> {
        ...
        Ok(PublicKeyResponse {
            last_update: DateTime::parse_from_rfc3339(&last_key.creation_time)?
                .timestamp() as u64,
    ...
    fn get_public_key_previous_version(
        ...
        match highest_version {
            Some(version) => {
                let pubkey =
                    pubkeys.iter().find(|pubkey| pubkey.version == version - 1);
                Ok(pubkey
                    .ok_or_else(|| {
                        Error::KeyVersionNotFound(
                            name,
            ...
            None => {
                Err(Error::KeyVersionNotFound(name, "previous version".into()))
            }
        }
    }
