...
fn test_get_public_key_previous_version(storage: &mut Storage) {
    ...
    for _ in 0..num_rotations {
        ...
        // Verify the previous public key can be used to fetch the corresponding
        // public key
        let private_key_previous_version = storage
            .export_private_key_for_version(
                CRYPTO_NAME,
                public_key_previous_version,
            )
            .unwrap();
...
fn test_import_key(storage: &mut Storage) {
    ...
    let rotated_imported_key =
        storage.export_private_key(imported_key_name).unwrap();
    let rotated_imported_public_key = storage
        .get_public_key(imported_key_name)
        .unwrap()
        .public_key;

    let rotated_message_signature =
        storage.sign(imported_key_name, &message).unwrap();
...
fn test_create_sign_rotate_sign(storage: &mut Storage) {
    ...
    // Rotate the key pair and sign the message again using the previous key
    // pair version
