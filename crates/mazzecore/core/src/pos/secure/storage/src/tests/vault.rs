...
use crate::{
    ...
    Capability, CryptoStorage, Error, Identity, KVStorage, Permission, Policy,
    Storage,
};
...
/// A test for verifying VaultStorage properly implements the DiemSecureStorage
/// API and enforces strict separation between unique namespaces. This test
/// depends on running Vault, which can be done by using the provided docker run
/// script in `docker/vault/run.sh`
#[test]
...
fn test_suite_multiple_namespaces() {
    let mut storage_1 = Storage::from(create_vault_with_namespace(Some(
        VAULT_NAMESPACE_1.into(),
    )));
    let mut storage_2 = Storage::from(create_vault_with_namespace(Some(
        VAULT_NAMESPACE_2.into(),
    )));
    let mut storage_3 = Storage::from(create_vault_with_namespace(Some(
        VAULT_NAMESPACE_3.into(),
    )));
...
fn test_vault_key_value_policies() {
    ...
    let mut writer =
        create_vault_storage(writer_token.clone(), None, ttl, false);
    ...
    let mut reader =
        create_vault_storage(reader_token.clone(), None, ttl, false);
    ...
    // Verify a reader with another namespace has no permission for the
    // operations
    let mut reader_with_namespace = create_vault_storage(
        reader_token,
    ...
    // Verify a writer with another namespace has no permission for the
    // operations
    let mut writer_with_namespace = create_vault_storage(
        writer_token,
        Some(VAULT_NAMESPACE_1.into()),
        ttl,
        false,
...
fn test_vault_crypto_policies() {
    ...
    let mut exporter_store =
        create_vault_storage(exporter_token.clone(), None, None, true);
    ...
    // Verify a exporter with another namespace has no permission for the
    // operations
    let exporter_store_with_namespace = create_vault_storage(
        exporter_token,
        Some(VAULT_NAMESPACE_1.into()),
        None,
        true,
    ...
    let mut noone_store = create_vault_storage(noone_token, None, None, true);
    ...
    let mut reader_store =
        create_vault_storage(reader_token.clone(), None, None, true);
    ...
    // Verify a reader with another namespace has no permission for the
    // operations
    let reader_store_with_namespace = create_vault_storage(
        reader_token,
        Some(VAULT_NAMESPACE_1.into()),
        None,
        true,
    ...
    let mut rotater_store =
        create_vault_storage(rotater_token.clone(), None, None, true);
    ...
    // Verify a rotater with another namespace has no permission for the
    // operations
    let mut rotater_store_with_namespace = create_vault_storage(
        rotater_token,
        Some(VAULT_NAMESPACE_1.into()),
        None,
        true,
    ...
    let mut signer_store =
        create_vault_storage(signer_token.clone(), None, None, true);
    ...
    // Verify a signer with another namespace has no permission for the
    // operations
    let signer_store_with_namespace = create_vault_storage(
        signer_token,
        Some(VAULT_NAMESPACE_1.into()),
        None,
        true,
...
fn test_vault_tokens() {
    let mut storage =
        create_vault_with_namespace(Some(VAULT_NAMESPACE_1.into()));
    ...
    let writer_token = storage.create_token(vec![&WRITER]).unwrap();
    let mut writer = create_vault_storage(
        writer_token.clone(),
        Some(VAULT_NAMESPACE_1.into()),
        None,
        true,
    ...
    let mut writer_without_namespace =
        create_vault_storage(writer_token, None, None, true);
    assert_eq!(
    ...
    let writer = create_vault_storage(
        "INVALID TOKEN".into(),
        Some(VAULT_NAMESPACE_1.into()),
        None,
        true,
...
fn test_vault_cas() {
    ...
    let mut without_cas =
        create_vault_storage(ROOT_TOKEN.into(), None, None, false);
...
fn test_vault_key_trimming() {
    ...
    for i in 2u32..4 {
        ...
        let key_versions = storage.get_all_key_versions(CRYPTO_KEY).unwrap();
        let max_version =
            key_versions.iter().map(|resp| resp.version).max().unwrap();
        let min_version =
            key_versions.iter().map(|resp| resp.version).min().unwrap();
