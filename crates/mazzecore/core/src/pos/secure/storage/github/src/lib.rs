...
impl Client {
    ...
    pub fn delete_file(&self, path: &str) -> Result<(), Error> {
        // Occasionally GitHub sends us back delayed results and the file is
        // already deleted.
    ...
    pub fn get_branches(&self) -> Result<Vec<String>, Error> {
        let url = format!(
            "{}/repos/{}/{}/branches",
            URL, self.owner, self.repository
        );
    ...
    pub fn get_file(&self, path: &str) -> Result<String, Error> {
        ...
        if value.len() == 1 && value[0].path == path {
            let content = value[0].content.as_ref().ok_or_else(|| {
                Error::InternalError("No content found".into())
            })?;
            // Apparently GitHub introduces newlines every 60 characters and at
            // the end of content, this strips those characters out.
    ...
    fn upgrade_request(&self, mut request: ureq::Request) -> ureq::Request {
        ...
        if let Some(proxy_url) = proxy_url {
            request.set_proxy(
                ureq::Proxy::new(proxy_url).expect("Unable to parse proxy_url"),
            );
        }
        request
    }
...
mod tests {
    ...
    fn test_directories() {
        ...
        // Verify the contents
        let b64_value =
            base64::decode(github.get_file(path1).unwrap()).unwrap();
        let value = std::str::from_utf8(&b64_value).unwrap();
        assert_eq!(value, value1);

        let b64_value =
            base64::decode(github.get_file(path2).unwrap()).unwrap();
        let value = std::str::from_utf8(&b64_value).unwrap();
        assert_eq!(value, value2);
    ...
    fn test_recursive_delete_file() {
        ...
        let b64_value =
            base64::decode(github.get_file(file0).unwrap()).unwrap();
        let rv = std::str::from_utf8(&b64_value).unwrap();
        assert_eq!(value, rv);

        let b64_value =
            base64::decode(github.get_file(file1).unwrap()).unwrap();
        let rv = std::str::from_utf8(&b64_value).unwrap();
        assert_eq!(value, rv);
