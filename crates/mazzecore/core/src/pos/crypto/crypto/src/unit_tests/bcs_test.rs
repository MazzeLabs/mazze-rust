...
fn ed25519_bcs_material() {
    ...
    let private_key =
        Ed25519PrivateKey::try_from([1u8; ED25519_PRIVATE_KEY_LENGTH].as_ref())
            .unwrap();
    let public_key = Ed25519PublicKey::from(&private_key);

    let serialized_public_key =
        bcs::to_bytes(&Cow::Borrowed(&public_key)).unwrap();
    // Expected size should be 1 byte due to BCS length prefix + 32 bytes for
    // the raw key bytes
    assert_eq!(serialized_public_key.len(), 1 + ED25519_PUBLIC_KEY_LENGTH);

    // Ensure public key serialization - deserialization is stable and
    // deterministic
    ...
    let serialized_signature =
        bcs::to_bytes(&Cow::Borrowed(&signature)).unwrap();
    // Expected size should be 1 byte due to BCS length prefix + 64 bytes for
    // the raw signature bytes
    assert_eq!(serialized_signature.len(), 1 + ED25519_SIGNATURE_LENGTH);

    // Ensure signature serialization - deserialization is stable and
    // deterministic
    let deserialized_signature: Ed25519Signature =
        bcs::from_bytes(&serialized_signature).unwrap();
...
fn multi_ed25519_bcs_material() {
    ...
    let multi_private_key_7of10 =
        MultiEd25519PrivateKey::new(private_keys_10, threshold).unwrap();
    ...
    let multi_signature_7of10: MultiEd25519Signature =
        multi_private_key_7of10.sign(&message);

    let serialized_multi_signature =
        bcs::to_bytes(&Cow::Borrowed(&multi_signature_7of10)).unwrap();
    // Expected size due to specialization is
    // 2 bytes for BCS length prefix (due to ULEB128)
