...
fn message() -> &'static TestDiemCrypto { &MESSAGE }
...
fn test_successful_public_key_serialization(
    original_keys: &[Ed25519PublicKey], threshold: u8,
) {
...
fn test_successful_signature_serialization(
    private_keys: &[Ed25519PrivateKey], threshold: u8,
) {
    let multi_private_key =
        MultiEd25519PrivateKey::new(private_keys.to_vec(), threshold).unwrap();
    ...
    let multi_signature_serialized_unwrapped =
        multi_signature_serialized.unwrap();
...
fn test_multi_ed25519_public_key_serialization() {
    let pub_keys_1: Vec<_> =
        generate_keys(1).iter().map(|x| x.public_key()).collect();
    let pub_keys_10: Vec<_> =
        generate_keys(10).iter().map(|x| x.public_key()).collect();
    let pub_keys_32: Vec<_> =
        generate_keys(32).iter().map(|x| x.public_key()).collect();
    let pub_keys_33: Vec<_> =
        generate_keys(33).iter().map(|x| x.public_key()).collect();
    ...
    test_failed_public_key_serialization(
        multi_key_empty_bytes,
        WrongLengthError,
    );
    ...
    let multi_key_31_bytes = MultiEd25519PublicKey::try_from(
        &[0u8; ED25519_PUBLIC_KEY_LENGTH - 1][..],
    );
    test_failed_public_key_serialization(multi_key_31_bytes, WrongLengthError);

    // Test try_from 32 bytes (should fail) because we always need
    // ED25519_PUBLIC_KEY_LENGTH * N + 1 bytes (thus 32N + 1).
    let multi_key_32_bytes =
        MultiEd25519PublicKey::try_from(&[0u8; ED25519_PUBLIC_KEY_LENGTH][..]);
    test_failed_public_key_serialization(multi_key_32_bytes, WrongLengthError);

    // Test try_from 34 bytes (should fail).
    let multi_key_34_bytes = MultiEd25519PublicKey::try_from(
    ...
    // validation issues).
    let multi_key_33_zero_bytes = MultiEd25519PublicKey::try_from(
        &[0u8; ED25519_PUBLIC_KEY_LENGTH + 1][..],
    );
    test_failed_public_key_serialization(
        multi_key_33_zero_bytes,
        ValidationError,
    );

    let priv_keys_10 = generate_keys(10);
    let pub_keys_10: Vec<_> =
        priv_keys_10.iter().map(|x| x.public_key()).collect();

    let multi_private_key_7of10 =
        MultiEd25519PrivateKey::new(priv_keys_10, 7).unwrap();
    let multi_public_key_7of10 =
        MultiEd25519PublicKey::new(pub_keys_10, 7).unwrap();

    // Check that MultiEd25519PublicKey::from MultiEd25519PrivateKey works as
    // expected.
    ...
    // Check that MultiEd25519PublicKey::from Ed25519PublicKey works as
    // expected.
...
fn test_publickey_smallorder() {
    ...
    let torsion_key =
        MultiEd25519PublicKey::try_from(&torsion_point_with_threshold_1[..]);
...
fn test_multi_ed25519_signature_serialization() {
    ...
    let multi_priv_key_1of3 =
        MultiEd25519PrivateKey::new(priv_keys_3.to_vec(), 1).unwrap();
    ...
    // Fail to construct a MultiEd25519Signature object from 33 or more single
    // signatures.
    ...
    // Fail to construct a MultiEd25519Signature object if there are duplicated
    // indexes.
    ...
    // Fail to construct a MultiEd25519Signature object if an index is out of
    // range.
...
fn test_multi_ed25519_signature_verification() {
    ...
    let multi_private_key_7of10 =
        MultiEd25519PrivateKey::new(priv_keys_10.clone(), 7).unwrap();
    let multi_public_key_7of10 =
        MultiEd25519PublicKey::from(&multi_private_key_7of10);

    // Verifying a 7-of-10 signature against a public key with the same
    ...
    // Verifying a 7-of-10 signature against a public key with bigger threshold
    // (i.e., 8) should fail.
    let multi_public_key_8of10 =
        MultiEd25519PublicKey::new(pub_keys_10.clone(), 8).unwrap();
    assert!(multi_signature_7of10
        .verify(message(), &multi_public_key_8of10)
        .is_err());

    // Verifying a 7-of-10 signature against a public key with smaller threshold
    // (i.e., 6) should pass.
    let multi_public_key_6of10 =
        MultiEd25519PublicKey::new(pub_keys_10.clone(), 6).unwrap();
    assert!(multi_signature_7of10
        .verify(message(), &multi_public_key_6of10)
        .is_ok());

    // Verifying a 7-of-10 signature against a reordered MultiEd25519PublicKey
    // should fail. To deterministically simulate reshuffling, we use a
    // reversed vector of 10 keys. Note that because 10 is an even number,
    // all of they keys will change position.
    ...
    let multi_private_key_1of3 =
        MultiEd25519PrivateKey::new(priv_keys_3.clone(), 1).unwrap();
    let multi_public_key_1of3 =
        MultiEd25519PublicKey::from(&multi_private_key_1of3);

    // Signing with the 2nd key must succeed.
    let sig_with_2nd_key = priv_keys_3[1].sign(message());
    let multi_sig_signed_by_2nd_key =
        MultiEd25519Signature::new(vec![(sig_with_2nd_key, 1)]);
    assert!(multi_sig_signed_by_2nd_key.is_ok());
    let multi_sig_signed_by_2nd_key_unwrapped =
        multi_sig_signed_by_2nd_key.unwrap();
    assert_eq!(
    ...
    let failed_multi_sig_signed_by_2nd_key_wrong_index =
        multi_sig_signed_by_2nd_key_wrong_index
            .unwrap()
            .verify(message(), &multi_public_key_1of3);
    assert!(failed_multi_sig_signed_by_2nd_key_wrong_index.is_err());

    // Signing with the 2nd and 3rd keys must succeed, even if we surpass the
    // threshold.
    ...
    // Signing with the 2nd and an unrelated key. Although threshold is met, it
    // should fail as we don't accept invalid signatures.
    ...
    let multi_private_key_2of3 =
        MultiEd25519PrivateKey::new(priv_keys_3.clone(), 2).unwrap();
    let multi_public_key_2of3 =
        MultiEd25519PublicKey::from(&multi_private_key_2of3);
    ...
    let signed_by_1st_and_2nd_key_unwrapped =
        signed_by_1st_and_2nd_key.unwrap();
    assert_eq!(
    ...
    let signed_by_1st_and_3rd_key_unwrapped =
        signed_by_1st_and_3rd_key.unwrap();
    assert_eq!(
    ...
    let signed_by_2nd_and_3rd_key_unwrapped =
        signed_by_2nd_and_3rd_key.unwrap();
    assert_eq!(
