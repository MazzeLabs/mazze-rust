...
impl LedgerInfo {
    ...
    pub fn main_decision(&self) -> Option<&MainBlockDecision> {
        self.commit_info.main_decision()
    }
...
use crate::{
    block_info::MainBlockDecision, validator_config::ConsensusSignature,
};
#[cfg(any(test, feature = "fuzzing"))]
...
mod tests {
    ...
    fn test_signatures_hash() {
        ...
        // Add the signatures in reverse order and ensure the serialization
        // matches
        let mut author_to_signature_map = BTreeMap::new();
