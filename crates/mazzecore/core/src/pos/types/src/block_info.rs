...
pub struct BlockInfo {
    ...
    /// The last main block selection after executing this block.
    /// None means choosing DETS genesis as the first main block.
    main: Option<MainBlockDecision>,
}
...
impl Display for BlockInfo {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        write!(
            f,
            "BlockInfo: [epoch: {}, round: {}, id: {}, executed_state_id: {}, version: {}, timestamp (us): {}, next_epoch_state: {}, main: {:?}]",
            self.epoch(),
            self.round(),
            self.id(),
...
pub struct MainBlockDecision {
    pub height: u64,
    pub block_hash: H256,
}

impl MainBlockDecision {
    pub fn main_select_event_key() -> EventKey {
        EventKey::new_from_address(
            &account_config::main_chain_select_address(),
            2,
        )
    }

    pub fn main_select_access_path() -> AccessPath {
        AccessPath::new(
            account_config::main_chain_select_address(),
            Self::resource_path(),
        )
...
#[cfg(any(test, feature = "fuzzing"))]
impl proptest::arbitrary::Arbitrary for MainBlockDecision {
    type Parameters = ();
    type Strategy = BoxedStrategy<Self>;

    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        any::<u8>()
            .prop_map(|_seed| MainBlockDecision {
                height: 0,
...
impl MoveResource for MainBlockDecision {
    const MODULE_NAME: &'static str = "main_decision";
    const STRUCT_NAME: &'static str = "main_decision";
}
