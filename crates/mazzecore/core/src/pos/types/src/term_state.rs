...
use crate::{
    ...
    account_config,
    block_info::{MainBlockDecision, Round, View},
...
pub struct PosState {
    ...
    main_decision: MainBlockDecision,
...
impl PosState {
    pub fn new(
        ...
        genesis_main_decision: MainBlockDecision,
    ) -> Self {
        ...
        let mut pos_state = PosState {
            ...
            main_decision: genesis_main_decision,
    ...
    pub fn set_main_decision(&mut self, main_decision: MainBlockDecision) {
        self.main_decision = main_decision;
    }

    pub fn main_decision(&self) -> &MainBlockDecision { &self.main_decision }
...
impl PosState {
    ...
    pub fn validate_main_decision_simple(
        &self, main_decision_tx: &MainBlockDecision,
    ) -> Option<DiscardedVMStatus> {
        if main_decision_tx.height <= self.main_decision.height {
            return Some(DiscardedVMStatus::MAIN_DECISION_HEIGHT_TOO_OLD);
        }
        None
    }
}
    ...
    pub fn validate_main_decision(
        ...
        if main_decision_tx.height <= self.main_decision.height {
            return Err(anyhow!(format!(
                "Main Decision height too small, found[{}], expect[{}]",
                main_decision_tx.height, self.main_decision.height
            )));
        }
        let senders = self
            .epoch_state
        ...
        if let Err(e) = signature.verify(main_decision_tx, &public_key) {
            return Err(anyhow!(format!(
                "Main Decision verification failed [{:?}]",
                e
            )));
        }
...
impl PosState {
    ...
    pub fn new_node_elected(&mut self, event: &ElectionEvent) -> Result<()> {
        ...
        if voting_power > 0 {
            // A workaround for too much staked MAZZE in Testnet.
        ...
        let epoch_state = if self.current_view == 1 {
            ...
            if view_in_term == 0 {
                ...
                self.term_list.new_term(
                    new_term,
                    self.main_decision.block_hash.as_bytes().to_vec(),
                );
