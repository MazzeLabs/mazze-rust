...
use super::{
    ...
    verify_transaction_info, MerkleTreeInternalNode, SparseMerkleInternalNode,
...
use diem_crypto::{
    hash::{
        ...
        TransactionAccumulatorHasher, SPARSE_MERKLE_PLACEHOLDER_HASH,
    },
    HashValue,
};
#[cfg(any(test, feature = "fuzzing"))]
...
where H: CryptoHasher
{
    ...
    pub fn siblings(&self) -> &[HashValue] { &self.siblings }
    ...
    pub fn verify(
        ...
        let actual_root_hash = self
            ...
            .fold(
                ...
                // `index` denotes the index of the ancestor of the element at
                // the current level.
...
impl<H> PartialEq for AccumulatorProof<H> {
    fn eq(&self, other: &Self) -> bool { self.siblings == other.siblings }
}

impl<H> Eq for AccumulatorProof<H> {}

pub type TransactionAccumulatorProof =
...
where V: CryptoHash
{
    ...
    pub fn new(
        leaf: Option<SparseMerkleLeafNode>, siblings: Vec<HashValue>,
    ) -> Self {
        SparseMerkleProof {
            leaf,
            siblings,
    ...
    pub fn leaf(&self) -> Option<SparseMerkleLeafNode> { self.leaf }

    /// Returns the list of siblings in this proof.
    pub fn siblings(&self) -> &[HashValue] { &self.siblings }

    /// If `element_value` is present, verifies an element whose key is
    /// `element_key` and value is `element_value` exists in the Sparse
    /// Merkle Tree using the provided proof. Otherwise verifies the proof
    /// is a valid non-inclusion proof that shows this key doesn't exist in the
    /// tree.
    pub fn verify(
        ...
        match (element_value, self.leaf) {
            (Some(value), Some(leaf)) => {
                // This is an inclusion proof, so the key and value hash
                // provided in the proof should match
                // element_key and element_value_hash. `siblings` should prove
                // the route from the leaf node to the root.
                ensure!(
            ...
            (None, Some(leaf)) => {
                ...
                // inserted, it will break a currently existing leaf
                // node represented by `proof_key` into a branch. `siblings`
                // should prove the route from that leaf node to
                // the root.
                ensure!(
                    element_key != leaf.key,
...
impl AccumulatorConsistencyProof {
    ...
    pub fn new(subtrees: Vec<HashValue>) -> Self { Self { subtrees } }

    /// Returns the subtrees.
    pub fn subtrees(&self) -> &[HashValue] { &self.subtrees }
}
...
where H: CryptoHasher
{
    ...
    /// `right_siblings`.
    pub fn new(
        left_siblings: Vec<HashValue>, right_siblings: Vec<HashValue>,
    ) -> Self {
        Self {
            left_siblings,
            right_siblings,
    ...
    pub fn new_empty() -> Self { Self::new(vec![], vec![]) }

    /// Get all the left siblngs.
    pub fn left_siblings(&self) -> &Vec<HashValue> { &self.left_siblings }

    /// Get all the right siblngs.
    pub fn right_siblings(&self) -> &Vec<HashValue> { &self.right_siblings }
    ...
    pub fn verify(
        ...
        // Keep reducing the list of hashes by combining all the children pairs,
            ...
            || right_sibling_iter.peek().is_some()
        {
            ...
            // If the first position on the current level is a right child, it
            // needs to be combined with a sibling on the left.
            if first_pos.is_right_child() {
                ...
                let right_hash =
                    *children_iter.next().expect("The first leaf must exist.");
                parent_hashes.push(
                    MerkleTreeInternalNode::<H>::new(left_hash, right_hash)
                        .hash(),
                );
            }
            ...
            // Similarly, if the last position is a left child, it needs to be
            // combined with a sibling on the right.
...
impl<H> PartialEq for AccumulatorRangeProof<H> {
    fn eq(&self, other: &Self) -> bool {
        self.left_siblings == other.left_siblings
...
pub type TransactionAccumulatorRangeProof =
    AccumulatorRangeProof<TransactionAccumulatorHasher>;
#[cfg(any(test, feature = "fuzzing"))]
...
impl TransactionInfoWithProof {
    ...
    pub fn verify(
        &self, ledger_info: &LedgerInfo, transaction_version: Version,
    ) -> Result<()> {
...
impl TransactionListProof {
    /// Constructs a new `TransactionListProof` using
    /// `ledger_info_to_transaction_info_proof` and `transaction_infos`.
    pub fn new(
        ledger_info_to_transaction_infos_proof: TransactionAccumulatorRangeProof,
        transaction_infos: Vec<TransactionInfo>,
    ) -> Self {
    ...
    pub fn unpack(
        self,
    ) -> (TransactionAccumulatorRangeProof, Vec<TransactionInfo>) {
        (
    ...
    /// Verifies the list of transactions are correct using the proof. The
