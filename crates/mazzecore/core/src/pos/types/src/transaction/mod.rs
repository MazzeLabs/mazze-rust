...
impl RawTransaction {
    ...
    pub fn new_main_decision(
        sender: AccountAddress, main_decision: MainBlockDecision,
        chain_id: ChainId,
    ) -> Self {
        RawTransaction {
            sender,
            payload: TransactionPayload::MainDecision(main_decision),
            // Write-set transactions are special and important and shouldn't
            // expire.
            expiration_timestamp_secs: u64::max_value(),
            chain_id,
    ...
    pub fn sign(
        ...
        let signature = match self.payload {
            TransactionPayload::MainDecision(ref main_decision) => {
                private_key.sign(main_decision)
            }
...
pub enum TransactionPayload {
    ...
    MainDecision(MainBlockDecision),
...
impl SignedTransaction {
    ...
    pub fn check_signature(self) -> Result<SignatureCheckedTransaction> {
        match self.payload() {
            TransactionPayload::MainDecision(main_decision) => {
                self.authenticator.verify(main_decision)?
            }
