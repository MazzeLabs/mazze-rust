...
where I: IntoIterator<Item = &'a MoveValue> {
    vals.into_iter()
        .map(|val| {
...
impl MoveStruct {
    pub fn new(value: Vec<MoveValue>) -> Self { MoveStruct(value) }

    pub fn simple_deserialize(
        blob: &[u8], ty: &MoveStructLayout,
    ) -> AResult<Self> {
        Ok(bcs::from_bytes_seed(ty, blob)?)
    }

    pub fn fields(&self) -> &[MoveValue] { &self.0 }

    pub fn into_inner(self) -> Vec<MoveValue> { self.0 }
}

impl MoveStructLayout {
    pub fn new(types: Vec<MoveTypeLayout>) -> Self { MoveStructLayout(types) }

    pub fn fields(&self) -> &[MoveTypeLayout] { &self.0 }
}
...
impl<'d> serde::de::DeserializeSeed<'d> for &MoveTypeLayout {
    ...
    fn deserialize<D: serde::de::Deserializer<'d>>(
        ...
        match self {
            MoveTypeLayout::Bool => {
                bool::deserialize(deserializer).map(MoveValue::Bool)
            }
            MoveTypeLayout::U8 => {
                u8::deserialize(deserializer).map(MoveValue::U8)
            }
            MoveTypeLayout::U64 => {
            ...
            MoveTypeLayout::Struct(ty) => {
                Ok(MoveValue::Struct(ty.deserialize(deserializer)?))
            }
...
impl<'d> serde::de::DeserializeSeed<'d> for &MoveStructLayout {
    ...
    fn deserialize<D: serde::de::Deserializer<'d>>(
        ...
        let fields = deserializer
            .deserialize_tuple(layout.len(), StructFieldVisitor(layout))?;
