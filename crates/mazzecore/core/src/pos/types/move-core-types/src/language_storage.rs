...
#[derive(
    Serialize, Deserialize, Debug, PartialEq, Hash, Eq, Clone, PartialOrd, Ord,
)]
pub enum TypeTag {
    Bool,
    U8,
...
/// Represents the intitial key into global storage where we first index by the
...
impl ResourceKey {
    pub fn address(&self) -> AccountAddress { self.address }

    pub fn type_(&self) -> &StructTag { &self.type_ }
}
...
impl From<ModuleId> for (AccountAddress, Identifier) {
    fn from(module_id: ModuleId) -> Self { (module_id.address, module_id.name) }
}
...
impl ModuleId {
    ...
    pub fn name(&self) -> &IdentStr { &self.name }
