...
impl AccountAddress {
    ...
    pub const fn new(address: [u8; Self::LENGTH]) -> Self { Self(address) }
    ...
    pub fn from_hex_literal(
        literal: &str,
    ) -> Result<Self, AccountAddressParseError> {
    ...
    pub fn to_hex(&self) -> String { format!("{:x}", self) }
...
impl From<[u8; AccountAddress::LENGTH]> for AccountAddress {
    fn from(bytes: [u8; AccountAddress::LENGTH]) -> Self { Self::new(bytes) }
}
...
impl From<AccountAddress> for Vec<u8> {
    fn from(addr: AccountAddress) -> Vec<u8> { addr.0.to_vec() }
}
...
impl From<AccountAddress> for [u8; AccountAddress::LENGTH] {
    fn from(addr: AccountAddress) -> Self { addr.0 }
}
...
impl From<&AccountAddress> for String {
    fn from(addr: &AccountAddress) -> String { ::hex::encode(addr.as_ref()) }
}
...
mod tests {
    ...
    fn test_address() {
        let hex =
            "ca843279e3427144cead5e4d5999a3d0ca843279e3427144cead5e4d5999a3d0";
        let bytes =
            Vec::from_hex(hex).expect("You must provide a valid Hex format");
    ...
    fn test_from_hex_literal() {
        ...
        let address_from_literal =
            AccountAddress::from_hex_literal(hex_literal).unwrap();
        let address = AccountAddress::from_hex(hex).unwrap();
    ...
    fn test_deserialize_from_json_value() {
        ...
        let json_value =
            serde_json::to_value(address).expect("serde_json::to_value fail.");
        let address2: AccountAddress = serde_json::from_value(json_value)
            .expect("serde_json::from_value fail.");
    ...
    fn test_serde_json() {
        ...
        let json_address: AccountAddress =
            serde_json::from_str(json_hex).unwrap();
