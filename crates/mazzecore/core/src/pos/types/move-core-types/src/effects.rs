...
fn publish_checked<K, V, F>(
    map: &mut BTreeMap<K, Option<V>>, k: K, v: V, make_err: F,
) -> Result<()>
where
    K: Ord,
    F: FnOnce() -> Error,
...
fn unpublish_checked<K, V, F>(
    map: &mut BTreeMap<K, Option<V>>, k: K, make_err: F,
) -> Result<()>
where
    K: Ord,
    F: FnOnce() -> Error,
...
impl AccountChangeSet {
    ...
    pub fn publish_or_overwrite_module(
        &mut self, name: Identifier, blob: Vec<u8>,
    ) {
        self.modules.insert(name, Some(blob));
    }

    pub fn publish_or_overwrite_resource(
        &mut self, struct_tag: StructTag, blob: Vec<u8>,
    ) {
...
impl ChangeSet {
    ...
    fn get_or_insert_account_changeset(
        &mut self, addr: AccountAddress,
    ) -> &mut AccountChangeSet {
        match self.accounts.entry(addr) {
            btree_map::Entry::Occupied(entry) => entry.into_mut(),
            btree_map::Entry::Vacant(entry) => {
                entry.insert(AccountChangeSet::new())
            }
        }
    }
    ...
    pub fn publish_or_overwrite_resource(
        &mut self, addr: AccountAddress, struct_tag: StructTag, blob: Vec<u8>,
    ) {
    ...
    pub fn unpublish_resource(
        &mut self, addr: AccountAddress, struct_tag: StructTag,
    ) -> Result<()> {
        self.get_or_insert_account_changeset(addr)
    ...
    pub fn into_modules(
        self,
    ) -> impl Iterator<Item = (ModuleId, Option<Vec<u8>>)> {
        self.accounts.into_iter().flat_map(|(addr, account)| {
            account
                .modules
                .into_iter()
                .map(move |(module_name, blob_opt)| {
                    (ModuleId::new(addr, module_name), blob_opt)
                })
        })
    }
