...
impl Identifier {
    ...
    pub fn as_ident_str(&self) -> &IdentStr { self }
    ...
    pub fn into_string(self) -> String { self.0.into() }
...
impl<'a> From<&'a IdentStr> for Identifier {
    fn from(ident_str: &'a IdentStr) -> Self { ident_str.to_owned() }
}

impl AsRef<IdentStr> for Identifier {
    fn as_ref(&self) -> &IdentStr { self }
}
...
impl Deref for Identifier {
    ...
    fn deref(&self) -> &IdentStr {
        // Identifier and IdentStr maintain the same invariants, so it is safe
        // to convert.
...
impl Borrow<IdentStr> for Identifier {
    fn borrow(&self) -> &IdentStr { self }
}

impl ToOwned for IdentStr {
    type Owned = Identifier;

    fn to_owned(&self) -> Identifier { Identifier(self.0.into()) }
}
...
impl Arbitrary for Identifier {
    ...
    fn arbitrary_with((): ()) -> Self::Strategy {
        ALLOWED_NO_SELF_IDENTIFIERS
            .prop_map(|s| {
                // Identifier::new will verify that generated identifiers are
                // correct.
