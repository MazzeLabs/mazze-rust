...
pub enum VMStatus {
    ...
    /// Indicates an error from the VM, e.g. OUT_OF_GAS, INVALID_AUTH_KEY,
    /// RET_TYPE_MISMATCH_ERROR etc.
    /// The code will neither EXECUTED nor ABORTED
    Error(StatusCode),

    /// Indicates an `abort` from inside Move code. Contains the location of
    /// the abort and the code
    MoveAbort(AbortLocation, /* code */ u64),

    /// Indicates an failure from inside Move code, where the VM could not
    /// continue exection, e.g. dividing by zero or a missing resource
    ExecutionFailure {
...
/// An `AbortLocation` specifies where a Move program `abort` occurred, either
/// in a function in a module, or in a script
#[derive(
    Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize,
)]
#[cfg_attr(any(test, feature = "fuzzing"), derive(Arbitrary))]
...
impl VMStatus {
    ...
    /// Returns the Move abort code if the status is `MoveAbort`, and `None`
    /// otherwise
    pub fn move_abort_code(&self) -> Option<u64> {
        match self {
            Self::MoveAbort(_, code) => Some(*code),
            Self::Error(_) | Self::ExecutionFailure { .. } | Self::Executed => {
                None
            }
        }
    }

    /// Return the status type for this `VMStatus`. This is solely determined by
    /// the `status_code`
    pub fn status_type(&self) -> StatusType { self.status_code().status_type() }

    /// Returns `Ok` with a recorded status if it should be kept, `Err` of the
    /// error code if it should be discarded
    pub fn keep_or_discard(self) -> Result<KeptVMStatus, DiscardedVMStatus> {
        match self {
            VMStatus::Executed => Ok(KeptVMStatus::Executed),
            VMStatus::MoveAbort(location, code) => {
                Ok(KeptVMStatus::MoveAbort(location, code))
            }
            VMStatus::ExecutionFailure {
                status_code: StatusCode::OUT_OF_GAS,
                ..
            }
            | VMStatus::Error(StatusCode::OUT_OF_GAS) => {
            ...
            VMStatus::Error(code) => {
                match code.status_type() {
                    ...
                    // Any error that is a validation status (i.e. an error
                    // arising from the prologue) causes the
                    // transaction to not be included.
                    StatusType::Validation => Err(code),
                    // If the VM encountered an invalid internal state, we
                    // should discard the transaction.
                    StatusType::InvariantViolation => Err(code),
                    // A transaction that publishes code that cannot be verified
                    // will be charged.
                    StatusType::Verification => {
                        Ok(KeptVMStatus::MiscellaneousError)
                    }
                    // If we are able to decode the`SignedTransaction`, but
                    // failed to decode `SingedTransaction.
                    // raw_transaction.payload` (i.e., the transaction script),
                    // there should be a charge made to that user's account for
                    // the gas fees related to decoding,
                    // running the prologue etc.
                    StatusType::Deserialization => {
                        Ok(KeptVMStatus::MiscellaneousError)
                    }
                    // Any error encountered during the execution of the
                    // transaction will charge gas.
...
impl fmt::Display for VMStatus {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let mut status =
            format!("status {:#?} of type {}", self.status_code(), status_type);
...
pub mod known_locations {
    ...
    pub static DIEM_MODULE: Lazy<ModuleId> = Lazy::new(|| {
        ModuleId::new(CORE_CODE_ADDRESS, DIEM_MODULE_IDENTIFIER.clone())
    });
...
pub enum StatusCode {
    ...
    // The transaction is not expected for Mazze PoS chain
    MAZZE_UNEXPECTED_TX = 27,
    ...
    MAIN_DECISION_HEIGHT_TOO_OLD=33,
...
fn test_status_codes() {
    ...
    // Make sure that within the 0-EXECUTION_STATUS_MAX_CODE that all of the
    // status codes succeed when they should, and fail when they should.
