...
/// The underlying carrier for gas-related units and costs. Data with this type
/// should not be manipulated directly, but instead be manipulated using the
/// newtype wrappers defined around them and the functions defined in the
/// `GasAlgebra` trait.
pub type GasCarrier = u64;

/// A trait encoding the operations permitted on the underlying carrier for the
/// gas unit, and how other gas-related units can interact with other units --
/// operations can only be performed across units with the same underlying
/// carrier (i.e. as long as the underlying data is the same).
pub trait GasAlgebra<GasCarrier>: Sized
where GasCarrier: Add<Output = GasCarrier>
        ...
        + Copy
{
    ...
    /// Map a function `f` of two arguments over the underlying carrier. Note
    /// that this function can take two different implementations of the
    /// trait -- one for `self` the other for the second argument. But, we
    /// enforce that they have the same underlying carrier.
    fn map2<F: Fn(GasCarrier, GasCarrier) -> GasCarrier>(
        self, other: impl GasAlgebra<GasCarrier>, f: F,
    ) -> Self {
        Self::new(f(self.get(), other.get()))
    }

    /// Apply a function `f` of two arguments to the carrier. Since `f` is not
    /// an endomorphism, we return the resulting value, as opposed to the
    /// result wrapped up in ourselves.
    ...
    /// We allow casting between GasAlgebras as long as they have the same
    /// underlying carrier -- i.e. they use the same type to store the
    /// underlying value.
    fn unitary_cast<T: GasAlgebra<GasCarrier>>(self) -> T { T::new(self.get()) }
...
// We would really like to be able to implement the standard arithmetic traits
// over the GasAlgebra trait, but that isn't possible.
...
pub const REFERENCE_SIZE: AbstractMemorySize<GasCarrier> =
    AbstractMemorySize(8);
...
pub const DEFAULT_ACCOUNT_SIZE: AbstractMemorySize<GasCarrier> =
    AbstractMemorySize(800);

/// Any transaction over this size will be charged `INTRINSIC_GAS_PER_BYTE` per
/// byte
pub const LARGE_TRANSACTION_CUTOFF: AbstractMemorySize<GasCarrier> =
    AbstractMemorySize(600);

/// For exists checks on data that doesn't exists this is the multiplier that is
/// used.
pub const MIN_EXISTS_DATA_SIZE: AbstractMemorySize<GasCarrier> =
    AbstractMemorySize(100);
...
pub struct GasConstants {
    ...
    /// Any transaction over this size will be charged an additional amount per
    /// byte.
    pub large_transaction_cutoff: AbstractMemorySize<GasCarrier>,

    /// The units of gas that to be charged per byte over the
    /// `large_transaction_cutoff` in addition to
    /// `min_transaction_gas_units` for transactions whose size exceeds
...
impl GasConstants {
    pub fn to_internal_units(
        &self, units: GasUnits<GasCarrier>,
    ) -> InternalGasUnits<GasCarrier> {
        InternalGasUnits::new(units.get() * self.gas_unit_scaling_factor)
    }

    pub fn to_external_units(
        &self, units: InternalGasUnits<GasCarrier>,
    ) -> GasUnits<GasCarrier> {
...
impl CostTable {
    ...
    pub fn instruction_cost(&self, instr_index: u8) -> &GasCost {
        precondition!(
            instr_index > 0
                && instr_index <= (self.instruction_table.len() as u8)
        );
