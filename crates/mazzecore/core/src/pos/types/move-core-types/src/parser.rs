...
impl Token {
    fn is_whitespace(&self) -> bool { matches!(self, Self::Whitespace(_)) }
}
...
fn next_number(
    initial: char, mut it: impl Iterator<Item = char>,
) -> Result<(Token, usize)> {
...
impl<I: Iterator<Item = Token>> Parser<I> {
    ...
    fn peek(&mut self) -> Option<&Token> { self.it.peek() }
    ...
    fn parse_comma_list<F, R>(
        &mut self, parse_list_item: F, end_token: Token,
...
where F: Fn(&mut Parser<std::vec::IntoIter<Token>>) -> Result<T> {
...
fn tests_parse_transaction_argument_positive() {
    ...
    for (s, expected) in &[
        ...
        (
            "0x54afa3526",
            T::Address(
                AccountAddress::from_hex_literal("0x54afa3526").unwrap(),
            ),
        ),
        (
            "0X54afa3526",
