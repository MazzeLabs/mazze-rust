...
impl TestTransaction {
    pub(crate) fn new(
        address: usize, sequence_number: u64, gas_price: u64,
    ) -> Self {
        Self {
            address,
            sequence_number,
...
pub(crate) fn add_txns_to_mempool(
    pool: &mut CoreMempool, txns: Vec<TestTransaction>,
...
pub(crate) fn add_txn(
    pool: &mut CoreMempool, transaction: TestTransaction,
) -> Result<()> {
    add_signed_txn(pool, transaction.make_signed_transaction())
}

pub(crate) fn add_signed_txn(
    pool: &mut CoreMempool, transaction: SignedTransaction,
) -> Result<()> {
    match pool
        .add_txn(
            transaction.clone(),
...
pub(crate) fn batch_add_signed_txn(
    pool: &mut CoreMempool, transactions: Vec<SignedTransaction>,
) -> Result<()> {
...
// Helper struct that keeps state between `.get_block` calls. Imitates work of
// Consensus.
pub struct ConsensusMock(HashSet<TxnPointer>);

impl ConsensusMock {
    pub(crate) fn new() -> Self { Self(HashSet::new()) }

    pub(crate) fn get_block(
        &mut self, mempool: &mut CoreMempool, block_size: u64,
    ) -> Vec<SignedTransaction> {
...
pub(crate) fn exist_in_metrics_cache(
    mempool: &CoreMempool, txn: &SignedTransaction,
) -> bool {
    mempool
        .metrics_cache
