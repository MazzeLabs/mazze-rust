...
use crate::{
    ...
    shared_mempool::{
        network::MempoolNetworkSender, start_shared_mempool,
...
use diem_types::{
    account_address::AccountAddress, transaction::GovernanceRole, PeerId,
};
...
/// TODO: This really only supports 2 networks, but there are currently nodes
/// with 3
#[enum_dispatch]
...
impl NodeInfoTrait for ValidatorNodeInfo {
    fn primary_peer_id(&self) -> PeerId { self.primary_peer_id }

    fn primary_network(&self) -> NetworkId { NetworkId::Validator }

    fn role(&self) -> RoleType { RoleType::Validator }

    fn peer_role(&self) -> PeerRole { PeerRole::Validator }

    fn secondary_peer_id(&self) -> Option<PeerId> { None }

    fn secondary_network(&self) -> Option<NetworkId> { None }
}
...
impl NodeInfoTrait for ValidatorFullNodeInfo {
    ...
    fn primary_network(&self) -> NetworkId { NetworkId::vfn_network() }

    fn role(&self) -> RoleType { RoleType::FullNode }

    fn peer_role(&self) -> PeerRole { PeerRole::ValidatorFullNode }

    fn secondary_peer_id(&self) -> Option<PeerId> {
        Some(self.secondary_peer_id)
    }

    fn secondary_network(&self) -> Option<NetworkId> { Some(NetworkId::Public) }
}
...
impl NodeInfoTrait for FullNodeInfo {
    fn primary_peer_id(&self) -> PeerId { self.peer_id }

    fn primary_network(&self) -> NetworkId { NetworkId::Public }

    fn role(&self) -> RoleType { RoleType::FullNode }

    fn peer_role(&self) -> PeerRole { self.peer_role }
...
pub fn validator_config(
    rng: &mut StdRng, account_idx: u32,
) -> (ValidatorNodeInfo, NodeConfig) {
...
pub fn public_full_node_config(
    rng: &mut StdRng, account_idx: u32, peer_role: PeerRole,
...
impl NodeInfoTrait for Node {
    ...
    fn primary_network(&self) -> NetworkId { self.node_info.primary_network() }

    fn role(&self) -> RoleType { self.node_info.role() }

    fn peer_role(&self) -> PeerRole { self.node_info.peer_role() }
...
impl Node {
    ...
    pub fn new(node: NodeInfo, config: NodeConfig) -> Node {
        let (network_interfaces, network_handles) =
            setup_node_network_interfaces(&node);
        let (mempool, runtime, subscriber) =
            start_node_mempool(config, network_handles);
    ...
    pub fn mempool(&self) -> MutexGuard<CoreMempool> { self.mempool.lock() }
    ...
    pub fn add_txns(&self, txns: Vec<TestTransaction>) {
        ...
        for txn in txns {
            let transaction =
                txn.make_signed_transaction_with_max_gas_amount(5);
            mempool.add_txn(
                transaction.clone(),
                0,
    ...
    pub fn send_new_peer_event(
        &mut self, is_primary: bool, new_peer: PeerId, peer_role: PeerRole,
        origin: ConnectionOrigin,
    )
    {
        let metadata = ConnectionMetadata::mock_with_role_and_origin(
            new_peer, peer_role, origin,
        );
    ...
    pub fn send_lost_peer_event(
        &mut self, is_primary: bool, lost_peer: PeerId,
    ) {
    ...
    fn send_connection_event(
        &mut self, is_primary: bool, notif: ConnectionNotification,
    ) {
    ...
    fn get_network_interface(
        &mut self, is_primary: bool,
    ) -> &mut NodeNetworkInterface {
    ...
    pub fn send_network_req(
        &mut self, is_primary: bool, protocol: ProtocolId,
...
impl NodeNetworkInterface {
    fn get_next_network_req(
        &mut self, runtime: Arc<Runtime>,
    ) -> PeerManagerRequest {
        runtime.block_on(self.network_reqs_rx.next()).unwrap()
    }

    fn send_network_req(
        &mut self, protocol: ProtocolId, message: PeerManagerNotification,
    ) {
    ...
    fn send_connection_notif(&mut self, notif: ConnectionNotification) {
        let peer_id = match &notif {
            ...
            ConnectionNotification::LostPeer(metadata, _, _) => {
                metadata.remote_peer_id
            }
        };
...
fn setup_node_network_interfaces(
    ...
    let (network_interface, network_handle) =
        setup_node_network_interface(PeerNetworkId(
...
fn setup_node_network_interface(
    ...
    let (network_notifs_tx, network_notifs_rx) =
        diem_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
    let (network_conn_event_notifs_tx, conn_status_rx) =
    ...
    let network_events =
        MempoolNetworkEvents::new(network_notifs_rx, conn_status_rx);
...
fn start_node_mempool(
    config: NodeConfig, network_handles: Vec<MempoolNetworkHandle>,
) -> (
