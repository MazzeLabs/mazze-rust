...
use crate::{
    ...
    tests::common::{
        add_signed_txn, add_txn, add_txns_to_mempool, exist_in_metrics_cache,
        setup_mempool, TestTransaction,
    },
};
...
fn test_ordering_of_governance_transactions() {
    ...
    for role1 in &gov_roles {
        for role2 in &gov_roles {
            ...
            // If the first role is "less than" the second then we should swap
            // them since the second txn should have higher
            // priority.
...
fn test_update_transaction_in_mempool() {
    ...
    let fixed_txns =
        add_txns_to_mempool(&mut mempool, vec![TestTransaction::new(0, 0, 5)]);
...
fn test_ignore_same_transaction_submitted_to_mempool() {
    let (mut mempool, _) = setup_mempool();
    let _ =
        add_txns_to_mempool(&mut mempool, vec![TestTransaction::new(0, 0, 0)]);
...
fn test_update_invalid_transaction_in_mempool() {
    ...
    let updated_txn =
        TestTransaction::make_signed_transaction_with_max_gas_amount(
            &TestTransaction::new(0, 0, 5),
            200,
        );
    let _added_tnx = add_signed_txn(&mut mempool, updated_txn);

    // Since both gas price and mas gas amount were updated, the ordering should
    // not have changed. The second transaction with gas price 2 should come
    // first.
    assert_eq!(consensus.get_block(&mut mempool, 1), vec![txns[1].clone()]);
...
fn test_commit_callback() {
    ...
    // Insert transaction with sequence number 6 to pool (while last known
    // executed transaction is 0).
    let txns =
...
fn test_sequence_number_cache() {
    ...
    // Try to add transaction with sequence number 6 to pool (while last known
    // executed transaction for AC is 0).
    add_txns_to_mempool(&mut pool, vec![TestTransaction::new(1, 6, 1)]);
    // Verify that we can execute transaction 6.
...
fn test_parking_lot_eviction() {
    ...
    // Make sure we can't insert any new transactions, cause parking lot
...
fn test_parking_lot_evict_only_for_ready_txn_insertion() {
    ...
    // Trying to insert a tx that would not be ready after inserting should
    // fail.
...
fn test_gc_ready_transaction() {
    ...
    // Make sure txns 2 and 3 became not ready and we can't read them from any
    // API.
...
fn test_clean_stuck_transactions() {
    ...
    let txn = TestTransaction::new(0, db_sequence_number, 1)
