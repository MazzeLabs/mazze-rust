...
pub struct AccountTransactions {
    ...
    main_decision_transaction: HashMap<HashValue, MempoolTransaction>,
}
...
impl AccountTransactions {
    pub(crate) fn new() -> Self {
        Self {
            ...
            main_decision_transaction: HashMap::new(),
        }
    }
    ...
    pub(crate) fn get(&self, hash: &HashValue) -> Option<&MempoolTransaction> {
        if let Some(txn) = self.normal_transaction.get(hash) {
            ...
            self.main_decision_transaction.get(hash)
        }
    }
    ...
    pub(crate) fn remove(
        ...
        if let Some(txn) = self.normal_transaction.remove(hash) {
            ...
            self.main_decision_transaction.remove(hash)
        }
    }

    pub(crate) fn insert(
        &mut self, hash: HashValue, txn: MempoolTransaction,
        is_main_decision: bool,
    ) {
        if is_main_decision {
            self.main_decision_transaction.insert(hash, txn);
        } else {
    ...
    pub(crate) fn iter_main_decision(&self) -> AccountTransactionIter {
        self.main_decision_transaction.values()
    }
}
