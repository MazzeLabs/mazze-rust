...
pub struct TransactionStore {
    ...
    // main decision helper structure
    main_decisions: HashMap<HashValue, HashSet<(AccountAddress, HashValue)>>,
...
impl TransactionStore {
    ...
    /// Fetch main decisions by main hash.
    pub(crate) fn get_main_decisions(
        &self, hash: &HashValue,
    ) -> Vec<HashValue> {
        if let Some(decisions) = self.main_decisions.get(hash) {
            decisions
                .iter()
    ...
    pub(crate) fn insert(
        ...
        if let TransactionPayload::MainDecision(main_decision) = payload {
            let main_decision_hash = main_decision.hash();
            self.main_decisions
                .entry(main_decision_hash)
                .or_insert_with(HashSet::new);
            if let Some(account_decision) =
                self.main_decisions.get_mut(&main_decision_hash)
            {
                diem_debug!("txpool::insert main {:?}", hash);
    ...
    pub(crate) fn commit_transaction(
        ...
        if let Some(transaction) = self.transactions.remove(&hash) {
            ...
            if let TransactionPayload::MainDecision(main_decision) = payload {
                let main_decision_hash = main_decision.hash();
                if let Some(indices) =
                    self.main_decisions.remove(&main_decision_hash)
                {
    ...
    pub(crate) fn reject_transaction(
        ...
        for txn in self.transactions.iter_main_decision() {
            if txn.get_sender() == *account {
                txns_log.add(txn.get_sender(), txn.get_hash());
                hashes.push(txn.get_hash());
            }
    ...
    fn gc(
        ...
        while let Some(key) = gc_iter.next() {
            if let Some(txn) = self.transactions.remove(&key.hash) {
                ...
                self.index_remove(&txn);
                if let TransactionPayload::MainDecision(main_decision) =
                    txn.txn.into_raw_transaction().into_payload()
                {
                    self.main_decisions.remove(&main_decision.hash());
                }
            }
        }
    ...
    pub(crate) fn iter_main_decision(&self) -> MainDecisionIter {
        self.main_decisions.values()
    }
}
