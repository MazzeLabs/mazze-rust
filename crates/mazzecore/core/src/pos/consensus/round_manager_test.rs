...
use super::{
    ...
    test_utils::{
        consensus_runtime, timed_block_on, MockStateComputer, MockStorage,
        MockTransactionManager, TreeInserter,
    },
...
impl NodeSetup {
    fn create_round_state(time_service: Arc<dyn TimeService>) -> RoundState {
        ...
        let time_interval =
            Box::new(ExponentialTimeInterval::fixed(base_timeout));
    ...
    fn create_nodes(
        playground: &mut NetworkPlayground, executor: Handle, num_nodes: usize,
    ) -> Vec<Self> {
        ...
        for (id, signer) in signers.iter().take(num_nodes).enumerate() {
            let (initial_data, storage) =
                MockStorage::start_for_testing((&validators).into());
    ...
    fn new(
        playground: &mut NetworkPlayground, executor: Handle,
        signer: ValidatorSigner, proposer_author: Author,
        storage: Arc<MockStorage>, initial_data: RecoveryData,
        safety_rules_manager: SafetyRulesManager, id: usize,
    ) -> Self
    {
        ...
        let network =
            NetworkSender::new(author, network_sender, self_sender, validators);

        let all_events = Box::new(select(network_events, self_receiver));

        let last_vote_sent = initial_data.last_vote();
        let (commit_cb_sender, commit_cb_receiver) =
            mpsc::unbounded::<LedgerInfoWithSignatures>();
    ...
    pub fn restart(
        self, playground: &mut NetworkPlayground, executor: Handle,
    ) -> Self {
...
fn new_round_on_quorum_cert() {
    ...
    let mut nodes =
        NodeSetup::create_nodes(&mut playground, runtime.handle().clone(), 1);
...
fn vote_on_successful_proposal() {
    let mut runtime = consensus_runtime();
    let mut playground = NetworkPlayground::new(runtime.handle().clone());
    // In order to observe the votes we're going to check proposal processing on
    // the non-proposer node (which will send the votes to the proposer).
    let mut nodes =
        NodeSetup::create_nodes(&mut playground, runtime.handle().clone(), 1);
    let node = &mut nodes[0];
    ...
    timed_block_on(&mut runtime, async {
        ...
        let proposal =
            Block::new_proposal(vec![], 1, 1, genesis_qc.clone(), &node.signer);
...
fn no_vote_on_mismatch_round() {
    ...
    let block_skip_round =
        Block::new_proposal(vec![], 2, 2, genesis_qc.clone(), &node.signer);
    timed_block_on(&mut runtime, async {
...
fn no_vote_on_invalid_proposer() {
    ...
    // the non-proposer node (which will send the votes to the proposer).
    let mut nodes =
        NodeSetup::create_nodes(&mut playground, runtime.handle().clone(), 2);
...
fn recover_on_restart() {
    ...
    let mut node =
        NodeSetup::create_nodes(&mut playground, runtime.handle().clone(), 1)
            .pop()
            .unwrap();
    let inserter = TreeInserter::new_with_store(
        node.signer.clone(),
...
fn sync_info_sent_on_stale_sync_info() {
    ...
    let genesis_qc = certificate_for_genesis();
    let block_0 =
        Block::new_proposal(vec![], 1, 1, genesis_qc, &nodes[0].signer);
    let parent_block_info = block_0.quorum_cert().certified_block();
    let block_0_quorum_cert = gen_test_certificate(
...
fn sync_on_partial_newer_sync_info() {
    ...
    timed_block_on(&mut runtime, async {
        ...
        let sync_info =
            SyncInfo::new(block_4_qc.clone(), certificate_for_genesis(), None);
        node.round_manager
        ...
        // Due to the asynchronous channel, the order of the sync info and next
        // proposal is not fixed. We assert one of them is the sync info
        // we expect.
        let m1 = node.next_message().await;
        let m2 = node.next_message().await;
        assert!(
            matches!(m1, ConsensusMsg::SyncInfo(_))
                || matches!(m2, ConsensusMsg::SyncInfo(_))
        );
    });
}
