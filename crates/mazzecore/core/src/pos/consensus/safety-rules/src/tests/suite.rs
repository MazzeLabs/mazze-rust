...
fn test_bad_execution_output(safety_rules: &Callback) {
    ...
    let a1 = test_utils::make_proposal_with_qc(
        round + 1,
        genesis_qc,
        &signer,
        key.as_ref(),
    ...
    let a2 =
        make_proposal_with_parent(round + 2, &a1, None, &signer, key.as_ref());
    let a3 =
        make_proposal_with_parent(round + 3, &a2, None, &signer, key.as_ref());
...
fn test_commit_rule_consecutive_rounds(safety_rules: &Callback) {
    ...
    let b1 = test_utils::make_proposal_with_qc(
        round + 2,
        genesis_qc,
        &signer,
        key.as_ref(),
    ...
    let b2 =
        make_proposal_with_parent(round + 3, &b1, None, &signer, key.as_ref());
    let a2 =
        make_proposal_with_parent(round + 4, &a1, None, &signer, key.as_ref());
    let a3 =
        make_proposal_with_parent(round + 5, &a2, None, &signer, key.as_ref());
...
fn test_end_to_end(safety_rules: &Callback) {
    ...
    let p0 = test_utils::make_proposal_with_qc(
        round + 1,
        genesis_qc.clone(),
        &signer,
        key.as_ref(),
...
fn test_preferred_block_rule(safety_rules: &Callback) {
    ...
    let b2 =
        make_proposal_with_parent(round + 3, &a1, None, &signer, key.as_ref());
    let a2 =
        make_proposal_with_parent(round + 4, &b1, None, &signer, key.as_ref());
    let b3 =
        make_proposal_with_parent(round + 5, &b2, None, &signer, key.as_ref());
    let a3 =
        make_proposal_with_parent(round + 6, &a2, None, &signer, key.as_ref());
    let a4 =
        make_proposal_with_parent(round + 7, &a3, None, &signer, key.as_ref());
...
/// Verify first that we can successfully sign a timeout on the correct
/// conditions, then ensure that poorly set last_voted_rounds both historical
/// and in the future fail as well as synchronization issues on preferred round
...
fn test_sign_timeout(safety_rules: &Callback) {
    ...
    let p1 =
        make_proposal_with_parent(round + 2, &p0, None, &signer, key.as_ref());
    let p2 =
        make_proposal_with_parent(round + 3, &p1, None, &signer, key.as_ref());
    let p3 =
        make_proposal_with_parent(round + 4, &p2, None, &signer, key.as_ref());
    let p4 =
        make_proposal_with_parent(round + 5, &p3, None, &signer, key.as_ref());
    ...
    let expected_err =
        Error::IncorrectLastVotedRound(timeout.round(), timeout.round() + 1);
    ...
    let expected_err =
        Error::IncorrectPreferredRound(ptimeout.round(), preferred_round);
    assert_eq!(actual_err, expected_err);
...
fn test_voting(safety_rules: &Callback) {
    ...
    let a3 =
        make_proposal_with_parent(round + 5, &a2, None, &signer, key.as_ref());
    let b3 =
        make_proposal_with_parent(round + 6, &b2, None, &signer, key.as_ref());
    let a4 =
        make_proposal_with_parent(round + 7, &a3, None, &signer, key.as_ref());
    let a4_prime =
        make_proposal_with_parent(round + 7, &a2, None, &signer, key.as_ref());
    let b4 =
        make_proposal_with_parent(round + 8, &b2, None, &signer, key.as_ref());
...
fn test_voting_potential_commit_id(safety_rules: &Callback) {
    ...
    let a2 =
        make_proposal_with_parent(round + 3, &a1, None, &signer, key.as_ref());
    let a3 =
        make_proposal_with_parent(round + 4, &a2, None, &signer, key.as_ref());
    let a4 = make_proposal_with_parent(
        round + 5,
        &a3,
        Some(&a2),
    ...
    let a5 = make_proposal_with_parent(
        round + 6,
        &a4,
        Some(&a3),
        &signer,
...
fn test_reconcile_key(_safety_rules: &Callback) {
    ...
    // epoch state, reconciling old signer key with the new one. Later when
    // it tries to verify the QC signed by the old signer key, safety
    ...
    let a1 =
        test_utils::make_proposal_with_qc(round + 1, genesis_qc, &signer, None);
    safety_rules.construct_and_sign_vote(&a1).unwrap();
