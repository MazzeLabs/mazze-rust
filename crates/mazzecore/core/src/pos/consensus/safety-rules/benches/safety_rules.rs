...
fn lsr(
    ...
    round += 1;
    let mut b0 =
        test_utils::make_proposal_with_qc(round, genesis_qc, &signer, None);
    safety_rules.construct_and_sign_vote(&b0).unwrap();

    round += 1;
    let mut b1 = test_utils::make_proposal_with_parent(
        data.clone(),
        round,
        &b0,
        None,
    ...
    round += 1;
    let mut b2 = test_utils::make_proposal_with_parent(
        data.clone(),
        round,
        &b1,
        None,
