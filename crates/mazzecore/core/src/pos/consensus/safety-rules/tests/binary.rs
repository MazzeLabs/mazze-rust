...
use diem_config::{
    config::{
        NodeConfig, PersistableConfig, RemoteService, SafetyRulesService,
    },
    utils,
};
...
fn test_consensus_state() {
    ...
    let server_address =
        SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), server_port).into();
    config.service =
    ...
    // Ensure the safety-rules subprocess is killed whether the test passes or
    // fails. Not doing this would result in a zombie process.
