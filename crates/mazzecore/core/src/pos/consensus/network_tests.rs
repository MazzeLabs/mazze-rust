...
/// `NetworkPlayground` mocks the network implementation and provides
/// convenience methods for testing. Test clients can use `wait_for_messages` or
/// `deliver_messages` to inspect the direct-send messages sent between peers.
/// They can also configure network messages to be dropped between specific
/// peers.
///
...
pub struct NetworkPlayground {
    ...
    node_consensus_txs: Arc<
        ...
        >,
    >,
    /// Nodes' outbound handlers forward their outbound non-rpc messages to
    /// this queue.
    outbound_msgs_tx: mpsc::Sender<(TwinId, PeerManagerRequest)>,
    /// NetworkPlayground reads all nodes' outbound messages through this
    /// queue.
...
impl NetworkPlayground {
    pub fn new(executor: Handle) -> Self {
        ...
        NetworkPlayground {
            ...
            executor,
            author_to_twin_ids: Arc::new(RwLock::new(
                AuthorToTwinIds::default(),
            )),
        }
    }
    ...
    async fn start_node_outbound_handler(
        drop_config: Arc<RwLock<DropConfig>>, src_twin_id: TwinId,
        mut network_reqs_rx: diem_channel::Receiver<
            (PeerId, ProtocolId),
            PeerManagerRequest,
        >,
        ...
        while let Some(net_req) = network_reqs_rx.next().await {
            match net_req {
                ...
                PeerManagerRequest::SendRpc(dst, outbound_req) => {
                    let dst_twin_ids =
                        author_to_twin_ids.read().get_twin_ids(dst);

                    let dst_twin_id =
                        match dst_twin_ids.iter().find(|dst_twin_id| {
                            !drop_config
                                .read()
    ...
    pub fn add_node(
        ...
        consensus_tx: diem_channel::Sender<
            (PeerId, ProtocolId),
            PeerManagerNotification,
        >,
        ...
        network_reqs_rx: diem_channel::Receiver<
            (PeerId, ProtocolId),
            PeerManagerRequest,
        >,
    ...
    async fn deliver_message(
        &mut self, src_twin_id: TwinId, dst_twin_id: TwinId,
    ...
    /// While all the sent messages are delivered (except those configured to be
    /// dropped), only the messages that satisfy the given msg inspector are
    /// counted.
    pub async fn wait_for_messages<F>(
        &mut self, num_messages: usize, msg_inspector: F,
    ) -> Vec<(Author, ConsensusMsg)>
    where F: Fn(&(Author, ConsensusMsg)) -> bool {
        ...
        while msg_copies.len() < num_messages {
            ...
            // Convert PeerManagerRequest to corresponding
            // PeerManagerNotification, and extract destination peer
    ...
    pub fn take_all(_msg_copy: &(Author, ConsensusMsg)) -> bool { true }
    ...
    pub fn extend_author_to_twin_ids(
        &mut self, author: Author, twin_id: TwinId,
    ) {
    ...
    fn is_message_dropped(
        &self, src: &TwinId, dst: &TwinId, msg: ConsensusMsg,
    ) -> bool {
    ...
    pub fn split_network(
        &self, partition_first: Vec<TwinId>, partition_second: Vec<TwinId>,
    ) -> bool {
        self.drop_config
            .write()
    ...
    /// Check if the message from 'src_twin_id' to 'dst_twin_id' should be
    /// dropped in the given round
    pub fn is_message_dropped_round(
        &self, src: &TwinId, dst: &TwinId, round: u64,
    ) -> bool {
    ...
    pub fn split_network_round(
        &mut self, round_partitions: &HashMap<u64, Vec<Vec<TwinId>>>,
    ) -> bool {
        let mut ret = true;
    ...
    pub async fn start(mut self) {
        // Take the next queued message
        while let Some((src_twin_id, net_req)) =
            self.outbound_msgs_rx.next().await
        {
            // Convert PeerManagerRequest to corresponding
            // PeerManagerNotification, and extract destination peer
            let (dst, msg) = match &net_req {
...
impl DropConfig {
    ...
    pub fn split_network(
        &mut self, partition_first: &[TwinId], partition_second: &[TwinId],
    ) -> bool {
        partition_first
            .iter()
    ...
    fn add_node(&mut self, src: TwinId) { self.0.insert(src, HashSet::new()); }
}
...
impl DropConfigRound {
    /// Check if the message from 'src' to 'dst' should be dropped in the given
    /// round
    fn is_message_dropped(
        &self, src: &TwinId, dst: &TwinId, round: u64,
    ) -> bool {
    ...
    fn drop_message_for_round(
        &mut self, round: u64, partition_first: &[TwinId],
...
mod tests {
    ...
    fn test_split_network_round() {
        ...
        let num_nodes = 5;
        let (signers, _validator_verifier) =
        ...
        let mut round_partitions: HashMap<u64, Vec<Vec<TwinId>>> =
            HashMap::new();
        // Round 1 partitions: [0], [1,2]
        round_partitions
            .insert(1, vec![vec![nodes[0]], vec![nodes[1], nodes[2]]]);
    ...
    fn test_network_api() {
        ...
        let (signers, validator_verifier) =
            random_validator_verifier(num_nodes, None, false);
        let peers: Vec<_> =
            signers.iter().map(|signer| signer.author()).collect();
        ...
        for (peer_id, peer) in peers.iter().enumerate() {
            ...
            let network_events =
                ConsensusNetworkEvents::new(consensus_rx, conn_status_rx);
            ...
            let (task, receiver) =
                NetworkTask::new(network_events, self_receiver);
    ...
    fn test_rpc() {
        ...
        let on_request_block = async move {
            while let Some(request) = block_retrieval.next().await {
                ...
                // we limit the network notification queue size to 1 so if it's
                // blocked, we can not process 2 votes and the
                // test will timeout
    ...
    fn test_bad_message() {
        ...
        let (connection_notifs_tx, connection_notifs_rx) =
            diem_channel::new(QueueStyle::FIFO, 8, None);
        let consensus_network_events = ConsensusNetworkEvents::new(
            peer_mgr_notifs_rx,
        ...
        let liveness_check_msg = PeerManagerNotification::RecvRpc(
            ...
            InboundRpcRequest {
                protocol_id,
                data: Bytes::from(bcs::to_bytes(&liveness_check_msg).unwrap()),fV
                res_tx,
            },
        );
