...
use consensus_types::block::{
    block_test_utils::certificate_for_genesis, Block,
};
...
fn test_rotating_proposer() {
    ...
    // Send a proposal from both chosen author and another author, the only
    // winning proposals follow the round-robin rotation.
...
fn test_rotating_proposer_with_three_contiguous_rounds() {
    ...
    let proposers = vec![chosen_author, another_author];
    let pe: Box<dyn ProposerElection> =
        Box::new(RotatingProposer::new(proposers, 3));

    // Send a proposal from both chosen author and another author, the only
    // winning proposals follow the round-robin rotation with 3 contiguous
    // rounds.
...
fn test_fixed_proposer() {
    ...
    // Send a proposal from both chosen author and another author, the only
    // winning proposal is from the chosen author.
