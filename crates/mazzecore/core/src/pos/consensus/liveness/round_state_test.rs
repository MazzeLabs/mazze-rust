...
use consensus_types::{
    common::Round, quorum_cert::QuorumCert, sync_info::SyncInfo,
    timeout::Timeout, timeout_certificate::TimeoutCertificate,
    vote_data::VoteData,
};
...
fn test_round_time_interval() {
    let interval =
        ExponentialTimeInterval::new(Duration::from_millis(3000), 1.5, 2);
...
async fn test_basic_timeout() {
    ...
    for _ in 0..2 {
        ...
        // round for timeout is not changed as no timeout certificate was
...
fn make_round_state() -> (RoundState, channel::Receiver<Round>) {
    let time_interval =
        Box::new(ExponentialTimeInterval::fixed(Duration::from_millis(2)));
    let simulated_time =
        SimulatedTimeService::auto_advance_until(Duration::from_millis(4));
...
fn generate_sync_info(
    quorum_round: Option<Round>, timeout_round: Option<Round>,
