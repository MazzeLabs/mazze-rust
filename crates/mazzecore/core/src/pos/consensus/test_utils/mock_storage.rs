...
impl MockStorage {
    pub fn new_with_ledger_info(
        shared_storage: Arc<MockSharedStorage>, ledger_info: LedgerInfo,
    ) -> Self {
...
impl PersistentLivenessStorage for MockStorage {
    fn save_tree(
        ...
        // When the shared storage is empty, we are expected to not able to
        // construct an block tree from it. During test we will
        // intentionally clear shared_storage to simulate the situation
        // of restarting from an empty consensusDB
        let should_check_for_consistency =
            !(self.shared_storage.block.lock().is_empty()
    ...
    fn save_highest_timeout_cert(
        &self, highest_timeout_certificate: TimeoutCertificate,
    ) -> Result<()> {
        self.shared_storage
            .highest_timeout_certificate
            .lock()
...
impl PersistentLivenessStorage for EmptyStorage {
    ...
    fn prune_tree(&self, _: Vec<HashValue>) -> Result<()> { Ok(()) }
