...
use super::util::mock_time_service::SimulatedTimeService;
use consensus_types::{
    block::block_test_utils::gen_test_certificate, common::Payload,
};
...
impl TreeInserter {
    pub fn default() -> Self { Self::new(ValidatorSigner::random(None)) }
    ...
    pub fn new_with_store(
        signer: ValidatorSigner, block_store: Arc<BlockStore>,
    ) -> Self {
        Self {
            signer,
            block_store,
    ...
    pub fn signer(&self) -> &ValidatorSigner { &self.signer }
    ...
    pub fn insert_block_with_qc(
        &mut self, parent_qc: QuorumCert, parent: &ExecutedBlock, round: Round,
    ) -> Arc<ExecutedBlock> {
        self.block_store
    ...
    pub fn create_qc_for_block(
        &self, block: &ExecutedBlock, committed_block: Option<BlockInfo>,
    ) -> QuorumCert {
    ...
    pub fn insert_qc_for_block(
        ...
        self.block_store
            .insert_single_quorum_cert(
                self.create_qc_for_block(block, committed_block),
            )
            .unwrap()
    }
...
fn nocapture() -> bool { ::std::env::args().any(|arg| arg == "--nocapture") }
...
pub fn timed_block_on<F>(
    runtime: &mut runtime::Runtime, f: F,
) -> <F as Future>::Output
where F: Future {
    runtime
