...
impl RoundManager {
    ...
    async fn process_new_round_event(
        ...
        if let Err(e) = self.broadcast_main_decision().await {
            diem_error!("error in broadcasting main decision tx: {:?}", e);
        }
    ...
    pub async fn broadcast_main_decision(&mut self) -> anyhow::Result<()> {
        if !self.is_validator() {
            // Not an active validator, so do not need to sign main decision.
            return Ok(());
        }
        diem_debug!("broadcast_main_decision starts");
        ...
        // Sending non-existent H256 (default) will return the latest main
        // decision.
        let parent_decision = parent_block
            .main_decision()
            .map(|d| d.block_hash)
            .unwrap_or_default();
        let main_decision = match self
            .block_store
            .pow_handler
            .next_main_decision(parent_decision)
            .await
        {
            Some(res) => res,
            None => {
                // No new main decision.
                diem_debug!("No new main decision");
                return Ok(());
            }
        };
        ...
        diem_info!("Broadcast new main decision: {:?}", main_decision);
        // It's allowed for a node to sign conflict main decision,
        // so we do not need to persist this signing event.
        let raw_tx = RawTransaction::new_main_decision(
            proposal_generator.author(),
            MainBlockDecision {
                block_hash: main_decision.1,
                height: main_decision.0,
            },
            self.chain_id,
        );
        let signed_tx =
        ...
        rx.await??;
        diem_debug!("broadcast_main_decision sends");
        Ok(())
    }
...
impl RoundManager {
    ...
    pub async fn force_sign_main_decision(
        &mut self, main_decision: MainBlockDecision,
    ) -> anyhow::Result<()> {
        let proposal_generator = self.proposal_generator.as_ref().ok_or(
            anyhow::anyhow!("Non-validator cannot sign main decision"),
        )?;
        diem_info!("force_sign_main_decision: {:?}", main_decision);
        // It's allowed for a node to sign conflict main decision,
        // so we do not need to persist this signing event.
        let raw_tx = RawTransaction::new_main_decision(
            proposal_generator.author(),
        ...
        // TODO(lpl): Check if we want to wait here.
        rx.await??;
        diem_debug!("force_sign_main_decision sends");
        Ok(())
    }
