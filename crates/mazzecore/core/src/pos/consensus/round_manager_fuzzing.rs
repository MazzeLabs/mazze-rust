...
static FUZZING_SIGNER: Lazy<ValidatorSigner> =
    Lazy::new(|| ValidatorSigner::from_int(1));

// helpers
fn build_empty_store(
    storage: Arc<dyn PersistentLivenessStorage>, initial_data: RecoveryData,
) -> Arc<BlockStore> {
    let (_commit_cb_sender, _commit_cb_receiver) =
...
fn create_node_for_fuzzing() -> RoundManager {
    ...
    // TODO: remove
    let validator =
        ValidatorVerifier::new_single(signer.author(), signer.public_key());
    ...
    let proposer_election =
        Box::new(RotatingProposer::new(vec![signer.author()], 1));
