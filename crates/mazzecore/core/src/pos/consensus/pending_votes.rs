...
use consensus_types::{
    common::Author, quorum_cert::QuorumCert,
...
/// Result of the vote processing. The failure case (Verification error) is
/// returned as the Error part of the result.
#[derive(Debug, PartialEq)]
pub enum VoteReceptionResult {
    /// The vote has been added but QC has not been formed yet. Return the
    /// amount of voting power the given (proposal, execution) pair.
    VoteAdded(u64),
...
pub struct PendingVotes {
    ...
    /// LedgerInfoWithSignatures). This might keep multiple LedgerInfos for
    /// the current round: either due to different proposals (byzantine
    /// behavior) or due to different NIL proposals (clients can have a
    /// different view of what block to extend).
    li_digest_to_votes: HashMap<
        HashValue, /* LedgerInfo digest */
        LedgerInfoWithSignatures,
    >,
    /// Tracks all the signatures of the votes for the given round. In case we
    /// succeed to aggregate 2f+1 signatures a TimeoutCertificate is
    /// formed.
...
impl PendingVotes {
    ...
    /// Insert a vote and if the vote is valid, return a QuorumCertificate
    /// preferentially over a TimeoutCertificate if either can can be formed
    pub fn insert_vote(
        &mut self, vote: &Vote, validator_verifier: &ValidatorVerifier,
    ) -> VoteReceptionResult {
        ...
        // obtain the ledger info with signatures associated to the vote's
        // ledger info
        let li_with_sig =
            self.li_digest_to_votes.entry(li_digest).or_insert_with(|| {
                // if the ledger info with signatures doesn't exist yet, create
                // it
        ...
        let voting_power = match validator_verifier
            .check_voting_power(li_with_sig.signatures().keys())
        {
        ...
        if let Some(timeout_signature) = vote.timeout_signature() {
            // form timeout struct
            // TODO(mimoo): stronger: pass the (epoch, round) tuple as arguments
            // of this function
...
impl fmt::Display for PendingVotes {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let votes = self
            ...
            .iter()
            .map(|(li_digest, li)| {
                (li_digest, li.signatures().keys().collect::<Vec<_>>())
            })
            .collect::<BTreeMap<_, _>>();

        // collect timeout votes
        let timeout_votes = self.maybe_partial_tc.as_ref().map(|partial_tc| {
            partial_tc.signatures().keys().collect::<Vec<_>>()
        });
...
mod tests {
    ...
    #[test]
    /// Verify that votes are properly aggregated to QC based on their
    /// LedgerInfo digest
    fn test_qc_aggregation() {
        ...
        let vote_data_1_author_0 =
            Vote::new(vote_data_1, signers[0].author(), li1, &signers[0]);
        ...
        let vote_data_2_author_2 =
            Vote::new(vote_data_2, signers[2].author(), li2, &signers[2]);
    ...
    fn test_tc_aggregation() {
        ...
        let mut vote1_author_0 =
            Vote::new(vote1, signers[0].author(), li1, &signers[0]);
        ...
        // another vote for a different block cannot form a TC if it doesn't
        // have a timeout signature
        let li2 = random_ledger_info();
        let vote2 = random_vote_data();
        let mut vote2_author_1 =
            Vote::new(vote2, signers[1].author(), li2, &signers[1]);
        assert_eq!(
            pending_votes.insert_vote(&vote2_author_1, &validator),
            VoteReceptionResult::VoteAdded(1)
        );

        // if that vote is now enhanced with a timeout signature ->
