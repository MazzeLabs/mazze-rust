...
impl SMRNode {
    fn start(
        playground: &mut NetworkPlayground, config: NodeConfig,
        storage: Arc<MockStorage>, twin_id: TwinId,
    ) -> Self
    {
        ...
        let network_events =
            ConsensusNetworkEvents::new(consensus_rx, conn_notifs_channel);
        ...
        let consensus_to_mempool_sender =
            shared_mempool.consensus_sender.clone();
    ...
    pub fn start_num_nodes_with_twins(
        num_nodes: usize, num_twins: usize, playground: &mut NetworkPlayground,
    ...
    ) -> Vec<Self>
    {
        ...
        let validator_set = ValidatorSet::new(
            node_configs
                ...
                .map(|config| {
                    let sr_test_config =
                        config.consensus.safety_rules.test.as_ref().unwrap();
        ...
        let proposer_type = match proposer_type {
            RoundProposer(_) => {
                let mut round_proposers: HashMap<Round, Author> =
                    HashMap::new();
                ...
                if let Some(proposers) = round_proposers_idx {
                    proposers.iter().for_each(|(round, idx)| {
                        round_proposers.insert(
                            *round,
                            author_from_config(&node_configs[*idx]),
                        );
                    })
                }
        ...
        for (smr_id, mut config) in node_configs.into_iter().enumerate() {
            let (_, storage) =
                MockStorage::start_for_testing(validator_set.clone());

            let waypoint =
                Waypoint::new_epoch_boundary(&storage.get_ledger_info())
                    .expect(
                    "Unable to produce waypoint with the provided LedgerInfo",
                );
            config
                .consensus
                .safety_rules
