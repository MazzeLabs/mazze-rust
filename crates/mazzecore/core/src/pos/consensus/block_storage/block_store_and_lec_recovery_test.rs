...
use execution_correctness::{
    ExecutionCorrectness, ExecutionCorrectnessManager,
};
...
fn build_inserter(
    config: &NodeConfig, initial_data: RecoveryData,
...
fn test_executor_restart() {
    ...
    let _execution_correctness_manager =
        ExecutionCorrectnessManager::new(&config);
