...
impl EpochManager {
    async fn process_test_command(
        ...
        match command {
            ...
            TestCommand::BroadcastMainDecision(decision) => {
                self.force_sign_main_decision(decision).await
            }
    ...
    async fn force_sign_main_decision(
        &mut self, main_decision: MainBlockDecision,
    ) -> anyhow::Result<()> {
        match self.processor_mut() {
            RoundProcessor::Normal(p) => {
                p.force_sign_main_decision(main_decision).await
            }
            _ => anyhow::bail!("RoundManager not started yet"),
        }
    }
