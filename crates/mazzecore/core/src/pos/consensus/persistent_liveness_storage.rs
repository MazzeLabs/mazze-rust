...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    fn prune_staking_events(
        &self, _committed_main_decision: &MainBlockDecision,
    ) -> Result<()> {
        unimplemented!()
    }
...
pub struct RootMetadata {
    ...
    pub main_decision: Option<MainBlockDecision>,
}
...
impl PersistentLivenessStorage for StorageWriteProxy {
    ...
    fn prune_staking_events(
        &self, committed_main_decision: &MainBlockDecision,
    ) -> Result<()> {
        Ok(self
            .db
            .delete_staking_events_before(committed_main_decision.height)?)
    }
}
