...
where V: VMExecutor
{
    ...
    fn process_vm_outputs(
        ...
        let main_select_event_key =
            MainBlockDecision::main_select_event_key();
        ...
        // Find the next main block.
        let mut main_decision = None;
        let mut new_pos_state = parent_trees.pos_state().clone();
        let parent_main_decision = new_pos_state.main_decision().clone();
        for vm_output in vm_outputs.clone().into_iter() {
            for event in vm_output.events() {
                // check for main block selection.
                if *event.key() == main_select_event_key {
                    if main_decision.is_some() {
                        bail!("Multiple main decisions in one block!");
                    }
                    main_decision = Some(MainBlockDecision::from_bytes(
                        event.event_data(),
                    )?);
        ...
        if *parent_block_id != *PRE_GENESIS_BLOCK_ID {
            if let Some(main_decision) = &main_decision {
                diem_debug!(
                    "process_vm_outputs: parent={:?} parent_main={:?}",
                    parent_block_id,
                    parent_main_decision
                );

                // The check and event processing below will be skipped during
                // PoS catching up, because pow has not processed these main
                // decisions.
                if !catch_up_mode {
                    if !self.pow_handler.validate_proposal_main_decision(
                        parent_main_decision.block_hash,
                        main_decision.block_hash,
                    ) {
                        bail!("Invalid main decision for block");
                    }
                    ...
                    let staking_events = self.pow_handler.get_staking_events(
                        parent_main_decision.height,
                        main_decision.height,
                        parent_main_decision.block_hash,
                        main_decision.block_hash,
                    )?;
                ...
                // No new main decision, so there should be no staking-related
                // transactions.
                ...
                main_decision = Some(parent_main_decision);
            }
        }
        // TODO(lpl): This is only for pos-tool
        if let Some(main_decision) = &main_decision {
            new_pos_state.set_main_decision(main_decision.clone());
        }
    ...
    fn execute_block(
                ...
                .has_reconfiguration()
        {
            ...
            let mut output = ProcessedVMOutput::new(
                ...
                // The block has no main decision transaction, so it's the
                // same as the parent.
                parent_output.main_block().clone(),
            );
        ...
        if ledger_info_with_sigs.ledger_info().epoch() != 0 {
            for (i, b) in blocks.iter().enumerate() {
                ...
                committed_blocks.push(CommittedBlock {
                    ...
                    main_decision: b.output().main_block().clone().unwrap(),
            ...
            committed_blocks.push(CommittedBlock {
                ...
                main_decision: ledger_info_with_sigs
                    .ledger_info()
                    .main_decision()
                    .unwrap()
                    .clone(),
