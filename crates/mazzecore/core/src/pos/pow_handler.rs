...
use mazze_executor::internal_contract::decode_register_info;
...
impl PowHandler {
    ...
    fn next_main_decision_impl(
        ...
        inner.get_next_main_decision(parent_decision, confirmed_height)
    }

    fn validate_proposal_main_decision_impl(
        pow_consensus: Arc<ConsensusGraph>, parent_decision: &H256,
        me_decision: &H256,
    ) -> bool {
        pow_consensus
            .inner
            .read()
            .validate_main_decision(parent_decision, me_decision)
    }
...
impl PowInterface for PowHandler {
    async fn next_main_decision(
        ...
        self.executor.spawn(async move {
            let r =
                Self::next_main_decision_impl(pow_consensus, &parent_decision);
            if let Err(e) = callback.send(r) {
                debug!("send next_main_decision err={:?}", e);
            }
        });
        cb_receiver.await.ok().flatten()
    }

    fn validate_proposal_main_decision(
        ...
        let pow_consensus = pow_consensus.unwrap();
        debug!("before spawn main_decision");
        let r = Self::validate_proposal_main_decision_impl(
            pow_consensus,
            &parent_decision,
            &me_decision,
        );
        debug!("after spawn main_decision");
        r
    }

    /// Return error if pow_consensus has not been initialized or the main
    /// decision blocks have not been processed in PoW. Thus, a PoS node
    /// will not vote for new main decisions if the PoW block has not been
    /// processed.
    ...
    async fn wait_for_initialization(&self, last_decision: H256) {
        ...
        loop {
            ...
            // on the main chain. Note that for full nodes, there
            // is no other persisted data to check for old blocks.
            {
                if self
                    ...
                    .inner
                    .read()
                    .main_block_processed(&last_decision)
                {
                    return;
                }
            }
