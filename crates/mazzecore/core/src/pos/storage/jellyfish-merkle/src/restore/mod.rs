...
//! This module implements the functionality to restore a `JellyfishMerkleTree`
...
use crate::{
    ...
    node_type::{
        get_child_and_sibling_half_start, Child, Children, InternalNode,
        LeafNode, Node, NodeKey,
    },
...
enum ChildInfo<V> {
    /// This child is an internal node. The hash of the internal node is stored
    /// here if it is known, otherwise it is `None`. In the process of
    /// restoring a tree, we will only know the hash of an internal node
    /// after we see all the keys that share the same prefix.
...
where V: crate::Value
{
    /// Converts `self` to a child, assuming the hash is known if it's an
    /// internal node.
...
struct InternalInfo<V> {
    ...
    /// The existing children. Every time a child appears, the corresponding
    /// position will be set to `Some`.
...
where V: crate::Value
{
    ...
    /// Converts `self` to an internal node, assuming all of its children are
    /// already known and fully initialized.
...
pub struct JellyfishMerkleRestore<V> {
    ...
    /// The nodes we have partially restored. Each `partial_nodes[i-1]` is the
    /// parent of `partial_nodes[i]`. If a node `partial_nodes[i-1]` has
    /// multiple children, only the rightmost known child will appear here
    /// as `partial_nodes[i]`, because any other children on the left would
    /// have been frozen.
    ///
    ...
    /// ```
    ///
    /// We insert the accounts from left to right. So if the next account
    /// appears at position `A`, it will cause the leaf at position `B` to
    /// be frozen. If it appears at position `B`, it might cause a few
    /// internal nodes to be created additionally. If it appears at position
    /// `C`, it will also cause `partial_nodes[1]` to be added to
    /// `frozen_nodes` as an internal node and be removed from
    /// `partial_nodes`.
    partial_nodes: Vec<InternalInfo<V>>,

    /// The nodes that have been fully restored and are ready to be written to
    /// storage.
    frozen_nodes: NodeBatch<V>,

    /// The most recently added leaf. This is used to ensure the keys come in
    /// increasing order and do proof verification.
    previous_leaf: Option<LeafNode<V>>,

    /// The number of keys we have received since the most recent restart.
    num_keys_received: u64,

    /// When the restoration process finishes, we expect the tree to have this
    /// root hash.
...
where V: crate::Value
{
    pub fn new<D: 'static + TreeReader<V> + TreeWriter<V>>(
        store: Arc<D>, version: Version, expected_root_hash: HashValue,
    ) -> Result<Self> {
        let tree_reader = Arc::clone(&store);
        let (partial_nodes, previous_leaf) =
            if let Some((node_key, leaf_node)) =
                tree_reader.get_rightmost_leaf()?
            {
                // TODO: confirm rightmost leaf is at the desired version
                // If the system crashed in the middle of the previous
                // restoration attempt, we need to recover the
                // partial nodes to the state right before the crash.
                (
                    Self::recover_partial_nodes(
                        tree_reader.as_ref(),
                        version,
                        node_key,
                    )?,
    ...
    pub fn new_overwrite<D: 'static + TreeWriter<V>>(
        ...
        Ok(Self {
            store,
            version,
            partial_nodes: vec![InternalInfo::new_empty(
    ...
    /// Recovers partial nodes from storage. We do this by looking at all the
    /// ancestors of the rightmost leaf. The ones do not exist in storage
    /// are the partial nodes.
    fn recover_partial_nodes(
        ...
        // Start from the parent of the rightmost leaf. If this internal node
        // exists in storage, it is not a partial node. Go to the parent
        // node and repeat until we see a node that does not exist. This
        // node and all its ancestors will be the partial nodes.
        ...
        // starting from the bottom. For all of them, we scan all its
        // possible child positions and see if there is one at
        // each position. If the node is not the bottom one, there is
        // additionally a partial node child at the position
        // `previous_child_index`.
        let mut partial_nodes = vec![];
        // Initialize `previous_child_index` to `None` for the first iteration
        // of the loop so the code below treats it differently.
        ...
        loop {
            ...
            for i in 0..previous_child_index.unwrap_or(16) {
                let child_node_key =
                    node_key.gen_child_node_key(version, (i as u8).into());
                if let Some(node) = store.get_node_option(&child_node_key)? {
                    let child_info = match node {
                        ...
                        Node::Leaf(leaf_node) => {
                            ChildInfo::Leaf { node: leaf_node }
                        }
                        Node::Null => {
                            bail!("Null node should not appear in storage.")
                        }
                    };
            ...
            // If this is not the lowest partial node, it will have a partial
            // node child at `previous_child_index`. Set the hash of
            // this child to `None` because it is a partial node and
            // we do not know its hash yet. For the lowest partial node, we just
            // find all its known children from storage in the loop above.
            if let Some(index) = previous_child_index {
                internal_info
                    .set_child(index, ChildInfo::Internal { hash: None });
            }
            ...
            previous_child_index =
                node_key.nibble_path().last().map(|x| u8::from(x) as usize);
            node_key = node_key.gen_parent_node_key();
        }
    ...
    /// Restores a chunk of accounts. This function will verify that the given
    /// chunk is correct using the proof and root hash, then write things to
    /// storage. If the chunk is invalid, an error will be returned and
    /// nothing will be written to storage.
    pub fn add_chunk(
        &mut self, chunk: Vec<(HashValue, V)>, proof: SparseMerkleRangeProof,
    ) -> Result<()> {
    ...
    fn add_one(&mut self, new_key: HashValue, new_value: V) {
        ...
        for i in 0..ROOT_NIBBLE_HEIGHT {
            let child_index =
                u8::from(nibbles.next().expect("This nibble must exist."))
                    as usize;

            match self.partial_nodes[i].children[child_index] {
                Some(ref child_info) => {
                    // If there exists an internal node at this position, we
                    // just continue the loop with the next
                    // nibble. Here we deal with the leaf case.
                ...
                None => {
                    // This means that we are going to put a leaf in this
                    // position. For all the descendants on
                    // the left, they are now frozen.
                    self.freeze(i + 1);
                    ...
                    // We do not add this leaf node to self.frozen_nodes because
                    // we don't know its node key yet. We
                    // will know its node key when the next account comes.
                    break;
                }
            }
        }
    ...
    /// Inserts a new account at the position of the existing leaf node. We may
    /// need to create multiple internal nodes depending on the length of
    /// the common prefix of the existing key and the new key.
    fn insert_at_leaf(
        ...
        // insert more nodes at this position in the future, we do not
        // know its hash yet.
        self.partial_nodes[num_existing_partial_nodes - 1]
            .set_child(child_index, ChildInfo::Internal { hash: None });

        // Next we build the new internal nodes from top to bottom. All these
        // internal node except the bottom one will now have a single
        // internal node child.
        ...
        for _ in num_existing_partial_nodes..common_prefix_len {
            ...
            let next_nibble =
                remaining_nibbles.next().expect("This nibble must exist.");
        ...
        // set the new child later, because this way it's easier in
        // `freeze` to find the correct leaf to freeze -- it's always
        // the rightmost leaf on the lowest level.
        self.partial_nodes.push(internal_info);
    ...
    /// Freezes the previously added leaf node. It should always be the
    /// rightmost leaf node on the lowest level, inserted in the previous
    /// `add_one` call.
    ...
    /// Freeze extra internal nodes. Only `num_remaining_nodes` partial internal
    /// nodes will be kept and the ones on the lower level will be frozen.
    fn freeze_internal_nodes(&mut self, num_remaining_nodes: usize) {
        while self.partial_nodes.len() > num_remaining_nodes {
            let last_node =
                self.partial_nodes.pop().expect("This node must exist.");
            let (node_key, internal_node) =
                last_node.into_internal_node(self.version);
            // Keep the hash of this node before moving it into `frozen_nodes`,
            // so we can update its parent later.
            let node_hash = internal_node.hash();
            self.frozen_nodes.insert(node_key, internal_node.into());

            // Now that we have computed the hash of the internal node above, we
            // will also update its parent unless it is root node.
            if let Some(parent_node) = self.partial_nodes.last_mut() {
                // This internal node must be the rightmost child of its parent
                // at the moment.
    ...
    /// Verifies that all accounts that have been added so far (from the
    /// leftmost one to `self.previous_leaf`) are correct, i.e., we are able
    /// to construct `self.expected_root_hash` by combining all existing
    /// accounts and `proof`.
    ...
    fn verify(&self, proof: SparseMerkleRangeProof) -> Result<()> {
        ...
        // If we have all siblings on the path from root to `previous_key`, we
        // should be able to compute the root hash. The siblings on the
        // right are already in the proof. Now we compute the siblings
        // on the left side, which represent all the accounts that have ever
        // been added.
        let mut left_siblings = vec![];

        // The following process might add some extra placeholder siblings on
        // the left, but it is nontrivial to determine when the loop
        // should stop. So instead we just add these siblings for now
        // and get rid of them in the next step.
        let mut num_visited_right_siblings = 0;
        for (i, bit) in previous_key.iter_bits().enumerate() {
            if bit {
                // This node is a right child and there should be a sibling on
                // the left.
                ...
                // This node is a left child and there should be a sibling on
                // the right.
        ...
        // Now we remove any extra placeholder siblings at the bottom. We keep
        // removing the last sibling if 1) it's a placeholder 2) it's a
        // sibling on the left.
    ...
    fn compute_left_sibling(
        partial_node: &InternalInfo<V>, n: Nibble, height: u8,
    ) -> HashValue {
        assert!(height < 4);
        let width = 1usize << height;
        let start = get_child_and_sibling_half_start(n, height).1 as usize;
        Self::compute_left_sibling_impl(
            &partial_node.children[start..start + width],
        )
        .0
    }

    /// Returns the hash for given portion of the subtree and whether this part
    /// is a leaf node.
    fn compute_left_sibling_impl(
        children: &[Option<ChildInfo<V>>],
    ) -> (HashValue, bool) {
    ...
    /// Finishes the restoration process. This tells the code that there is no
    /// more account, otherwise we can not freeze the rightmost leaf and its
    /// ancestors.
