...
use crate::{
    mock_tree_store::MockTreeStore, node_type::LeafNode, JellyfishMerkleTree,
};
use diem_crypto::{
...
impl From<Vec<u8>> for ValueBlob {
    fn from(blob: Vec<u8>) -> Self { Self(blob) }
}
...
/// version.
pub fn init_mock_db<V>(
    kvs: &HashMap<HashValue, V>,
) -> (MockTreeStore<V>, Version)
...
pub fn arb_existent_kvs_and_nonexistent_keys<V: crate::TestValue>(
    num_kvs: usize, num_non_existing_keys: usize,
) -> impl Strategy<Value = (HashMap<HashValue, V>, Vec<HashValue>)> {
    hash_map(any::<HashValue>(), any::<V>(), 1..num_kvs).prop_flat_map(
        move |kvs| {
...
pub fn arb_tree_with_index<V: crate::TestValue>(
    ...
    btree_map(any::<HashValue>(), any::<V>(), 1..tree_size).prop_flat_map(
        |btree| {
            let len = btree.len();
            (Just(btree), 0..len)
        },
...
pub fn test_get_range_proof<V: crate::TestValue>(
...
fn verify_range_proof<V: crate::TestValue>(
    ...
    // leaves are discarded. In this example, we assume `btree` has the keys
    // `a` to `e` and the proof has `X` and `h` in the siblings.

    // Now we want to construct a set of key-value pairs that covers the entire
    // set of leaves. For `a` to `e` this is simple -- we just insert them
    // directly into this set. For the rest of the leaves, they are
    // represented by the siblings, so we just make up some keys that make
    // sense. For example, for `X` we just use 101000... (more zeros
    // omitted), because that is one key that would cause `X` to end up in
    // the above position.
    ...
    // Using the above example, `last_proven_key` is `e`. We look at the path
    // from root to `e`. For each 0-bit, there should be a sibling in the
    // proof. And we use the path from root to this position, plus a `1` as
    // the key.
    ...
    {
        // This means the `i`-th bit is zero. We take `i` bits from
        // `last_proven_key` and append a one to make up the key for
        // this sibling.
        let mut buf: Vec<_> = last_proven_key.iter_bits().take(i).collect();
        buf.push(true);
        // The rest doesn't matter, because they don't affect the position of
        // the node. We just add zeros.
    ...
    for (key, value) in &btree1 {
        // The length of the common prefix of the previous key and the current
        // key.
        let prev_common_prefix_len = prev_key(&btree1, key)
            .map(|pkey| pkey.common_prefix_bits_len(*key));
        // The length of the common prefix of the next key and the current key.
        let next_common_prefix_len = next_key(&btree1, key)
            .map(|nkey| nkey.common_prefix_bits_len(*key));

        // We take the longest common prefix of the current key and its
        // neighbors. That's how much we need to keep.
...
fn compute_root_hash_impl(kvs: Vec<(&[bool], HashValue)>) -> HashValue {
    ...
    // Otherwise the tree has more than one leaves, which means we can find
    // which ones are in the left subtree and which ones are in the right
    // subtree. So we find the first key that starts with a 1-bit.
    let left_hash;
    let right_hash;
    match kvs.iter().position(|(key, _value)| key[0]) {
        Some(0) => {
            // Every key starts with a 1-bit, i.e., they are all in the right
            // subtree.
        ...
        None => {
            // Every key starts with a 0-bit, i.e., they are all in the left
            // subtree.
