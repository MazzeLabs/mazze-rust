...
//! A transaction can have multiple operations on state. For example, it might
//! update values for a few existing keys. Imagine that we have the following
//! tree.
//!
//! ```text
//!                 root0
...
//! ```
//!
//! The next transaction updates `key1`'s value to `value12` and `key2`'s value
//! to `value22`. Let's assume we update key2 first. Then the tree becomes:
//!
//! ```text
//!                   (on disk)              (in memory)
//!                     root0                  root1'
...
//!
//! Note that
//!   1) we created a new version of the tree with `root1'` and the new `key2`
//! node generated;   2) both `root1'` and the new `key2` node are still held in
//! memory within a batch of nodes      that will be written into db atomically.
//!
//! Next, we need to update `key1`'s value. This time we are dealing with the
//! tree starting from the new root. Part of the tree is in memory and the rest
//! of it is in database. We'll update the left child and the new root. We
//! should   1) create a new version for `key1` child.
...
//! ```
//!
//! This means that we need to be able to tell whether to create a new version
//! of a node or to update an existing node by deleting it and creating a new
//! node directly. `TreeCache` provides APIs to cache intermediate nodes and
//! values in memory and simplify the actual tree implementation.
//!
//! If we are dealing with a single-version tree, any complex tree operation can
//! be seen as a collection of the following operations:
//!   - Put a new node.
//!   - Delete a node.
//! When we apply these operations on a multi-version tree:
//!   1) Put a new node.
//!   2) When we remove a node, if the node is in the previous on-disk version,
//! we don't need to do      anything. Otherwise we delete it from the tree
//! cache. Updating node could be operated as deletion of the node followed by
//! insertion of the updated node.
...
/// `FrozenTreeCache` is used as a field of `TreeCache` storing all the nodes
/// and values that are generated by earlier transactions so they have to be
/// immutable. The motivation of `FrozenTreeCache` is to let `TreeCache` freeze
/// intermediate results from each transaction to help commit more than one
/// transaction in a row atomically.
struct FrozenTreeCache<V> {
    ...
    /// the stats vector including the number of new nodes, new leaves, stale
    /// nodes and stale leaves.
...
/// `TreeCache` is a in-memory cache for per-transaction updates of sparse
/// Merkle nodes and values.
pub struct TreeCache<'a, R, V> {
    /// `NodeKey` of the current root node in cache.
    root_node_key: NodeKey,

    /// The version of the transaction to which the upcoming `put`s will be
    /// related.
    ...
    /// The immutable part of this cache, which will be committed to the
    /// underlying storage.
    ...
    V: crate::Value,
{
    ...
    pub fn new(reader: &'a R, next_version: Version) -> Result<Self> {
        ...
        let root_node_key = if next_version == 0 {
            let pre_genesis_root_key =
                NodeKey::new_empty_path(PRE_GENESIS_VERSION);
            let pre_genesis_root =
                reader.get_node_option(&pre_genesis_root_key)?;

            match pre_genesis_root {
                Some(_) => {
                    // This is to support the extreme case where things really
                    // went wild, and we need to ditch the
                    // transaction history and apply a new
                    // genesis on top of an existing state db.
                    pre_genesis_root_key
                }
                None => {
                    // Hack: We need to start from an empty tree, so we insert
                    // a null node beforehand deliberately to deal with this
                    // corner case.
                    let genesis_root_key = NodeKey::new_empty_path(0);
                    node_cache
                        .insert(genesis_root_key.clone(), Node::new_null());
                    genesis_root_key
                }
            }
        } else {
    ...
    /// Gets a node with given node key. If it doesn't exist in node cache, read
    /// from `reader`.
    ...
    pub fn get_root_node_key(&self) -> &NodeKey { &self.root_node_key }
    ...
    pub fn put_node(
        &mut self, node_key: NodeKey, new_node: Node<V>,
    ) -> Result<()> {
        match self.node_cache.entry(node_key) {
            ...
            Entry::Occupied(o) => {
                bail!("Node with key {:?} already exists in NodeBatch", o.key())
            }
        };
        Ok(())
    }
    ...
    pub fn delete_node(&mut self, old_node_key: &NodeKey, is_leaf: bool) {
        // If node cache doesn't have this node, it means the node is in the
        // previous version of the tree on the disk.
        if self.node_cache.remove(&old_node_key).is_none() {
            let is_new_entry =
                self.stale_node_index_cache.insert(old_node_key.clone());
    ...
    /// Freezes all the contents in cache to be immutable and clear
    /// `node_cache`.
    pub fn freeze(&mut self) {
        ...
        self.frozen_cache.stale_node_index_cache.extend(
            self.stale_node_index_cache.drain().map(|node_key| {
                StaleNodeIndex {
                    stale_since_version,
                    node_key,
                }
...
impl<'a, R, V> From<TreeCache<'a, R, V>>
    for (Vec<HashValue>, TreeUpdateBatch<V>)
where R: 'a + TreeReader<V>
{
    fn from(tree_cache: TreeCache<'a, R, V>) -> Self {
        (
            ...
            TreeUpdateBatch {
                ...
                stale_node_index_batch: tree_cache
                    .frozen_cache
