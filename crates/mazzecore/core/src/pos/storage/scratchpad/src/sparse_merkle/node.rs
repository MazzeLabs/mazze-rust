...
impl<V: CryptoHash> InternalNode<V> {
    pub fn calc_hash(&self) -> HashValue {
        SparseMerkleInternalNode::new(self.left.hash(), self.right.hash())
            .hash()
    }
}
...
impl<V> NodeHandle<V> {
    pub fn new_unkonwn() -> Self { Self::Weak(Weak::new()) }

    pub fn new_shared(node: Node<V>) -> Self { Self::Shared(Arc::new(node)) }
...
impl<V: CryptoHash> SubTree<V> {
    pub fn new_empty() -> Self { Self::Empty }
    ...
    pub fn new_leaf_with_value_hash(
        key: HashValue, value_hash: HashValue,
    ) -> Self {
    ...
    #[cfg(test)]
    pub fn is_empty(&self) -> bool { matches!(self, SubTree::Empty) }
}
