...
fn hash_internal(left_child: HashValue, right_child: HashValue) -> HashValue {
    diem_types::proof::SparseMerkleInternalNode::new(left_child, right_child)
        .hash()
}
...
fn test_construct_subtree_zero_siblings() {
    ...
    let subtree = SparseMerkleTree::construct_subtree(
        std::iter::empty(),
        std::iter::empty(),
        leaf,
    );
...
fn test_construct_subtree_three_siblings() {
    ...
    let subtree =
        SparseMerkleTree::construct_subtree(bits.into_iter(), siblings, leaf);
...
fn test_construct_subtree_panic() {
    ...
    let _ = SparseMerkleTree::construct_subtree(
        std::iter::once(true),
        std::iter::empty(),
        unknown,
    );
...
fn test_construct_subtree_with_new_leaf_create_extension() {
    ...
    let existing_leaf =
        SubTree::new_leaf_with_value_hash(existing_key, existing_blob.hash());
...
#[test]
#[should_panic(
    expected = "Reached an internal node at the bottom of the tree."
)]
fn test_construct_subtree_at_bottom_found_internal_node() {
    let left_child =
        SubTree::new_unknown(HashValue::new([1; HashValue::LENGTH]));
...
fn test_construct_subtree_at_bottom_found_leaf_node() {
    ...
    let current =
        SubTree::new_leaf_with_value_hash(existing_key, existing_blob_hash);
    let remaining_bits = {
    ...
    let leaf =
        Some(SparseMerkleLeafNode::new(existing_key, existing_blob_hash));
...
fn test_construct_subtree_at_bottom_found_subtree_node() {
    ...
    let new_leaf_hash = hash_leaf(new_key, new_blob_hash);
    let x_hash =
        hash_internal(HashValue::new([6; HashValue::LENGTH]), new_leaf_hash);
    let new_subtree_hash =
        hash_internal(x_hash, HashValue::new([5; HashValue::LENGTH]));
...
fn test_update_256_siblings_in_proof() {
    ...
    let mut siblings: Vec<_> =
        std::iter::repeat(*SPARSE_MERKLE_PLACEHOLDER_HASH)
            .take(255)
            .collect();
    ...
    let old_root_hash =
        siblings.iter().fold(leaf1_hash, |previous_hash, hash| {
    ...
    let new_root_hash =
        siblings.iter().fold(new_leaf1_hash, |previous_hash, hash| {
            hash_internal(previous_hash, *hash)
        });
...
fn test_update() {
    ...
    // For smt2, only key1 should be available since smt2 was constructed by
    // updating smt1 with key1.
    assert_eq!(smt2.get(key1), AccountStatus::ExistsInScratchPad(value1));
    assert_eq!(smt2.get(key2), AccountStatus::Unknown);
    assert_eq!(smt2.get(key3), AccountStatus::Unknown);
    assert_eq!(smt2.get(key4), AccountStatus::Unknown);

    // For smt22, only key4 should be available since smt22 was constructed by
    // updating smt1 with key4.
    assert_eq!(smt22.get(key1), AccountStatus::Unknown);
    assert_eq!(smt22.get(key2), AccountStatus::Unknown);
    assert_eq!(smt22.get(key3), AccountStatus::Unknown);
