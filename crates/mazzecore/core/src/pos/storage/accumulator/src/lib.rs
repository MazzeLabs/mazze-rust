...
//! This module provides algorithms for accessing and updating a Merkle
//! Accumulator structure persisted in a key-value store. Note that this doesn't
//! write to the storage directly, rather, it reads from it via the `HashReader`
//! trait and yields writes via an in memory `HashMap`.
//!
//! # Merkle Accumulator
//! Given an ever growing (append only) series of "leaf" hashes, we construct an
...
//!
//! # Leaf Nodes
//! Leaf nodes carry hash values to be stored and proved. They are only appended
//! to the tree but never deleted or updated.
//!
//! # Internal Nodes
//! A non-leaf node carries the hash value derived from both its left and right
//! children.
//!
//! # Placeholder Nodes
//! To make sure each Leaf node has a Merkle Proof towards the root, placeholder
...
//!
//! A placeholder node can appear as either a Leaf node or a non-Leaf node, but
//! there is at most one placeholder leaf at any time.
//!
//! # Frozen Nodes & Non-frozen Nodes
//! As leaves are added to the tree, placeholder nodes get replaced by
...
//!
//! Other nodes, which have one or more placeholder descendants are Non-Frozen.
//! As new elements are appended to the accumulator the hash value of these
//! nodes will change.
//!
//! # Leaf Count
//! Given a count of the number of leaves in a Merkle Accumulator it is possible
//! to determine the shape of the accumulator -- which nodes are filled and
//! which nodes are placeholder nodes.
//!
//! Example:
...
//! As a Merkle Accumulator tree expands to the right and upwards, we number
...
//! unless otherwise stated, this is the in-order position.
//!
//! For writing to disk however, we write all the children of a node before the
...
use diem_types::proof::{
    ...
    AccumulatorConsistencyProof, AccumulatorProof, AccumulatorRangeProof,
    ...
    H: CryptoHasher,
{
    ...
    /// `num_existing_leaves` and a `reader` that is able to fetch all
    /// existing frozen nodes), and a list of leaves to be appended, returns
    /// the result root hash and new nodes to be frozen.
    pub fn append(
        reader: &R, num_existing_leaves: LeafCount, new_leaves: &[HashValue],
    ) -> Result<(HashValue, Vec<Node>)> {
        MerkleAccumulatorView::<R, H>::new(reader, num_existing_leaves)
            .append(new_leaves)
    }

    /// Get proof of inclusion of the leaf at `leaf_index` in this Merkle
    /// Accumulator of `num_leaves` leaves in total. Siblings are read via
    /// `reader` (or generated dynamically if they are non-frozen).
    ///
    /// See [`diem_types::proof::AccumulatorProof`] for proof format.
    pub fn get_proof(
        reader: &R, num_leaves: LeafCount, leaf_index: u64,
    ) -> Result<AccumulatorProof<H>> {
        MerkleAccumulatorView::<R, H>::new(reader, num_leaves)
            .get_proof(leaf_index)
    }

    /// Gets a proof that shows the full accumulator is consistent with a
    /// smaller accumulator.
    ///
    /// See [`diem_types::proof::AccumulatorConsistencyProof`] for proof format.
    pub fn get_consistency_proof(
        reader: &R, full_acc_leaves: LeafCount, sub_acc_leaves: LeafCount,
    ...
    /// See `get_range_proof`. This is the version of it that returns
    /// `Position`s only.
...
/// Actual implementation of Merkle Accumulator algorithms, which carries the
/// `reader` and `num_leaves` on an instance for convenience
    ...
    H: CryptoHasher,
{
    ...
    fn append(
        ...
        if new_leaves.is_empty() {
            if self.num_leaves == 0 {
                ...
                let root_hash = self.get_hash(
                    Position::root_from_leaf_count(self.num_leaves),
                )?;
        ...
        let root_level =
            Position::root_level_from_leaf_count(last_new_leaf_count);
        let mut to_freeze =
            Vec::with_capacity(Self::max_to_freeze(num_new_leaves, root_level));

        // Iterate over the new leaves, adding them to to_freeze and then adding
        // any frozen parents when right children are encountered.  This
        // has the effect of creating frozen nodes in
        // perfect post-order, which can be used as a strictly increasing append
        // only index for the underlying storage.
        //
        // We will track newly created left siblings while iterating so we can
        // pair them with their right sibling, if and when it becomes
        // frozen.  If the frozen left sibling is not created
        ...
        for (leaf_offset, leaf) in new_leaves.iter().enumerate() {
            let leaf_pos = Position::from_leaf_index(
                self.num_leaves + leaf_offset as LeafCount,
            );
            let mut hash = *leaf;
            ...
            // The node remaining must be a left child, possibly the root of a
            // complete binary tree.
            left_siblings.push((pos, hash));
        }

        // Now reconstruct the final root hash by walking up to root level and
        // adding placeholder hash nodes as needed on the right, and
        // left siblings that have either been newly created or read
        // from storage.
        let (mut pos, mut hash) =
            left_siblings.pop().expect("Must have at least one node");
    ...
    ///     new leaves and resulting frozen internal nodes forming a complete
    /// binary subtree         num_new_leaves * 2 - 1 < num_new_leaves * 2
    ///     and the full route from root of that subtree to the accumulator root
    /// turns frozen         height - (log2(num_new_leaves) + 1) < height -
    /// 1 = root_level
    fn max_to_freeze(num_new_leaves: usize, root_level: u32) -> usize {
        ...
        precondition!(
            num_new_leaves * 2 <= usize::max_value() - root_level as usize
        );
    ...
    fn rightmost_leaf_index(&self) -> u64 { (self.num_leaves - 1) as u64 }
    ...
    /// `MerkleAccumulator::get_consistency_proof`.
    fn get_consistency_proof(
        &self, sub_acc_leaves: LeafCount,
    ) -> Result<AccumulatorConsistencyProof> {
        ensure!(
        ...
        let subtrees =
            FrozenSubtreeSiblingIterator::new(sub_acc_leaves, self.num_leaves)
    ...
    /// Implementation for public interface
    /// `MerkleAccumulator::get_range_proof`.
    fn get_range_proof(
        &self, first_leaf_index: Option<u64>, num_leaves: LeafCount,
    ) -> Result<AccumulatorRangeProof<H>> {
    ...
    fn get_range_proof_positions(
        ...
        let left_siblings =
            self.get_sibling_positions(first_leaf_index, |p| p.is_left_child());
        let right_siblings =
            self.get_sibling_positions(last_leaf_index, |p| p.is_right_child());
        Ok((left_siblings, right_siblings))
    }

    fn get_siblings(
        &self, leaf_index: u64, filter: impl Fn(Position) -> bool,
    ) -> Result<Vec<HashValue>> {
        self.get_hashes(&self.get_sibling_positions(leaf_index, filter))
    }

    /// Helper function to get siblings on the path from the given leaf to the
    /// root. An additional filter function can be applied to filter out
    /// certain siblings.
    ...
    /// Implementation for public interface
    /// `MerkleAccumulator::get_frozen_subtree_hashes`.
