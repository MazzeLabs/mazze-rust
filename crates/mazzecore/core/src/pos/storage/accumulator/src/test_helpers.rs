...
use diem_crypto::hash::{
    HashValue, TestOnlyHasher, ACCUMULATOR_PLACEHOLDER_HASH,
};
...
pub(crate) type TestAccumulator =
    MerkleAccumulator<MockHashStore, TestOnlyHasher>;
...
impl MockHashStore {
    ...
    fn verify_in_store_if_frozen(
        &self, position: u64, hash: HashValue, frozen: bool,
    ) -> Result<()> {
        if frozen {
            ensure!(
                hash == *self
    ...
    fn hash_internal(left: HashValue, right: HashValue) -> HashValue {
        if left == *ACCUMULATOR_PLACEHOLDER_HASH
            && right == *ACCUMULATOR_PLACEHOLDER_HASH
        {
    ...
    fn verify_subtree(
        &self, leaves: &[HashValue], min_position: u64,
    ) -> Result<Subtree> {
        ...
        let me = if leaves.len() == 1 {
            ...
            let left =
                self.verify_subtree(&leaves[..subtree_width], min_position)?;
            let root_position = left.max_position + 1;
            let right = self
                .verify_subtree(&leaves[subtree_width..], root_position + 1)?;

            let max_position = right.max_position;
            let hash = Self::hash_internal(left.hash, right.hash);
            let frozen = left.frozen && right.frozen;
            let num_frozen_nodes = left.num_frozen_nodes
                + right.num_frozen_nodes
                + frozen as LeafCount;
...
pub(crate) fn verify(
    ...
    leaves.iter().enumerate().for_each(|(i, hash)| {
        ...
        let proof =
            TestAccumulator::get_proof(store, num_leaves, leaf_index).unwrap();
...
pub fn test_proof_impl((batch1, batch2): (Vec<HashValue>, Vec<HashValue>)) {
    ...
    let (root_hash1, writes1) =
        TestAccumulator::append(&store, 0, &batch1).unwrap();
    store.put_many(&writes1);
    let (root_hash2, writes2) =
        TestAccumulator::append(&store, batch1.len() as LeafCount, &batch2)
            .unwrap();
...
pub fn test_consistency_proof_impl(
    (batch1, batch2): (Vec<HashValue>, Vec<HashValue>),
) {
    let mut store = MockHashStore::new();
    ...
    let (root_hash1, writes1) =
        TestAccumulator::append(&store, 0, &batch1).unwrap();
    store.put_many(&writes1);
    let proof1 = TestAccumulator::get_consistency_proof(
        &store,
...
pub fn test_range_proof_impl(
    ...
    let (root_hash, writes) =
        TestAccumulator::append(&store, 0, &all_hashes).unwrap();
...
pub fn test_get_frozen_subtree_hashes_impl(leaves: Vec<HashValue>) {
    let mut store = MockHashStore::new();
    let (root_hash, writes) =
        TestAccumulator::append(&store, 0, &leaves).unwrap();
    store.put_many(&writes);

    let frozen_subtree_hashes = TestAccumulator::get_frozen_subtree_hashes(
        &store,
        leaves.len() as LeafCount,
    )
    .unwrap();
    let in_mem_acc = InMemoryAccumulator::new(
...
pub fn test_append_empty_impl(leaves: Vec<HashValue>) {
    ...
    let (root_hash2, writes2) =
        TestAccumulator::append(&store, leaves.len() as LeafCount, &[])
            .unwrap();
