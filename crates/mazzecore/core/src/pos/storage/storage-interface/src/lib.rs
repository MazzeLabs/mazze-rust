...
pub trait DbReader: Send + Sync {
    ...
    // Gets an account state by account address, out of the ledger state
    // indicated by the state Merkle tree root with a sparse merkle proof
    // proving state tree root. See [`DiemDB::
