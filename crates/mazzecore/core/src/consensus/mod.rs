...
use mazze_executor::{executive::ExecutionOutcome, state::State};
...
use mazze_types::{AddressWithSpace, AllChainID, Bloom, Space, H256, U256};
use either::Either;
...
///
/// ConsensusGraph maintains the DAG-Embedded Tree Structure (DETS) structure of the client and
/// implements *Timer Chain WLSR*/*Mazze* algorithm to determine the block
...
impl ConsensusGraph {
    ...
    pub fn choose_correct_parent(
        ...
        if correct_parent_hash != *parent_hash {
            ...
            // correct_parent may not be on the main chain, so recompute
            ...
            for hash in hashes {
                ...
                for tx in block.transactions.iter() {
                    if space == Space::Native && tx.space() != Space::Native {
                        // For mazze_gasPrice, we only count Native transactions.
                        continue;
                    }
    ...
    pub fn get_block_execution_info(
        ...
        let main_hash = results_with_epoch.0;

        let maybe_state_root = match self.executor.wait_for_result(main_hash) {
    ...
    pub fn get_block_epoch_number_with_main_check(
        &self, hash: &H256, require_main: bool,
    ) -> RpcResult<u64> {
        let inner = &*self.inner.read();
        ...
        if require_main {
            if let Err(..) =
                inner.check_block_main_assumption(&hash, epoch_number)
            {
                bail!(invalid_params(
                    "epoch parameter",
                    format!(
                        "should receive a main block hash, receives: {:?}",
                        hash
                    ),
                ))
            }
    ...
    pub fn next_nonce(
        ...
        let epoch_number = match block_hash_or_epoch_number {
            BlockHashOrEpochNumber::BlockHashWithOption {
                ...
                self.get_block_epoch_number_with_main_check(
                    &hash,
    ...
    fn filter_block<'a>(
        ...
        epoch: u64, main_hash: H256, block_hash: H256,
    ...
    fn filter_single_epoch<'a>(
        ...
        // retrieve epoch hashes and main hash
        let mut epoch_hashes =
            self.inner.read_recursive().block_hashes_by_epoch(epoch)?;

        let main_hash = *epoch_hashes.last().expect("Epoch set not empty");
    ...
    fn filter_logs_by_epochs(
        ...
        // we store the last epoch processed and the corresponding main hash so
    ...
    // collect epoch number, block index in epoch, block hash, main hash
    fn collect_block_info(
        ...
        // find epoch number
        let epoch = match self.data_man.block_header_by_hash(&main_hash) {
            Some(h) => h.height(),
            None => {
                // internal error
                error!("Header of main block {:?} not found", main_hash);
                bail!(FilterError::UnknownBlock { hash: main_hash });
            }
        };
        ...
        Ok((epoch, index_in_epoch, block_hash, main_hash))
    }
    ...
    fn filter_logs_by_block_hashes(
        ...
        let mut logs = block_infos
            .into_iter()
            .map(|(epoch, _, block_hash, main_hash)| {
                self.filter_block(
                    &filter,
                    &bloom_possibilities,
                    epoch,
    ...
    pub fn call_virtual(
        ...
        {
            (v.last().expect("main block always exist").clone(), v.len())
        } else {
    ...
    fn filter_traces_by_epochs(
        ...
        let epochs_and_main_hash = {
            let inner = self.inner.read();
            let mut epochs_and_main_hash = Vec::new();
            for epoch_number in self.get_trace_filter_epoch_range(filter)? {
                epochs_and_main_hash.push((
                    epoch_number,
                    inner.get_main_hash_from_epoch_number(epoch_number)?,
                ))
            }
            epochs_and_main_hash
        };

        let block_traces = epochs_and_main_hash
            .into_par_iter()
            .map(|(epoch_number, assumed_main)| {
    ...
    /// Return `Vec<(main_hash, block_hash, block_traces, block_txs)>`
    pub fn collect_traces_single_epoch(
        &self, filter: &TraceFilter, epoch_number: u64, assumed_main: H256,
    ) -> Result<
    ...
    > {
        ...
        if block_hashes.last().expect("epoch set not empty") != &assumed_main {
            bail!(FilterError::MainChainReorg {
                epoch: epoch_number,
                from: assumed_main,
        ...
        let block_traces = block_hashes
            ...
            .map(|h| {
                ...
                self.data_man
                    ...
                    .map(|DataVersionTuple(main_hash, trace)| {
                        (main_hash, h, trace, block.transactions.clone())
                    })
    ...
    /// `block_traces` is a list of tuple `(main_hash, block_hash,
    /// block_trace)`.
    pub fn filter_block_traces(
        ...
        let mut traces = Vec::new();
        for (main_hash, block_hash, block_trace, block_txs) in block_traces {
            ...
            let epoch_number = self
                .data_man
                .block_height_by_hash(&main_hash)
                .ok_or_else(|| {
                    FilterError::Custom(
                        format!(
                            "main block header missing, hash={:?}",
                            main_hash
                        )
                        .into(),
                    )
    ...
    pub fn get_phantom_block_main_by_number(
        &self, block_num: EpochNumber, main_assumption: Option<H256>,
        include_traces: bool,
    ...
    fn get_phantom_block_by_number_inner(
        &self, block_num: EpochNumber, main_assumption: Option<H256>,
        include_traces: bool, only_main: bool,
    ) -> Result<Option<PhantomBlock>, String> {
        let hashes = self.get_block_hashes_by_epoch(block_num)?;
        ...
        if hashes.last() == Some(&genesis.hash()) {
            return Ok(Some(PhantomBlock {
                main_header: genesis.block_header.clone(),
                transactions: vec![],
                receipts: vec![],
                errors: vec![],
                bloom: Bloom::zero(),
        ...
        if matches!(main_assumption, Some(h) if h != main.hash()) {
            return Ok(None);
        }

        let mut phantom_block = PhantomBlock {
            main_header: main.block_header.clone(),
            transactions: vec![],
            receipts: vec![],
            errors: vec![],
            ...
            let block_traces = if include_traces {
                    ...
                    .transactions_traces_by_block_hash(&b.hash())
                {
                    ...
                    Some((main_hash, block_traces)) => {
                        ...
                        // sanity check: no main reorg during processing
                        if main_hash != main.hash() {
                            return Err(
                                "Inconsistent state: main hash mismatch"
                                    .into(),
                            );
                        }
    ...
    fn get_state_db_by_epoch_number_with_space(
        ...
        let hash =
            self.inner.read().get_main_hash_from_epoch_number(height)?;
        Ok(StateDb::new(
...
impl ConsensusGraphTrait for ConsensusGraph {
    ...
    fn construct_main_state(&self) {
        ...
        self.new_block_handler
            .construct_main_state(inner, &self.confirmation_meter);
    ...
    /// If the process crashes and recovered, the blocks in the outlier of the
