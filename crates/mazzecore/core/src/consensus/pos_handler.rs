...
use diem_types::{
    ...
    block_info::{MainBlockDecision, Round},
    chain_id::ChainId,
...
impl PosHandler {
    ...
    pub fn get_main_decision(&self, h: &PosBlockId) -> Option<H256> {
        // Return None if `pos` has not been initialized
        self.pos
            .get()?
            .get_committed_block(h)
            .map(|b| b.main_decision)
    }
...
impl PosHandler {
    ...
    pub fn force_sign_main_decision(
        &self, main_decision: MainBlockDecision,
    ) -> anyhow::Result<()> {
        self.test_command_sender
            .lock()
            .as_mut()
            .ok_or(anyhow::anyhow!("Pos not initialized!"))?
            .try_send(TestCommand::BroadcastMainDecision(main_decision))
            .map_err(|e| anyhow::anyhow!("try_send: err={:?}", e))
    }
...
impl PosInterface for PosConnection {
    ...
    fn get_committed_block(&self, h: &PosBlockId) -> Option<PosBlock> {
        ...
        Some(PosBlock {
            ...
            main_decision: committed_block.main_decision.block_hash,
