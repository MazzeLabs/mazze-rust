...
use dag::{
    get_future, topological_sort, Graph, RichDAG, RichDETS, DETS, DAG,
};
...
pub struct ConsensusInnerConfig {
    /// Beta is the threshold in WLSR algorithm
    pub adaptive_weight_beta: u64,
    /// The heavy block ratio (h) in WLSR algorithm
...
/// change as the consensus graph state evolves (e.g., main chain changes).
...
pub struct ConsensusGraphNodeData {
    ...
    /// corresponding main chain block of this one
    ...
    /// are not in the same era of the main block.
    ...
    /// correct or not.
    vote_valid: bool,
    /// It denotes the height of the last main chain in the past set of this
    /// block.
    last_main_in_past: u64,
...
#[derive(DeriveMallocSizeOf)]
struct ConsensusGraphMainData {
    /// The set of blocks whose last_main_in_past point to this main chain
    /// location
    last_main_in_past_blocks: HashSet<usize>,
    /// The total weight of the past set of the main block. This value
    /// is used by the confirmation meter.
    past_weight: i128,
}

impl Default for ConsensusGraphMainData {
    fn default() -> Self {
        ConsensusGraphMainData {
            last_main_in_past_blocks: HashSet::new(),
            past_weight: 0,
        }
    }
}
...
///
/// Era in Mazze is defined based on the height of a block. Every
...
/// definition of Era enables Mazze to form checkpoints at the stabilized
/// era genesis blocks.
///
...
/// subtree of b). Force-confirm rule overrides any WLSR weight rule, i.e.,
/// new blocks will always be generated under b.
///
///
/// # Implementation details of the WLSR algorithm
///
/// Mazze uses the Weighted Ledger Selection Rule (WLSR) algorithm to
/// select a chain from the genesis block to one of the leaf blocks as the main
/// chain. For each block b, WLSR algorithm computes it is adaptive
///
/// ```python
/// B = Past(b)
...
///
/// Blaming is used to provide proof for state root of a specific main block.
/// The rationale behind is as follows. Verifying state roots of blocks off
/// main chain is very costly and sometimes impractical, e.g., when the block
/// refers to another block that is not in the current era. It is preferred to
/// avoid this verification if possible. Normally, Mazze only needs to store
/// correct state root in header of main block to provide proof for light node.
/// However, the main chain may oscillate at the place close to ledger tail,
/// which means that a block that is off main at some point may become main
/// block in the future. If we do not verify the state root in the header of
/// that block, when it becomes a main block later, we cannot guarantee the
/// correctness of the state root in its header. Therefore, if we do not verify
/// the state root in off-main block, we cannot guarantee the correctness of
/// state root in main block. Of course, one may argue that you can switch
/// main chain when incorrect state root in main block is observed. However,
/// this makes the check for the correct parent selection rely on state root
/// checking. Then, since Mazze is an inclusive protocol which adopts
/// off-main blocks in its final ledger, it needs to verify the correctness of
/// parent selection of off-main blocks and this relies on the state
/// verification on all the parent candidates of the off-main blocks.
/// Therefore, this eventually will lead to state root verification on any
/// blocks including off-main ones. This violates the original goal of saving
/// cost of the state root verification in off-main blocks.
///
/// We therefore allow incorrect state root in main block header, and use the
...
/// information stored in the header of each main block. It blames the blocks
...
/// main chain and the newly mined block. For example, consider the blocks
...
/// the block. If the block is a main block when computing its reward,
/// it is required that:
///
...
///    edges.
///
/// If the block is an off-main block when computing its reward,
/// it is required that:
/// 1. the block correctly chooses its parent;
/// 2. the block correctly blames the blocks in the intersection of main chain
///    blocks and all its previous blocks following parent edges. (This is to
///    encourage the node generating the off-main block to keep verifying main
///    chain blocks.)
///
...
/// in header is correct, starting from block Bi+2 along with the main
...
/// block is block Bj.
///
/// 4. The verifier then searches backward along the main chain from Bj for
...
pub struct ConsensusGraphInner {
    ...
    /// The current main chain indexes.
    main_chain: Vec<usize>,
    /// The metadata associated with each main chain block
    main_chain_metadata: Vec<ConsensusGraphMainData>,
    ...
    /// The set of *graph* tips in the DAG-Embedded Tree Structure (DETS) for mining.
    ...
    /// initial stage it may not be always on the main chain.
    ...
    /// weight_tree maintains the subtree weight of each node in the DAG-Embedded Tree Structure (DETS)
    ...
    /// The cache to store Outlier information of each node. This could be
    ...
    /// This is to record the main chain reorganization height since the last
    ...
    /// block in its outlier.
    has_timer_block_in_outlier_cache: HashSet<usize>,
...
impl MallocSizeOf for ConsensusGraphInner {
    fn size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        self.arena.size_of(ops)
            ...
            + self.main_chain.size_of(ops)
            + self.main_chain_metadata.size_of(ops)
            ...
            + self.outlier_cache.size_of(ops)
...
pub struct ConsensusGraphNode {
    ...
    /// consensus graph state evolves (e.g., main chain changes). Unlike the
...
impl ConsensusGraphInner {
    pub fn with_era_genesis(
        ...
        let mut inner = ConsensusGraphInner {
            ...
            has_timer_block_in_outlier_cache: Default::default(),
            header_only: true,
        };
        ...
        inner.arena[inner.cur_era_genesis_block_arena_index]
            .data
            .last_main_in_past = cur_era_genesis_height;
        inner
            .main_chain
            .push(inner.cur_era_genesis_block_arena_index);
        let mut last_main_in_past_blocks = HashSet::new();
        last_main_in_past_blocks
            .insert(inner.cur_era_genesis_block_arena_index);
        inner.main_chain_metadata.push(ConsensusGraphMainData {
            last_main_in_past_blocks,
    ...
    fn persist_epoch_set_hashes(&mut self, main_index: usize) {
        let height = self.main_index_to_height(main_index);
        let arena_index = self.main_chain[main_index];
    ...
    /// `self.main_chain` range. Otherwise the function may panic.
    pub fn get_main_block_arena_index(&self, height: u64) -> usize {
        let main_index = (height - self.cur_era_genesis_height) as usize;
        assert!(main_index < self.main_chain.len());
        self.main_chain[main_index]
    }

    #[inline]
    pub fn get_main_height(&self) -> u64 {
        self.cur_era_genesis_height + self.main_chain.len() as u64
    }

    #[inline]
    pub fn height_to_main_index(&self, height: u64) -> usize {
        (height - self.cur_era_genesis_height) as usize
    }

    #[inline]
    pub fn main_index_to_height(&self, main_index: usize) -> u64 {
        self.cur_era_genesis_height + main_index as u64
    }
    ...
    fn ancestor_at(&self, me: usize, height: u64) -> usize {
        let height_index = self.height_to_main_index(height);
    ...
    /// This function computes the epoch set of block *main* based on
    /// the past set of block *lca* which is the parental ancestor of
    /// *main*. The algorithm processes the blocks along the parental
    /// path from *lca* to *main* to produce the *visited* block set,
    /// and then compute the epoch block set of *main* based on the
    /// *pastset* and the *visited*. The following figure illustrates
    /// this process. B[lca] refers to the block *lca*, B[piv] refers
    /// to the block *main*, and B[par] refers to the parent of *main*
    ///
    /// I    ------------\-------------------\-------------------\
    /// I        lca      \                   \       epoch       \
    ...
    /// block *main*. It also computes the ordered set of executable
    ...
    /// same era with *main*. It then topologically sorts the retained
    ...
    fn compute_blockset_in_own_view_of_epoch(&mut self, main: usize) {
        if !self.arena[main].data.blockset_cleared {
            return;
        }
        // TODO: consider the speed for recovery from db
        let parent = self.arena[main].parent;
        if parent != NULL {
            let last = *self.main_chain.last().unwrap();
        ...
        for idx in &self.arena[main].data.blockset_in_own_view_of_epoch {
            if self.is_same_era(*idx, main) {
        ...
        ordered_executable_epoch_blocks.push(main);
        ...
        self.arena[main].data.skipped_epoch_blocks =
            skipped_epoch_block_indices
                .into_iter()
                .map(|i| self.arena[i].hash)
                .collect();
        self.arena[main].data.ordered_executable_epoch_blocks =
            ordered_executable_epoch_blocks;
        self.arena[main].data.blockset_cleared = false;
    }
    ...
    fn get_blame_with_main_index(&self, main_index: usize) -> u32 {
        let arena_index = self.main_chain[main_index];
        self.get_blame(arena_index)
    }

    pub fn find_first_index_with_correct_state_of(
        &self, main_index: usize, blame_bound: Option<u32>,
        min_vote_count: usize,
    ) -> Option<usize> {
        // this is the earliest block we need to consider; blocks before `from`
        // cannot have any information about the state root of `main_index`
        let from = main_index + DEFERRED_STATE_EPOCH_COUNT as usize;
    ...
    pub fn find_first_trusted_starting_from(
        ...
        while trusted_index != from {
            let blame = self.get_blame_with_main_index(trusted_index);
    ...
    fn find_first_with_trusted_blame_starting_from(
        ...
        trace!(
            "find_first_with_trusted_blame_starting_from main_index={:?}",
            main_index
        );
        let mut cur_main_index = main_index;
        while cur_main_index < self.main_chain.len() {
            let arena_index = self.main_chain[cur_main_index];
    ...
    /// I                      a me a
    ///
    /// *a* refers to the elements in outlier barrier who are
    /// outlier of *me* while whose parents are in the past set
    /// of *me*. The subtree to process does not include *a* and
    /// *me*.
    fn compute_subtree_weights(
        &self, me: usize, outlier_barrier: &BitSet,
    ) -> Vec<i128> {
        ...
        while let Some((stage, index)) = stack.pop() {
            if stage == 0 {
                ...
                for child in &self.arena[index].children {
                    if !outlier_barrier.contains(*child as u32) && *child != me
                    {
                        stack.push((0, *child));
                    }
                }
    ...
    fn adaptive_weight_impl(
        ...
        for index in outlier_barrier.iter() {
    ...
    fn adaptive_weight(
        &mut self, me: usize, outlier_barrier: &BitSet,
        weight_tuple: Option<&Vec<i128>>,
        timer_chain_tuple: &(u64, HashMap<usize, u64>, Vec<usize>, Vec<usize>),
    ) -> bool {
    ...
    #[inline]
    fn is_same_era(&self, me: usize, main: usize) -> bool {
        self.arena[me].era_block == self.arena[main].era_block
    }
    ...
    /// All referees should be in the outlier of each other.
    ...
    fn compute_force_confirm(
        &self, timer_chain_choice: usize, pos_main_decision: &(H256, u64),
    ) -> usize {
        if let Some(arena_index) =
            self.hash_to_arena_indices.get(&pos_main_decision.0)
        {
            if self.arena[timer_chain_choice].height > pos_main_decision.1
            ...
            // If pos_main_decision is before checkpoint, we just think it's on
            // the main chain.
            timer_chain_choice
        }
    }
    ...
    /// Return the consensus graph indexes of the main block where the rewards
    ...
    ///     min(OUTLIER_PENALTY_UPPER_EPOCH_COUNT, D).
    ...
    fn adjust_difficulty(&mut self, new_best_arena_index: usize) {
        ...
        let old_best_arena_index = *self.main_chain.last().expect("not empty");
    ...
    pub fn best_block_hash(&self) -> H256 {
        self.arena[*self.main_chain.last().unwrap()].hash
    }

    pub fn best_block_number(&self) -> u64 {
        self.arena[*self.main_chain.last().unwrap()].past_num_blocks
    }
    ...
    pub fn best_state_epoch_number(&self) -> u64 {
        let main_height = self.main_index_to_height(self.main_chain.len());
        if main_height < DEFERRED_STATE_EPOCH_COUNT {
            0
        } else {
            main_height - DEFERRED_STATE_EPOCH_COUNT
        }
    }

    fn best_state_arena_index(&self) -> usize {
        self.get_main_block_arena_index(self.best_state_epoch_number())
    }
    ...
    pub fn best_epoch_number(&self) -> u64 {
        self.cur_era_genesis_height + self.main_chain.len() as u64 - 1
    }
    ...
    fn get_arena_index_from_epoch_number(
        ...
        if epoch_number >= self.cur_era_genesis_height {
            ...
            if main_index >= self.main_chain.len() {
                Err("Epoch number larger than the current main chain tip"
                    .into())
            } else {
                Ok(self.get_main_block_arena_index(epoch_number))
            }
        } else {
    ...
    /// Get the main hash from an epoch number. This function will try to query
    /// the data manager if it is not available in the ConsensusGraph due to
    /// out of the current era.
    pub fn get_main_hash_from_epoch_number(
        &self, epoch_number: u64,
        ...
        if height >= self.cur_era_genesis_height {
            ...
            if main_index >= self.main_chain.len() {
                ...
                Ok(self.arena[self.get_main_block_arena_index(height)].hash)
            }
        } else {
    ...
    /// This function differs from `get_main_hash_from_epoch_number` in that it
    /// only returns the hash if it is in the current consensus graph.
    pub fn epoch_hash(&self, epoch_number: u64) -> Option<H256> {
        let main_index = self.height_to_main_index(epoch_number);
        self.main_chain
            .get(main_index)
            .map(|idx| self.arena[*idx].hash)
    }
    ...
    pub fn block_hashes_by_epoch(
        ...
        debug!(
            "block_hashes_by_epoch epoch_number={:?} main_chain.len={:?}",
            epoch_number,
            self.main_chain.len()
        );
        ...
        match self.get_arena_index_from_epoch_number(epoch_number) {
            Ok(main_arena_index) => {
                if main_arena_index != self.cur_era_genesis_block_arena_index {
                    return Ok(self
                        .get_ordered_executable_epoch_blocks(main_arena_index)
                        .iter()
    ...
    pub fn skipped_block_hashes_by_epoch(
        ...
        debug!(
            "skipped_block_hashes_by_epoch epoch_number={:?} main_chain.len={:?}",
            epoch_number,
            self.main_chain.len()
        );
            ...
            Ok(main_arena_index) => {
                if main_arena_index != self.cur_era_genesis_block_arena_index {
                    if let Some(skipped_block_set) =
                        self.get_skipped_epoch_blocks(main_arena_index)
                    {
    ...
    pub fn bounded_terminal_block_hashes(
        ...
        let best_block_arena_index = *self.main_chain.last().unwrap();
    ...
    /// Return the block receipts in the current main view and the epoch block
    ...
    pub fn block_execution_results_by_hash(
        ...
        match self.get_epoch_hash_for_block(hash) {
            ...
            None => {
                ...
                let execution_main_hash = res.0;
                let epoch_number = self
                    .data_man
                    .block_header_by_hash(&execution_main_hash)?
                    .height();

                match self.get_main_hash_from_epoch_number(epoch_number) {
                    // main chain has not changed, result should be correct
                    Ok(h) if h == execution_main_hash => Some(res),

                    // main chain has changed, block is not re-executed yet
                    _ => None,
                }
            }
        }
    ...
    pub fn check_block_main_assumption(
        &self, main_hash: &H256, epoch: u64,
    ) -> Result<(), String> {
        let last_number = self.best_epoch_number();
        let hash = self.get_main_hash_from_epoch_number(epoch)?;
        if epoch > last_number || hash != *main_hash {
            return Err("Error: main chain assumption failed".to_owned());
        }
        Ok(())
    }

    /// Compute the block weight following the WLSR algorithm:
    ...
    ///
    /// Assumption:
    ///   * [Bm] is a main block on current main chain.
    ...
    ///      make state_valid of all the main blocks before the new
    ...
    fn compute_blame_and_state_with_execution_result(
        ...
        loop {
            ...
            // Note that this function should never return errors for main
            ...
            if self.arena[cur].height == self.cur_era_genesis_height {
                // Note that this should never happen for main chain blocks,
    ...
    fn recompute_metadata(
        ...
        self.main_chain_metadata
            .resize_with(self.main_chain.len(), Default::default);
        let main_height = self.get_main_height();
        for i in start_at..main_height {
            let me = self.get_main_block_arena_index(i);
            self.arena[me].data.last_main_in_past = i;
            let i_main_index = self.height_to_main_index(i);
            self.main_chain_metadata[i_main_index]
                .last_main_in_past_blocks
                .clear();
            self.main_chain_metadata[i_main_index]
                .last_main_in_past_blocks
                .insert(me);
            self.main_chain_metadata[i_main_index].past_weight =
                if i_main_index > 0 {
                    let blockset = self
                        .exchange_or_compute_blockset_in_own_view_of_epoch(
                            me, None,
        ...
        while !stack.is_empty() {
            ...
            {
                let mut last_main = if parent == NULL {
                    0
                } else {
                    self.arena[parent].data.last_main_in_past
                };
                for referee in &self.arena[me].referees {
                    let x = self.arena[*referee].data.last_main_in_past;
                    last_main = max(last_main, x);
                }
                self.arena[me].data.last_main_in_past = last_main;
                let last_main_index = self.height_to_main_index(last_main);
                self.main_chain_metadata[last_main_index]
                    .last_main_in_past_blocks
                    .insert(me);
            }
        }
    ...
    fn get_timer_chain_index(&self, me: usize) -> usize {
        ...
        {
            // This is only possible if `me` is in the outlier of
            // `cur_era_genesis`.
            return NULL;
        }
    ...
    pub fn get_trusted_blame_block(
        ...
        let main_index =
            self.height_to_main_index(self.arena[arena_index].height);
        // the given checkpoint hash is invalid
        if main_index >= self.main_chain.len()
            || self.main_chain[main_index] != arena_index
        {
            debug!(
                "get_trusted_blame_block: block {:?} not on main chain",
                checkpoint_hash
            );
            return None;
        }
        self.find_first_index_with_correct_state_of(
            main_index + plus_depth,
            None, /* blame_bound */
            0,    /* min_vote_count */
        )
        .and_then(|index| Some(self.arena[self.main_chain[index]].hash))
    }
    ...
    pub fn reset_epoch_number_in_epoch(&mut self, main_arena_index: usize) {
        self.set_epoch_number_in_epoch(main_arena_index, NULLU64);
    }

    fn set_epoch_number_in_epoch(
        &mut self, main_arena_index: usize, epoch_number: u64,
    ) {
        assert!(!self.arena[main_arena_index].data.blockset_cleared);
        ...
        self.arena[main_arena_index].data.epoch_number = epoch_number;
    }
    ...
    fn get_deferred_state_arena_index(
        ...
        let main_idx = self.height_to_main_index(height);
        // If it is on the main chain already, we can avoid O(log n) lca query
        if main_idx < self.main_chain.len()
            && self.main_chain[main_idx] == me
        {
            return Ok(
                self.main_chain[self.height_to_main_index(target_height)]
            );
        } else {
    ...
    /// Find the first state valid block on the main chain after
    ...
    pub fn recover_state_valid(&mut self) {
        ...
        if start_main_index >= self.main_chain.len() {
            // TODO: Handle this after refactoring
            // `state_availability_boundary`.
            return;
        }
        let start_epoch_hash =
            self.arena[self.main_chain[start_main_index]].hash;
        // We will get the first
        // main block whose `state_valid` is `true` after `start_epoch_hash`
    ...
    pub fn get_main_chain_and_weight(
        ...
        for i in start..=end {
            let main_arena_index = self.get_main_block_arena_index(i);
            chain.push((
                self.arena[main_arena_index].hash.into(),
                (self.weight_tree.get(main_arena_index) as u64).into(),
            ));
        }
        Ok(chain)
    }
    ...
    pub fn validate_main_decision(
        ...
        debug!(
            "validate_main_decision: ancestor={:?}, me={:?}",
        ...
        match (
            ...
            (_, Some(_me)) => {
                ...
                if !self.header_only {
                    warn!(
                        ...
                        self.main_block_processed(ancestor_hash)
                    );
                }
                // ancestor is before checkpoint and is on main chain, so me
            ...
            (_, _) => {
                // This should not happen after catching up for a normal node.
                if !self.header_only {
                    warn!("ancestor and me are both not in consensus graph, processed={} {}", self.main_block_processed(ancestor_hash), self.main_block_processed(me_hash));
                }
                true
            }
        }
    ...
    pub fn main_block_processed(&self, main_hash: &H256) -> bool {
        if let Some(height) = self.data_man.block_height_by_hash(main_hash) {
            if let Ok(epoch_hash) =
                self.get_main_hash_from_epoch_number(height)
            {
                if epoch_hash == *main_hash {
                    return true;
                } else {
                    debug!(
                        "main_block_processed: {:?} is not on main chain",
                        main_hash
                    );
                }
            } else {
                debug!("main_block_processed: epoch not processed height={:?} main={:?}", height, main_hash);
            }
        } else {
    ...
    /// Return if a block has been confirmed by the main decision by the latest
    /// committed PoS block.
    ///
    ...
    pub fn is_confirmed_by_pos(&self, block_hash: &H256) -> bool {
        ...
        epoch_number <= self.best_pos_main_decision.1
    }

    /// Return the latest PoS main decision processed in ConsensusGraph.
...
impl DETS for ConsensusGraphInner {
...
impl RichDETS for ConsensusGraphInner {
...
impl StateMaintenanceTrait for ConsensusGraphInner {
    fn get_main_hash_from_epoch_number(
        ...
        ConsensusGraphInner::get_main_hash_from_epoch_number(
            self,
            epoch_number,
        )
    }
