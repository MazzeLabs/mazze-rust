...
use mazze_parameters::consensus_internal::*;
...
impl ConfirmationMeter {
    ...
    pub fn confirmation_risk_by_hash(
        ...
        if epoch_num == NULLU64 {
            // The block is in the outlier of cur era genesis or its not
            // included in any epoch on the main chain yet.
    ...
    fn confirmation_risk(
        ...
        // Compute w_1
        let idx = g_inner.get_main_block_arena_index(epoch_num);
        let main_idx = g_inner.height_to_main_index(epoch_num);
        ...
        // Compute w_3
        let w_3 = g_inner.main_chain_metadata[main_idx].past_weight;
    ...
    pub fn update_confirmation_risks(&self, g_inner: &ConsensusGraphInner) {
        if g_inner.main_chain.len() > DEFERRED_STATE_EPOCH_COUNT as usize {
            let w_0 = g_inner
                .weight_tree
    ...
    /// used by Mazze to determine when we will remove old snapshots. If
    ...
    pub fn is_adaptive_possible(
        ...
        // Find the first main chain block whose timer diff is less than 140
        let mut cur_height = g_inner.cur_era_stable_height;
        let mut cur_arena_index =
            g_inner.get_main_block_arena_index(cur_height);
        ...
        {
            cur_height += 1;
            cur_arena_index = g_inner.get_main_block_arena_index(cur_height);
        }
        ...
        let total_weight = g_inner
            ...
            .get(g_inner.cur_era_genesis_block_arena_index);
        let me_index =
            g_inner.height_to_main_index(g_inner.arena[me].data.epoch_number);
        let x_3 =
            total_weight - g_inner.main_chain_metadata[me_index].past_weight;

        let mut adaptive_risk = 0f64;
        let d = i128::try_from(g_inner.current_difficulty.low_u128()).unwrap();
        for i in 0..n {
            let a_main_index = g_inner.height_to_main_index(
                g_inner.cur_era_stable_height + i * psi as u64,
            );
            let b_main_index = g_inner.height_to_main_index(
                g_inner.cur_era_stable_height + (i + 1) * psi as u64,
            );
            let b = g_inner.main_chain[b_main_index];
            let y = g_inner.weight_tree.get(b);
            let mut x_1 = 0;
            for v in a_main_index..b_main_index {
                let main = g_inner.main_chain[v];
                let next_main = g_inner.main_chain[v + 1];
                for child in &g_inner.arena[main].children {
                    if *child != next_main {
            ...
            let m_j = (total_weight
                - g_inner.main_chain_metadata[a_main_index].past_weight)
                / d;
