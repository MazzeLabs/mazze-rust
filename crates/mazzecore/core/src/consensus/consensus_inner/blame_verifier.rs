...
use mazze_parameters::{consensus::*, light::BLAME_CHECK_OFFSET};
...
impl BlameVerifier {
    ...
    fn header_from_height(
        ...
        self.data_man.block_header_by_hash(&main_hash)
    }
    ...
    fn first_trusted_header_starting_from(
        ...
        let main_index = match height {
            h if h < inner.get_cur_era_genesis_height() => return None,
            h => inner.height_to_main_index(h),
        };
        ...
        inner
            ...
            .map(|index| inner.main_index_to_height(index))
    }
    ...
    pub fn check(&mut self, inner: &ConsensusGraphInner, epoch: u64) -> bool {
        ...
        match epoch {
            ...
            // in most cases, we will iterate over the main chain sequentially;
        ...
        // convert epoch number into main height
        ...
        match self.first_trusted_header_starting_from(inner, height) {
            ...
            None => {
                ...
                //     `BLAME_CHECK_OFFSET` suffix of the main
                ...
                //  ---       ---       ---       ---
                //
                // if we have such a section of the main chain of length
                ...
                // later. we assume here that the main chain will eventually
            ...
            Some(w) if w == height => {
                ...
                // continue from the next header on the main chain
