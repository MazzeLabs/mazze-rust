...
use mazze_types::{Address, AddressSpaceUtil, Bloom, H256};
use primitives::{
...
/// NOTE: we use `height` when we talk about headers on the main chain
...
impl LedgerInfo {
    ...
    /// Get hash of block at `height` on the main chain, if it exists.
    #[inline]
    fn main_hash_of(&self, height: u64) -> Result<H256, Error> {
        let epoch = EpochNumber::Number(height);
        Ok(self.consensus.get_hash_from_epoch_number(epoch)?)
    }

    /// Get header at `height` on the main chain, if it exists.
    #[inline]
    pub fn main_header_of(&self, height: u64) -> Result<BlockHeader, Error> {
        let main = self.main_hash_of(height)?;
        self.header(main)
    }

    /// Get all block hashes corresponding to the main block at `height`.
    ...
    /// main chain based on local execution information.
    #[inline]
    fn correct_deferred_state_root_hash_of(
        &self, height: u64,
    ) -> Result<H256, Error> {
        let epoch = height.saturating_sub(DEFERRED_STATE_EPOCH_COUNT);
        let main = self.main_hash_of(epoch)?;

        let commitments = self
            .consensus
            .get_data_manager()
            .get_epoch_execution_commitment_with_db(&main)
            .ok_or_else(|| {
    ...
    /// main chain stores the correct roots based on the blame information.
    /// NOTE: This list will contains `witness` in all cases.
    #[inline]
    pub fn headers_seen_by_witness(
        &self, witness: u64,
    ) -> Result<Vec<u64>, Error> {
        let blame = self.main_header_of(witness)?.blame() as u64;
