...
/// It might change depending on this block is executed under which main
/// block's view.
#[derive(Clone, Debug)]
...
impl BlockExecutionResultWithEpoch {
    pub fn new(main_hash: H256, receipts: BlockExecutionResult) -> Self {
        DataVersionTuple(main_hash, receipts)
    }
}

impl BlockTracesWithEpoch {
    pub fn new(main_hash: H256, traces: BlockExecTraces) -> Self {
        DataVersionTuple(main_hash, traces)
    }
}

/// The structure to maintain block data under different views.
///
/// Note that in database only the data corresponding to the current main
...
pub struct BlockDataWithMultiVersion<Version, T> {
    ...
    // The current main epoch that this block is executed.
