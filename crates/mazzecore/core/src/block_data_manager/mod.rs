...
pub struct BlockDataManager {
    ...
    /// the main block that produces the result when executed.
    ...
    ///    main blocks around snapshot block based on blaming information;
...
impl BlockDataManager {
    ...
    /// Get the traces for a single block without checking the assumed main
    /// block
    ...
    pub fn insert_block_traces(
        &self, hash: H256, trace: BlockExecTraces, main_hash: H256,
        persistent: bool,
    ) {
        trace! {"insert_block_traces start main={:?}", main_hash};
        self.insert_version(
            hash,
            &main_hash,
            trace,
        ...
        trace! {"insert_block_traces ends main={:?}", main_hash};
    }
    ...
    /// or has been overwritten by another new main chain in db.
    /// If `update_main_assumption` is true and we have execution results of
    /// `assumed_epoch` in memory, we will also ensure `assumed_epoch`
    /// is persisted as the main hash in db.
    ///
    ...
    pub fn block_epoch_number(&self, hash: &H256) -> Option<u64> {
        ...
        self.block_execution_result_by_hash_from_db(&hash)
            ...
            .and_then(|main| self.block_header_by_hash(&main))
    ...
    pub fn insert_block_execution_result(
        ...
        trace! {"insert_block_traces start main={:?}", epoch};
        let bloom =
            block_receipts
                .receipts
                .iter()
    ...
    /// not on the main chain after a checkpoint
