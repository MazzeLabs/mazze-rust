...
use mazze_types::{Address, Bloom, BloomInput, Space, H256};
use std::{
    error, fmt,
    ops::{Deref, DerefMut},
};
...
pub enum FilterError {
    ...
    /// There was a main chain reorganization during log filtering
    MainChainReorg {
        epoch: u64,
        from: H256,
        to: H256,
...
impl fmt::Display for FilterError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let msg = match *self {
            ...
            MainChainReorg { epoch, from, to } => format! {
                "main chain at epoch {} has been reorganized during log filtering: {:?} -> {:?}. Operation terminated to avoid inconsistent results.",
                epoch, from, to,
            },
