...
impl SnapshotDbManagerSqlite {
    ...
    pub fn on_close(
        ...
        if remove_on_close {
            ...
            //
            // It's fine to just ignore the error because Mazze doesn't remove
            // then immediate create a snapshot, or open the snapshot for
            // modification.
            //
            // Mazze will remove orphan storage upon restart.
    ...
    fn destroy_snapshot(&self, snapshot_epoch_id: &EpochId) -> Result<()> {
        ...
        let maybe_snapshot = loop {
            match self.already_open_snapshots.read().get(&path) {
                ...
                Some(None) => {
                    // This should not happen because Mazze always write on a
