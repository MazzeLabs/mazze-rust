...
pub struct State {
    ...
    // FIXME: this is a hack to get main chain from parent snapshot to a
...
impl State {
    pub fn new(
        ...
        Self {
            ...
            recover_mpt_during_construct_main_state: construct_main_state,
        }
    }
    ...
    fn commit(&mut self, epoch_id: EpochId) -> Result<StateRootWithAuxInfo> {
            ...
            && self.maybe_intermediate_trie.is_some()
        {
            ...
            self.manager.check_make_snapshot(
                ...
                &self.intermediate_epoch_id,
                snapshot_height,
                self.recover_mpt_during_construct_main_state,
            )?;
        }
