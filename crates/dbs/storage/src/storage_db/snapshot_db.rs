...
pub struct SnapshotInfo {
    ...
    // the last element of main_chain_parts is the epoch id of the snapshot
    // itself.
    #[derivative(Debug = "ignore")]
    pub main_chain_parts: Vec<EpochId>,
}
...
impl SnapshotInfo {
    pub fn genesis_snapshot_info() -> Self {
        Self {
            ...
            main_chain_parts: vec![NULL_EPOCH],
        }
    }

    pub fn get_snapshot_epoch_id(&self) -> &EpochId {
        self.main_chain_parts.last().unwrap()
    }
    ...
    pub fn get_epoch_id_at_height(&self, height: u64) -> Option<&EpochId> {
        if height < self.parent_snapshot_height {
            ...
            unsafe {
                Some(self.main_chain_parts.get_unchecked(
